...
13. **전투 판정 산식은 하나다 — 예외 없음** (2026-09-06 사용자 결정, 위반이 두 번 나와 제약으로 승격):
    ...
    - ⚠ **「원점 항」은 «원점이 무엇이냐»가 아니라 «효과의 형»이 정한다** (2026-09-06 정정).
      초판은 「원점이 **유닛**이면 그 유닛의 몸, **칸**이면 칸 반폭」이라고 썼는데 **거짓이다** —
      **퇴근 운석의 원점은 «유닛»인데 그 유닛의 몸이 안 붙는다.** 운석은 그 유닛에서 나오는 것이
    ...
    - **원점의 몸이 «붙는가»는 효과의 «형»이 정한다** (2026-09-06 사용자 결정). 두 형뿐이다:
      - **몸에서 나오는 것** — 사거리 · 자기중심 광역 · 자폭 · **시체 폭발** · 오라 · 도발.
        「그 몸이 터진다 / 그 몸에서 뻗는다」 → 원점의 몸 = **그 몸의 `HitRadius`**.
      - **자리에 떨어지는 것** — **운석** · 투사체 착탄 · 장판 · 회오리 · 수류탄 ·
        **착지 슬램**(보스 도약 · 궁극기 강습).
        「어떤 좌표에 내린다」 → **몸이 없다**(칸 반폭 0.5). 그 좌표를 «지정한» 유닛의 몸은
        붙지 않는다 — 지정은 기하가 아니라 귀속이다.
      ⚠ **착지 슬램은 2026-09-07 에 이쪽으로 «옮겨졌다»** (사용자 결정: *「이건 운석과 마찬가지로
        착지지점(좌표) 기준 거리 N」*). unit 23b 가 「보스의 몸이 내리찍는 것」으로 읽어 몸형에
...
0. **신규 기능의 «성질»은 에이전트가 정하지 않는다** (2026-09-06 사용자 결정: *「차라리 앞으로 신규
   기능들에 대해서 나한테 먼저 물어보고 정하자」*). 새 기능·새 효과·새 판정 축이 나오면 **설계를
   제안하기 전에 먼저 묻는다.** 「합리적 기본값으로 진행 후 고지」는 **기존 기능의 구현 세부**에만
   적용되고, **무엇을 만드느냐·그것이 어떤 성질이냐**에는 적용되지 않는다.
   - 판별: 이 결정이 «플레이어가 겪는 규칙»을 바꾸면 → 묻는다. 코드 구조·이름·분할이면 → 알아서 한다.
   - 실패 사례: 감지 반경 spec 에서 **비행 적 저작을 임의로 켰다**(배치 구역에 파고드는 새 성질을
     낳았다). 그리고 전투 판정 통일에서 «시체·착탄점에 몸이 있나»·«퇴근 운석의 형»을 물었더니
     **에이전트가 상정한 두 답 어디에도 없는 제3의 축**(「몸에서 나오는가 / 자리에 떨어지는가」)이
     돌아왔다 — 물어보지 않았으면 규칙이 틀린 채로 박혔다.

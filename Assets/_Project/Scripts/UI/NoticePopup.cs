...
namespace Wassup.UI
{
    ...
    // alert(제목+메시지+[닫기]). play 게이팅(unit 1)과 score 실패 알림(unit 2)이
    // 공유하며, 정적 TournamentMatchReporter 콜백에서도 불러야 하므로 DontDestroyOnLoad
    // 단일 인스턴스 + 정적 API 로 둔다(SceneTransition 선례).
    //
    ...
    public class NoticePopup : MonoBehaviour
    {
        ...
        // 실패 안내. 닫기 단독 — 재시도는 로비 '시작' 재탭(=새 시도)과 동일해서 두지 않는다.
        public static void ShowAlert(string title, string message)
        {
            if (Instance == null) { Debug.LogWarning("[NoticePopup] 인스턴스 없음 — ShowAlert 무시."); return; }
            Instance.ShowAlertInternal(title, message);
        }
        ...
        private void Build()
        {
            ...
            dimGo.GetComponent<Image>().color = UiOverlay.Dim;
            ...
            _closeButton = MakeCloseButton();
            _closeButton.onClick.AddListener(OnClose);

            UiLayer.Apply(gameObject);
        }

        // 닫기 버튼 — gold 주액션, 하단 중앙.
        private Button MakeCloseButton()
        {
            var go = new GameObject("닫기", typeof(RectTransform), typeof(Image), typeof(Button));
            ...
            var t = CreateLabel(rt, "Label", "닫기", 34, TextAlignmentOptions.Center, BtnTextDark, FontStyles.Bold);

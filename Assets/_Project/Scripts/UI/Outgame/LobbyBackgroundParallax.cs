...
namespace Wassup.UI
{
    ...
    //
    // 틸트 = 앰비언트 자동 드리프트(다주기 sin) + 로비 캐릭터 드래그. 둘을 합산해 스프링으로 감쇠한 뒤
    ...
    public class LobbyBackgroundParallax : MonoBehaviour
    {
        ...
        [Header("드래그 연동")]
        [Tooltip("이 로컬px/s 드래그에서 틸트 |1| 포화.")]
        [SerializeField] private float dragRefSpeed = 800f;
        [Tooltip("드래그 틸트 배율. 음수면 방향 반전(튜닝 노브).")]
        ...
        private Vector2 _dragRaw;          // 드래그 속도(로컬px/s)
        private float _lastDragFeed = -99f;

        private void OnEnable() => LobbyKeyringDrag.DragMoved += OnDragMoved;
        private void OnDisable() => LobbyKeyringDrag.DragMoved -= OnDragMoved;
        ...
        private void OnDragMoved(Vector2 localDelta)
        {
            _dragRaw = localDelta / Mathf.Max(Time.unscaledDeltaTime, 1e-4f);
            _lastDragFeed = Time.unscaledTime;
        }
        ...
        private void Update()
        {
            ...
            // 드래그 staleness — 놓으면 발행이 끊기므로 0 복귀(컷신 플레이어와 같은 계약).
            if (Time.unscaledTime - _lastDragFeed > s.tiltStalenessSeconds) _dragRaw = Vector2.zero;
            ...
            Vector2 dragTilt = Vector2.ClampMagnitude(_dragRaw / Mathf.Max(dragRefSpeed, 1f), 1f) * dragGain;
            Vector2 target = Vector2.ClampMagnitude(ambient + dragTilt, 1f);

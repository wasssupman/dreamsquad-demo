...
namespace Wassup.UI
{
    // 소형 확인 팝업 — dim + 메시지 + [취소]/[확인]. 아웃게임 UI 전용, 전투(TimeManager)
    // 의존 없음. 단일 역할 뷰 — Manager 싱글톤 아님. 페이지 빌더가 페이지 루트 아래
    // 생성해 컨트롤러에 주입한다.
    //
    // authored-preset-removal unit 0 — 구 `PresetConfirmPopup` 에서 개명. 원래도 preset
    // 전용 로직은 없었고 확인 버튼 라벨만 "적용" 으로 하드코딩돼 있었다. 라벨을 인자로
    // 받게 열어 page-local-presets 의 미저장 경고("이동")가 같은 위젯을 쓴다.
    public class ConfirmPopup : MonoBehaviour
    {
        ...
        // EnsureBuilt 는 1회만 도므로 라벨을 빌드 시점에 고정하면 두 번째 Show 의 다른
        // 라벨이 반영되지 않는다. 텍스트를 들고 매 Show 에서 대입한다.
        ...
        public void Show(string message, Action onConfirm, string confirmLabel = "확인")
        {
            ...
            if (_confirmLabel != null)
                _confirmLabel.text = string.IsNullOrEmpty(confirmLabel) ? "확인" : confirmLabel;
        ...
        private void EnsureBuilt()
        {
            ...
            _confirmLabel = MakeButton(panel.transform, "확인", ConfirmBg, new Vector2(170, 60), OnConfirm);

            _root.SetActive(false);
        }

        private TMP_Text MakeButton(Transform parent, string label, Color color, Vector2 bottomPos, Action onClick)
        {

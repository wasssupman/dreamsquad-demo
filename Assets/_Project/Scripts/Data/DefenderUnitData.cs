...
namespace Wassup.Data
{
    ...
    public class DefenderUnitData : ScriptableObject, ISpineUnitVisualData, IDefenderSpineExtras
    {
        ...
        // 파생식**. rev 2026-09-04(사용자 결정): `min(W,H)/2` 내접원 → **`가로/2`(열만, 행 배제)**.
        // 적은 정면(세로 진행)에서 오므로 유닛의 "크기"는 레인을 가로막는 폭이고, 세로 깊이는
        // 반경에 기여하지 않는다 (1×2 → 0.5, 2×3 → 1.0, 3×2 → 1.5). 표가 아니라 식이라 신규
        // footprint 에 자동 적용. 판정은 `d² ≤ (사거리 + 내몸 + 상대몸)²` — 오차 보정 항 없음
        // (unit 9). ⚠ 저작 필드 `bodyRadius`(0.25)는 rev 3 에서 은퇴 — 크기 축이 footprint
        // 하나로 수렴한다(rev 2 는 축이 둘이라 조합이 무한이었다).
        public float BodyRadiusTiles => Footprint.x * 0.5f;

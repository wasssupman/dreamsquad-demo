...
{
    // effect-tiles unit 0 — 맵 seed 결정론으로 효과 타일을 놓을 Place 셀을 선정하는 순수 함수.
    ...
    public static class EffectTilePlacer
    {
        ...
        // Place 셀만, 중복 없음, count 상한(가용 셀보다 크면 전부). 수집은 row-major 라 결정론.
        public static List<int2> SelectCells(in GeneratedMap map, int seed, int count)
        {
            ...
            for (int x = 0; x < map.gridSize.x; x++)
            {
                ...
                if (map.TileAt(cell) == MapTileType.Place)
                    placeCells.Add(cell);
            }

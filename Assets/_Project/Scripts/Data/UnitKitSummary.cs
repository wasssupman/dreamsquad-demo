...
namespace Wassup.Data
{
    ...
    public static class UnitKitSummary
    {
        ...
        public static string Build(DefenderUnitData u)
        {
            ...
            // directional-attack-shape rev 3 — 도형은 **부가 타격**만 거르므로 문안도 「최대 N체 동시 타격」을 키운 형태다
            // (N = 1 이면 도형 문안 없음 — 효과 0). ⚠ 「전방」을 쓰지 않는다(배치 방향이 없다) — 방향은 «때리는 놈 쪽».
            // bake 를 지나 정의역 밖(reflex)은 Omni 문안으로 떨어진다(sim 과 같은 폴백).
            var shape = AttackShapeBake.From(u.attackShape, out _);
            if (u.attackTargetCount > 1)
            {
                if (shape.kind == AttackShapeBaked.SectorKind)
                    traits.Add($"휘두르는 쪽 {u.attackShape.angleDeg:0.#}° 안 최대 {u.attackTargetCount}체 동시 타격");
                else if (shape.kind == AttackShapeBaked.BandKind)
                    traits.Add($"찌르는 방향 일직선(세로 폭 {u.attackShape.width:0.#}) 최대 {u.attackTargetCount}체 동시 타격");
                else

...
namespace Wassup.Bridge
{
    ...
    public partial class BattleBridge
    {
        ...
        private Wassup.Core.DcHostProfile BuildHostProfile(Entity defender)
        {
            ...
            // 판정 순서 고정: 폭탄맨도 DeployedFacing 을 가지므로 BombThrow 를 먼저
            // 걸러야 한다. HazardCast 는 attackRange 0 이라 RESOLVE 에 못 가는 부류.
            ...
            else if (_em.HasComponent<VolleyFireState>(defender) && _em.HasComponent<DeployedFacing>(defender))

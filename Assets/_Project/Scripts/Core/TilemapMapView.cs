...
namespace Wassup.Core
{
    ...
    public class TilemapMapView : MonoBehaviour
    {
        ...
        // 그린다(대상 몸은 대상마다 달라 **대상의 그림자**가 말한다). 상수 시절엔 0.5 고정이었다.
        // ⚠ 이 선언과 실제 링 그리기가 2026-09-04 까지 어긋나 있었다 — 아래 `ShowRangeRing`
        // 주석 참조. **칸 채움**은 링과 달리 여전히 표준 잡몹(0.25)을 가정한다(칸은 크기를
        // 표현할 수 없다) — 그래서 채움이 링보다 최대 0.25칸 바깥까지 칠해질 수 있다.
                                      ...
                                      float selfBodyRadiusTiles = 0f, Vector2 centerOffsetTiles = default)
        {
            ...
            // unit 5 — 윤곽. **셰이더 인자는 술어와 1:1 이다**(rev 3): 몸 = 원 → `_HalfExtent` 0.
            //
            // ⚠ **rev 2026-09-04(사용자 결정 A · unit 20 리뷰): 표준 상대 항을 뺀다.**
            // 링 반경 = `사거리 + 내몸` 이고 **대상 몸은 대상의 그림자가 말한다** — 그래야
            // 「적 그림자가 링에 닿으면 사거리 안」이 판정식(`d ≤ 사거리 + 내몸 + 상대몸`)과
            // 정확히 동치가 된다. 이 파일 위쪽 주석(`selfBodyRadiusTiles` 절)이 원래 그렇게
            // 선언하고 있었고, **코드만 표준 상대(0.25)를 더해 자기 문서와 어긋나 있었다**
            // (형제 경로 `SetAreaRange` 는 처음부터 안 더한다).
            // 종전 형태는 「칸 채움과 테두리를 맞춘다」가 이유였는데, 채움은 칸 단위라
            // 상대 크기를 표현할 수 없어 **표준 잡몹**을 가정할 수밖에 없다. 적 몸이 티어로

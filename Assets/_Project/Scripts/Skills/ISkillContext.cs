...
namespace Wassup.Skills
{
    ...
    // 거리 자 — 셋 다 원이고 셋 다 몸 걸침이다. **다른 것은 「원점 항」 하나뿐**이고,
    // 그 값을 정하는 것이 **효과의 형**이다(제약 13):
    //   SelfArea   **몸에서 나오는 것** — 반경 = tileRange + **시전자 몸** + 대상 몸.
    //              자기중심 광역·자폭·오라·도발. 같은 카드도 붙는 host 에 따라 넓이가 다르다.
    //   CellArea   **자리에 떨어지는 것** — 반경 = tileRange + 칸 반폭(0.5) + 대상 몸.
    //              칸 조준 광역·착탄. 0.5 는 **몸이 아니라** 「칸 하나를 덮는 도형 보정」이고,
    //              그 일이 곧 「반경 1 = 여덟 이웃 전부」(대각 1.414 ≤ 1.5)다.
    //   Euclidean  사거리/비행 거리. 반경 = tileRange + 대상 몸. 발사명세(EmitPattern)처럼 「탄이
    //              실제로 가는 거리」라 칸 반폭을 더하면 사거리 밖 후보를 골라 탄이 도중 소멸한다.
    //
    // ⚠ **구 `AreaCircle`(= 오늘의 `CellArea`)이 자기중심까지 삼켰던 것이 unit 23 의 결함이다.**
    // 이름이 「원」만 말해서, 폭심이 «몸 있는 유닛» 인 경우에도 칸 반폭이 옳아 보였다.

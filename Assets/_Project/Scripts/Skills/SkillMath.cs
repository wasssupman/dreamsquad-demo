namespace Wassup.Skills
{
    ...
    public static class SkillMath
    {
        ...
        // **표준 «방어유닛» 의 몸 반지름** = 로스터에서 가장 흔한 footprint 폭 2 → 가로/2 = 1.0.
        // 위 상수의 형제다 — 저쪽이 「적을 기준으로 그린다」면 이쪽은 **「방어유닛을 기준으로 그린다」**.
        //
        // 쓰는 곳: **착지 슬램 예고**. 예고가 칸을 칠할 때 그 칸을 «빈 칸»(반폭 0.5)으로 보면
        // 두 가지가 동시에 나빠진다(2026-09-07 실측, 강습 반경 2):
        //   ① **모양** — 도달이 정확히 3.0 이라 축 칸 (3,0) 만 «경계 동률»로 들어오고 이웃
        //      (3,1) 은 3.16 이라 빠진다 → 5×5 덩어리에 **축으로 뿔 4개**. 원으로 안 보인다.
        //      (사용자 지적: *「완전한 원이어야지 왜 타원형을 그리지」*.) 반경이 정수에 딱
        //      떨어지면 격자에서 항상 이 모양이 나온다 — 3.5 는 그 동률에서 벗어난다.
        //   ② **거짓 안심** — 예고 밖인데 맞는 앵커가 2×2 2곳 · 배스티온 3곳 · 캐논 2곳이었다.
        //      1.0 으로 올리면 배스티온 1곳만 남는다(대가: 안 맞는데 칠하는 자리가 늘어난다 —
        //      «맞는데 예고 밖»이 배치 결정을 배신하는 쪽이라 그 방향으로 오차를 몰았다).
        // 즉 「이 칸이 비었나」가 아니라 **「이 칸에 유닛을 놓으면 맞나」**를 그린다.
        // ⚠ **판정에 쓰지 말 것** — 판정은 언제나 그 상대의 실제 몸을 잰다. 이건 표기 기준이다.
        // ⚠ 로스터가 바뀌면 이 값도 바뀐다 — `StandardDefenderBodyDriftTests`(Assets lane)가
        //    실제 `.asset` 을 세어 고정한다. 여기만 고치면 그 그물이 빨개진다.
        public const float StandardDefenderBodyRadiusTiles = 1.0f;

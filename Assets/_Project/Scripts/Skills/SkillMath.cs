namespace Wassup.Skills
{
    ...
    public static class SkillMath
    {
        ...
        // ⚠ **`StandardDefenderBodyRadiusTiles`(1.0) 는 만들었다가 철거했다**(2026-09-07).
        //   착지 예고를 「이 칸에 유닛을 놓으면 맞나」로 그리려고 넣었는데, 사용자 결정이
        //   *「보스 리프는 중심점을 기준으로 N거리 적용한다고 몸체 크기 상관없이. 운석과 같은
        //   메커니즘」* 이다 — **표기에 몸을 끌어들이지 않는다.** 예고는 `ReachFromCell(…, 0)` 이고
        //   도달은 N + 칸 반폭이다. 이 값이 **정수가 될 수 없어서** 축 뿔도 구조적으로 안 생긴다.
        ...
        // ── 방향 도형 게이트 (directional-attack-shape unit 0 · rev 3) ──────────────
        //
        // 반경 판정(`Reach`) **뒤에 AND 로 곱해지는 항**이다. 길이는 원이 정하고, 도형은 「주 대상 쪽으로
        ...
        // 주 대상 쪽 띠. 상자 = `along ∈ [0, length]` · `|across| ≤ halfWidth`. `length` 는 호출부가

    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            struct appdata_t
            {
                ...
                float2 crease  : TEXCOORD2; // .x = 크리스 깊이(0..1), .y = 면 셰이드(로우폴리)
            };
            ...
            v2f vert(appdata_t v)
            {
                ...
                o.uv = v.texcoord;
                // .x = 크리스 그림자, .y = 면 셰이드(unfold=1 이면 중립 1.0 → 무회귀).
                o.ao = float2(saturate(v.crease.x) * k, lerp(1.0, v.crease.y, k));

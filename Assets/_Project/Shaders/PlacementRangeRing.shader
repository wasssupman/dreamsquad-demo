Shader "Wassup/PlacementRangeRing"
{
    ...
    //
    // ⚠ 이력: 「`_Range` 하나짜리 원 셰이더로 만들지 말 것」(2026-08-31)은 **2026-09-01 외부
    // 세션 결정(몸 = 원 회귀 — distance-based-range 계약 1 rev 3)으로 폐기**됐다. 이제 몸이
    // 원이라 호출부가 `_HalfExtent = (0, 0)` 을 공급해 항상 원이 그려진다. 셰이더는 그대로
    // 둔다 — 파라미터가 판정식과 1:1 이면 저작(공급값)만 바뀌고 여기는 그대로라는 원리가
    // 이 전복에서도 작동했다(코드 무변).
    //

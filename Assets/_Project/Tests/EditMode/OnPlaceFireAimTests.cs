...
namespace Wassup.Tests.EditMode
{
    ...
    public class OnPlaceFireAimTests
    {
        private static bool Run(float2 host, bool hasAim, float2 aim, float2[] candidates,
                                out float2 dir, out int picked)
        {
            var arr = new NativeArray<float2>(candidates.Length, Allocator.Temp);
            for (int i = 0; i < candidates.Length; i++) arr[i] = candidates[i];
            bool ok = OnPlaceFireAim.TryResolve(host, hasAim, aim, arr, out dir, out picked);
            arr.Dispose();
            return ok;
        }

...
namespace Wassup.Tests.EditMode
{
    ...
    public class FrontmostTargetingTests
    {
        ...
        // not the allocator's Entity.Index/Version.
        [Test]
        public void EqualFlowAndDistance_BreaksBySimEntityId()
        {
            var a = new FrontmostTargeting.Candidate { flowDist = 3, sqDist = 4f, simId = 7 };
            var earlier = new FrontmostTargeting.Candidate { flowDist = 3, sqDist = 4f, simId = 4 };
            Assert.IsTrue(FrontmostTargeting.RanksBefore(earlier, a), "lower simId (earlier spawn) ranks first");
            Assert.IsFalse(FrontmostTargeting.RanksBefore(a, earlier), "strict order — no both-ways");

            // 미발급은 맨 뒤로 밀린다(0 폴백이면 «0번 유닛» 의 순위를 훔친다).
            var unassigned = new FrontmostTargeting.Candidate
            {
                flowDist = 3, sqDist = 4f, simId = Wassup.Battle.Units.SimEntityId.Unassigned,
            };
            Assert.IsTrue(FrontmostTargeting.RanksBefore(a, unassigned));
        }

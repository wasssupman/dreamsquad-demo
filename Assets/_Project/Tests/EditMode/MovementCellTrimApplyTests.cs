...
namespace Wassup.Tests.EditMode
{
    ...
    public class MovementCellTrimApplyTests
    {
        ...
        private static FlowFieldSingleton MakeField(out NativeArray<float2> flow, out NativeArray<int> dist)
        {
            int w = 4, h = 2;
            ...
            for (int i = 4; i < 8; i++) flow[i] = float2.zero; // row1 = walls
            var f = new FlowFieldSingleton();
            f.flow = flow; f.dist = dist; f.gridSize = new int2(w, h); f.goalCell = new int2(3, 0);
        ...
        {
            NativeArray<float2> flow; NativeArray<int> dist; var field = MakeField(out flow, out dist);
        ...
        public void ObstacleCell_Clamped()
        {
            ...
            obs.blockedCells.Dispose(); flow.Dispose(); dist.Dispose();
        }
    }
}

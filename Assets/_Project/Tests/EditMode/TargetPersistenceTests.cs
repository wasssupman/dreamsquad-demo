    ...
    public class TargetPersistenceTests
    {
        ...
        private static bool Keeps(float gapTiles, int range)
            // unit 9 — 양쪽 몸 0.25 합 0.5 = 상수 시절과 같다. 기대값 보존.
             => TargetPersistence.KeepsLock(true, At(0f), At(gapTiles), range, Tile, 0.25f, 0.25f, AttackShapeBaked.Omni);
        ...
        {
            Assert.IsFalse(TargetPersistence.KeepsLock(false, At(0f), At(0f), 3, Tile, 0.25f, 0.25f, AttackShapeBaked.Omni));
        }
        ...
        public void Maintain_IsWiderThanAcquire_ByExactlyTheHysteresis()
        {
            ...
            Assert.IsFalse(AttackReach.InReach(At(0f), At(justOutside), range, Tile, 0.25f, 0.25f, AttackShapeBaked.Omni, 0),
                "획득은 이미 놓쳤다");

...
namespace Wassup.Tests.EditMode.UnitStatImport
{
    ...
    public class DcAttachRequirementValidatorTests
    {
        ...
        private static DreamcatcherCard Card(DcAttachRequireKind kind,
        ...
        {
            Assert.IsEmpty(Warn(Card(DcAttachRequireKind.None)), "제한 없음 = 검증 대상 아님");
            Assert.IsEmpty(Warn(Card(DcAttachRequireKind.Class, cls: DefenderClass.Guardian)));
            Assert.IsEmpty(Warn(Card(DcAttachRequireKind.UnitId, unitId: "shield_shuttle")));
            Assert.IsEmpty(Warn(null), "null 카드는 조용히 통과");
            // 범위 밖이어도 제한이 없으면 신고 대상 아님 — 기존 Squad 카드 전부가 여기 해당.
            Assert.IsEmpty(Warn(Card(DcAttachRequireKind.None, type: CardType.Squad)));
        }

        [Test]
        public void InvalidValues_AreFlaggedOnce()
        {
            var cls = Warn(Card(DcAttachRequireKind.Class, cls: DefenderClass.None));
            Assert.AreEqual(1, cls.Count, "Class×None 은 1건");
            Assert.That(cls[0], Does.Contain("fail-closed"));

            var uid = Warn(Card(DcAttachRequireKind.UnitId, unitId: ""));
        ...
        {
            var card = Card(DcAttachRequireKind.UnitId, unitId: "no_such_unit");
        ...
        public void OutOfScopeSetting_IsFlagged()
        {
            ...
            var squad = Warn(Card(DcAttachRequireKind.Class, cls: DefenderClass.Guardian,
                type: CardType.Squad));
            Assert.AreEqual(1, squad.Count);
            Assert.That(squad[0], Does.Contain("type=Squad"));

            // 적 지정 카드도 defender 게이트를 안 탄다.
            var bounty = Warn(Card(DcAttachRequireKind.Class, cls: DefenderClass.Guardian,
                bountyMark: true));
        ...
        public void OutOfScopeAndInvalid_AreReportedTogether()
        {
            ...
            var w = Warn(Card(DcAttachRequireKind.Class, cls: DefenderClass.None,

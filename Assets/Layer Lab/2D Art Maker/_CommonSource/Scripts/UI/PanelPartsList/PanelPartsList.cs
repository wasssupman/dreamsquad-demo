...
/// <summary>
        ...
        public void Show(PartsType partsType, string[] parts, bool canHide)
        {
            ...
            if (DemoControl.CanChangeColor(partsType))
            {
                ...
                // 파츠 선택 시 Hex 업데이트 트리거 / Trigger hex update on parts selection
                if (ColorFavoriteManager.Instance != null)
                {
                    // 약간의 지연 후 Hex 업데이트 (컬러피커 상태 업데이트 후) / Update hex after slight delay (after color picker state update)
        ...
        private System.Collections.IEnumerator UpdateHexAfterDelay()
        {
            ...
            yield return new WaitForEndOfFrame(); // 한 프레임 더 대기 / Wait one more frame
        ...
        {
            // 싱글턴 인스턴스 null 체크 - 종료 시 파괴 순서 보장 불가 / Null check singleton instances - destruction order not guaranteed on quit
            if (Player.Instance != null && Player.Instance.PartsManager != null)
            {
                Player.Instance.PartsManager.OnColorChange -= OnChangeColor;
                Player.Instance.PartsManager.OnChangedParts -= ChangePartType;
            }

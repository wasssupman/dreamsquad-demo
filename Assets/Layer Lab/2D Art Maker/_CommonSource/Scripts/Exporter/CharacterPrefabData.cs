...
namespace LayerLab.ArtMaker
{
   public class CharacterPrefabData : MonoBehaviour
   {
       ...
       {
           // ColorPicker 인스턴스가 초기화될 때까지 대기 / Wait until ColorPicker instance is initialized
       ...
       {
           // 리플렉션 대신 public API 사용 / Use public API instead of Reflection
       ...
       private void AddColorPickerPositionData(PartsType partType)
       {
           if (ColorPicker.Instance != null)
           {
               ...
               if (position.x >= 0) // 유효한 위치값인 경우에만 저장 / Save only if position is valid
               {
       ...
       public Vector2 GetSavedColorPickerPosition(PartsType partType)
       {
           ...
           return new Vector2(-1, -1); // 저장된 위치가 없음을 나타냄 / Indicates no saved position
       }
       ...
       {
           // 기존 데이터 제거 / Remove existing data
           RemoveExistingPositionData(partType);

           // 새 데이터 추가 / Add new data

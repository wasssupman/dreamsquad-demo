...
namespace LayerLab.ArtMaker
{
    ...
    public class PartsManager : MonoBehaviour
    {
        ...
        private bool _needsOptimize = false; // 스킨 최적화 지연 플래그 / Deferred skin optimization flag
        private Skin _combinedSkin; // 합성 스킨 재사용 객체 / Reusable combined skin object
        private readonly List<KeyValuePair<Slot, Color>> _cachedSlotColors = new(); // 슬롯 색상 캐시 / Cached slot color references
        private bool _slotColorCacheDirty = true; // 슬롯 색상 캐시 무효화 여부 / Whether slot color cache needs rebuild

        // 모든 PartsType 열거형 값 캐시 / Cached array of all PartsType enum values
        public static readonly PartsType[] AllPartsTypes = (PartsType[])Enum.GetValues(typeof(PartsType));

        // 색상 적용 슬롯 접두사 배열 / Slot prefix arrays for color application
        private static readonly string[] SkinPrefixes = { "body", "leg_l", "leg_r", "arm_l", "arm_r", "head" };
        private static readonly string[] HairPrefixes = { "hair", "helmet_hair" };
        private static readonly string[] BeardPrefixes = { "beard" };
        private static readonly string[] BrowPrefixes = { "brow" };
        #endregion
        ...
        {
            // 부품 타입과 스킨 리스트 매핑 설정 / Set parts type to skin list mapping
        ...
        {
            foreach (PartsType partsType in AllPartsTypes)
            {
        ...
        {
            foreach (PartsType skinType in AllPartsTypes)
            {
        ...
        public void SetSkinActiveIndex(Dictionary<PartsType, int> indexList)
        {
            ...
            {
                OnChangedParts?.Invoke(kvp.Key, kvp.Value);
            }
        ...
        {
            return CustomSlotColors.TryGetValue(slotName, out var color) ? color : Color.white;
        }
        ...
        public Skeleton GetSkeleton() => _skeleton;

        /// <summary>
        /// 애니메이션 상태 가져오기
        /// Get animation state
        /// </summary>
        /// <returns>애니메이션 상태 / Animation state</returns>
        public AnimationState GetAnimationState() => _animationState;
        ...
        /// <summary>
        /// 특정 부품이 숨김 상태인지 확인
        /// Check if a specific part is hidden
        /// </summary>
        /// <param name="partsType">부품 유형 / Parts type</param>
        /// <returns>숨김 상태 / Hidden status</returns>
        public bool IsPartHidden(PartsType partsType) =>
            _hideStatus.ContainsKey(partsType) && _hideStatus[partsType];
        ...
        public void UpdateCharacter()
        {
            UpdateCharacterSkin();
            UpdateCombinedSkin();
            if (IsOptimizeSkin) _needsOptimize = true;
        }

        /// <summary>
        /// LateUpdate에서 지연된 최적화 자동 실행
        /// Auto-execute deferred optimization in LateUpdate
        /// </summary>
        private void CheckDeferredOptimize()
        {
            if (_needsOptimize)
            {
                OptimizeSkin();
                _needsOptimize = false;
            }
        }

        /// <summary>
        /// 지연된 최적화 실행 (외부에서 호출)
        /// Execute deferred optimization (called externally)
        /// </summary>
        ...
        private void UpdateCombinedSkin()
        {
            ...
            if (_combinedSkin == null)
                _combinedSkin = new Skin("character-combined");
            else
                _combinedSkin.Clear();
        ...
        private void ApplyCustomColors()
        {
            ...
            // 캐시가 무효화되었을 때만 슬롯 참조 재구성 / Rebuild slot references only when cache is dirty
            if (_slotColorCacheDirty)
            {
                _cachedSlotColors.Clear();
                foreach (var pair in CustomSlotColors)
                {
                    var slot = _skeleton.FindSlot(pair.Key);
                    if (slot != null)
                    {
                        _cachedSlotColors.Add(new KeyValuePair<Slot, Color>(slot, pair.Value));
                    }
                }
                _slotColorCacheDirty = false;
            }

            for (int i = 0; i < _cachedSlotColors.Count; i++)
            {
                var entry = _cachedSlotColors[i];
        ...
        {
            ApplyColorToSlotsWithPrefixes(color, SkinPrefixes);
        ...
        {
            ApplyColorToSlotsWithPrefixes(color, HairPrefixes);
            _hasCustomColors = true;
        }
        ...
        {
            ApplyColorToSlotsWithPrefixes(color, BeardPrefixes);
            _hasCustomColors = true;
        }
        ...
        {
            ApplyColorToSlotsWithPrefixes(color, BrowPrefixes);
            _hasCustomColors = true;
        }

        /// <summary>
        /// 부품 타입별 통합 색상 적용
        /// Apply color by parts type
        /// </summary>
        /// <param name="partsType">부품 유형 / Parts type</param>
        /// <param name="color">색상 / Color</param>
        public void ApplyColorByType(PartsType partsType, Color color)
        {
            switch (partsType)
            {
                ...
                case PartsType.Beard:
                    ...
                    break;
                case PartsType.Brow:
                    ChangeBrowColor(color);
                    break;
                case PartsType.Skin:
                    ChangeSkinColor(color);
                    break;
            }
            OnColorChange?.Invoke(partsType, color);
        }
        ...
        {
            _animationState.SetAnimation(0, animationName, true);
        }
        ...
        private List<Slot> GetSlotsWithPrefix(string prefix)
        {
            ...
            {
                if (slot.Data.Name.StartsWith(prefix, StringComparison.OrdinalIgnoreCase))
                {
                    result.Add(slot);
                }
            }

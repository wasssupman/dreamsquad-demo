...
**판을 끝내는 것은 둘뿐이다.** `complete`(3분 만료) · `submitted`(유저 제출).
시스템이 판을 끝내는 판정은 하나도 없다 — `BattleBridge.EndMatch` 호출부를 새로 만들면
그게 곧 패배 조건의 부활이다(`score-formula.md` 와 같은 계약).
...
> 마음(골 안정도)은 판정·점수에 관여하지 않는다. 뚫려도 판은 계속되고 그 셀이 유출
> 구멍으로 열릴 뿐이다. 상세는 `score-formula.md`.

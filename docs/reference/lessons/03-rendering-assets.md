...
## `Mathf.SmoothStep(a, b, t)` 은 GLSL 의 `smoothstep(edge0, edge1, x)` 가 아니다

이름이 같아서 **edge 함수로 착각하기 쉽지만**, Unity 의 것은 `a~b` 사이를 `t` 로 부드럽게 **보간**한다. 즉 `a`/`b` 는 "구간 경계"가 아니라 **출력 범위**다. 구간을 경계처럼 넘기면 `t=0` 에서 이미 `a` 를 돌려준다.

```csharp
Mathf.SmoothStep(0.85f, 1f, 0f)   // → 0.85  (0 이 아니다!)
Mathf.SmoothStep(0f, 1f, t)       // → 이건 의도대로 동작 (a=0,b=1 이라 곡선과 일치)
```

- **실제 사례(2026-07-20)**: `SpawnAlertPresenter` 의 "마지막 15% 에서만 페이드" 를 `1f - Mathf.SmoothStep(0.85f, 1f, rt)` 로 썼다. 의도는 `rt<0.85` 면 `fade=1`. 실제로는 `rt=0` 에서 `fade = 1-0.85 = 0.15` — **수렴 연출이 시작되자마자 알파가 85% 날아갔다.** 기하학적 수렴은 정상 동작했지만 거의 투명해서 "그냥 한 번에 사라진다"로 보였다.
- **증상이 헷갈리는 이유**: 지오메트리는 멀쩡하고 알파만 무너지므로, 로그로 길이만 찍으면 정상으로 보인다. **길이와 알파를 같이 찍어야** 드러난다(길이는 서서히 줄어드는데 알파는 첫 프레임부터 0.15 고정).
- **처방**: 구간을 먼저 0~1 로 정규화한다.
  ```csharp
  float fade = 1f - Mathf.SmoothStep(0f, 1f, Mathf.InverseLerp(0.85f, 1f, rt));
  ```
- **점검법**: 값을 몇 개 찍어보면 즉시 갈린다 — 런타임 없이 검증 가능하다.
  ```csharp
  // rt:      0     0.5    0.85   1.0

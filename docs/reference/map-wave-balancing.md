...
## 사거리 판정이 바뀌었다 (distance-based-range, 2026-08-31)

「N칸 안」의 자가 **체비셰프(정사각형) → 몸 사이 거리(원)** 로 바뀌었다. 밸런스를 만질 때
**대역별 손실이 균일하지 않다**는 것을 먼저 알아야 한다.

| 사거리 | 옛(칸 수) | 새(칸 수) | 변화 | 저작 유닛 |
|---:|---:|---:|---:|---:|
| 1 | 8 | 8 | **±0%** | 23 (41%) |
| 2 | 24 | 20 | −16.7% | 12 |
| 3 | 48 | 36 | **−25.0%** | 12 |
...
- **사거리 1 은 손실이 없다.** 원 반지름 1.5 가 여덟 이웃을 정확히 덮는다. 가장 흔한 대역(41%)이
  무손실이므로 **전역 knob 으로 갚으려 하지 말 것** — 안 아픈 곳까지 같이 부풀린다.

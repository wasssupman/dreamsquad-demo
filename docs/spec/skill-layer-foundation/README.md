...
> 상태: **완료 2026-08-26** (units 0~5 구현·커밋). 이전 작업까지 끝났다 — migration README 참조.
...
**«통합»의 정의 = 단일 레지스트리 · 단일 어휘 · 드레인 지점 «여럿».**
...
3. **쓰기는 의도 방출.** concrete 는 상태를 바꾸지 않고 intent 를 `ctx.Emit` 한다.
   ...
   **두 컴포넌트의 원자 동시 부착**인 스킬이 있다(궁극기 — 잠금과 무적은 레이어가 갈리지만
   ...
   - **직접 쓰기(예외 3건, 폐쇄 목록)** — ECB 가 **구조적으로 표현할 수 없는** 경우에만.
     ...
     | `EmitPattern` 의 `PatternSlot` 요소 대입 | `fireCountBase` 를 **읽고-더해-쓴다**. append 가 아니라 durable 버퍼 원소 덮어쓰기라 첫 갈래도 아니다(정합성 감사 2026-08-26 — 리뷰 M-1 이 찾은 뒤 어댑터 주석에만 반영돼 있었다) | 아니오 |
...
7. **통합 = 단일 레지스트리 · 단일 어휘.** 드레인 지점은 **규칙이 정하지 개수가 정하지 않는다.**

   > **seam 생성 규칙**: 감지자가 다른 프레임 창을 가지면 seam 도 따로 난다.
   > 같은 큐를 쓰더라도 감지 위치가 드레인 위치보다 뒤면 한 프레임 밀리고,
   > 그 밀림이 하류 계약을 깬다. 그래서 단일 드레인은 **산술적으로 불가능**하다.

   **현행 목록의 정본은 `SkillSeam` enum 이다** — 문서가 아니라 코드다.
   ⚠ 이 조항은 한때 「드레인 지점 3」이었다. 3 은 그때 세어 본 값이고 계약이 아니었는데,
   문면이 숫자였던 탓에 구현이 7 이 되는 동안 문서만 뒤처졌다(정합성 감사 2026-08-26).
   **계약에 개수를 적지 않는다** — 개수는 코드가 소유하고 계약은 규칙만 갖는다.
   같은 이유로 계약 3 의 폐쇄 목록도 「셋」이 아니라 「아래 표」를 정본으로 삼는다.

8. **이벤트는 값 스냅샷이다.** `SkillFiredEvent` 가 params 값 + 대상 핸들 + 발화 위치 +
   **시전자 진영**을 싣는다. 드레인 시점에 슬롯을 재독하면 죽음 계열에서 host 가 이미 없다.

   ⚠ 진영이 뒤늦게 추가된 축이다(`CasterFaction`). 자기 죽음 seam 은 **정의상 파괴 뒤**라
   진영을 엔티티에서 못 읽는데, 종전 폴백이 「플레이어 시전 = 방어유닛 편」이라
   **적의 작별 선물이 자기 진영을 때리는** 형태였다. 「host 가 없다」는 자리·피해·반경뿐
   아니라 **진영에도 해당한다**는 것을 그때 알았다.

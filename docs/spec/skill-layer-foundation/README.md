...
7. **통합 = 단일 레지스트리 · 단일 어휘 · 드레인 지점 «여럿».**
   ⚠ 초안은 **3** 이라 적었고 구현은 **7** 이다(Periodic·Attack·Threshold·Death·Lifecycle·
   Cast·Immediate). 숫자가 틀린 게 아니라 **숫자가 계약이 아니었다** — 계약은
   「감지자의 same-frame 하류 계약이 겹치지 않으면 드레인을 합칠 수 없다」이고,
   3 은 그때 세어 본 값일 뿐이다. 새 감지 지점이 그 성질을 가지면 8번째가 생긴다.
   감지자들이 각자 same-frame 하류

...
동시에, 순진하게 부착 범위를 넓히면 **판이 통째로 달라진다.**
...
3. **캐리어 ID 는 별도 대역으로 발급한다.** 장판 캐리어는 매치 **중간**(스킬 발동 시점)에
   ...
   **타겟팅 동률 승자와 발사 RNG 열을 정한다**(`SimEntityId.cs:9~11`) — 즉 **판이 통째로
   달라진다.** 동작 무변경이어야 하는 unit 에서 이건 치명적이다.
...
- [x] 도메인 핸들 타입이 `Wassup.Skills` 안에 있고 Entities 를 참조하지 않는다
      — `Wassup.Skills.asmdef` = `references: ["Unity.Mathematics"]` · `noEngineReferences: true`.
      **이름조차 부를 수 없다**(컴파일 게이트 성립 확인 2026-08-25)
- [x] `SkillEntityId.None` 이 액티브(플레이어 시전)를 표현한다 — `CasterRef.Player(faction)`
- [x] 캐리어 ID 발급이 기존 유닛 ID 열을 **밀지 않는다**
      — **부착 자체를 하지 않는 쪽**을 택했다(구현 3 의 두 번째 선택지). 지금 부착하면 ID 열이
      밀려 판이 달라지는데 **얻는 것이 없다** — 포트가 캐리어를 참조하기 시작하는 unit 에서 연다.
- [x] 발급 싱글턴 승격이 이 spec 에 **들어오지 않았다** — M1 로 반환(README 후속 후보)
- [x] 컴파일 초록 — Runtime + 테스트 3어셈블리

**완료 2026-08-25.**

## 이 unit 이 실제로 한 것 (구현 노트)

- `Assets/_Project/Scripts/Skills/` 신설 + `Wassup.Skills.asmdef`
- `SkillEntityId`(도메인 핸들) · `CasterRef`(호출자 = 소유자, caster 없음 표현)
- **`Faction.cs` 를 Skills 어셈블리로 옮겼다** — `CasterRef` 가 진영을 표현해야 하는데
  그 파일이 Runtime 에 있으면 **순환 참조**(Skills → Runtime → Skills)가 된다.
  순수 C#(`using System;` 뿐)이라 이동 비용이 0이었다. **네임스페이스는 유지**했다 —
  참조 23파일이고 리네임은 검증 질문과 무관하다.
- `Wassup.Runtime` + 테스트 3어셈블리에 `Wassup.Skills` 참조 추가(단방향, 순환 없음)

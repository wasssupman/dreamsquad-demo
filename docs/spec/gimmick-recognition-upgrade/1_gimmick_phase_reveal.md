...
**미확인**: 선물 → 리빌 → 배치 **전체 흐름**. 이 세션의 자동 검증은 전부 `BeginReveal` 직접 호출이라 실제 라우팅 경로는 안 탔다. 특히 `GamePhase.Gimmick` 전이가 `GiftPhaseView.OnPhaseChanged`(`phase != Gift` → 패널 숨김 + 시퀀스 정리)를 한 박자 앞당기므로, **첫 판 선물 튜토리얼 홀드와 겹칠 때**를 봐야 한다. 드림캐쳐 카드가 같은 문제를 `DreamcatcherCardText` 포매터(에셋 description 은 폴백)로 푼 전례가 있다. 기믹은 4종·저빈도 튜닝이라 지금은 정적 문자열로 두되, 수치를 자주 만지게 되면 같은 포매터 패턴으로 옮긴다.

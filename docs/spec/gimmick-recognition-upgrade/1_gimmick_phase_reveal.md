...
**부족하면 다음 후보**: 그 기믹을 N번 본 뒤 자동으로 짧아지는 familiarity 게이팅. `TutorialProgress`/`PlayerProfileSO` 플래그 전례가 있다. 드림캐쳐 카드가 같은 문제를 `DreamcatcherCardText` 포매터(에셋 description 은 폴백)로 푼 전례가 있다. 기믹은 4종·저빈도 튜닝이라 지금은 정적 문자열로 두되, 수치를 자주 만지게 되면 같은 포매터 패턴으로 옮긴다.

...
- **알려진 리스크**: `summary` 에 수치가 문자열로 박혀 있어 SO 값을 재튜닝하면 **문구가 조용히 stale** 해진다. 드림캐쳐 카드가 같은 문제를 `DreamcatcherCardText` 포매터(에셋 description 은 폴백)로 푼 전례가 있다. 기믹은 4종·저빈도 튜닝이라 지금은 정적 문자열로 두되, 수치를 자주 만지게 되면 같은 포매터 패턴으로 옮긴다.

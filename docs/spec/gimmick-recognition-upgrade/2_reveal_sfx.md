...
**사용자 Play 확인 2026-08-01** — Take1(스탬프+시머) 채택 확정. 교체 지시 없음.

Take2·Take3 는 `Audio/GimmickRevealTakes/` 에 남아 있다. 나중에 톤을 바꾸고 싶으면 그 파일 내용을 `GimmickReveal.mp3` 에 덮어쓰면 된다 — GUID 가 유지돼 재배선이 없다. 더 손댈 생각이 없으면 그 폴더는 지워도 된다(백로그 항목).

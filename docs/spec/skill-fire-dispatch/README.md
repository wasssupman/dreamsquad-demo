...
> 상태: **흡수됨 2026-08-25 → [`docs/spec/skill-layer-foundation/`](../skill-layer-foundation/README.md)
> + [`docs/spec/skill-layer-migration/`](../skill-layer-migration/README.md)**
> 이 문서는 **읽기 전용 이력**이다. 최신 계약은 위 두 spec 이 갖는다.
> 흡수 사유: 사용자가 끝점을 「모든 보스/배치/특수 스킬을 하나의 레이어로」로 확장하고
> **도메인의 ECS 참조 금지**를 하드 제약으로 세웠다(2026-08-24). 그 범위에서 아래 rev 4 의
> 계약 6·12 가 어긋난다(각 항목에 폐기 표시). 살아남은 것은 **결정 기록**과 **rev 3 오류**다 —
> 새 spec 이 그것을 인용한다.
>
> ~~상태: 홀드 2026-08-12 (rev 4 작성 완료 · 미착수)~~
> ~~재개 시점 = 다음 보스 제작 때~~ — **홀드 해제 2026-08-24 (사용자 결정 override).**
>
...
5. **`SkillContext` 는 프레임 스코프 mutable 값이다.** 전 경로 `ref` 전달, lazy 풀
   ...
   선행 작업이다.
6. ~~**진영은 상대적으로 부른다**~~ — **폐기 2026-08-25.** 뒷문장(「지금 라이브 경로는 전부 적
   시전이므로 하드코딩 리터럴 교체는 이 spec 밖」)이 뒤집혔다. 사용자의 끝점이 그 경로를
   미사용이 아니게 만든다 → `skill-layer-foundation/2b_faction_relativization.md` 가 **선행**이다.
   원문: `OpponentsOf`/`AlliesOf(시전자)`, 축은 유닛 태그
...
11. **이름 충돌 회피**: `Data/Skills/Skill_*.asset` 은 이미 액티브 드림캐쳐 스킬
    ...
    `UnitSkillDef` 를 쓴다.
12. ~~**순서**: `battle-sim-extraction` M0 **앞**~~ — **폐기 2026-08-25.** M0 는 2026-08-22 완료됐고
    새 순서는 **M1 앞**이다(사용자 결정 override 2026-08-24). 단 「끝점이 전제를 뒤집었다」는
    이 조항을 해소하지 못한다 — 골든 기준선 충돌은 실재하며 그 실체가
    `skill-layer-foundation/1_golden_and_net.md`(골든이 증인이 아님) 와 `2a`(ID 열 밀림)다.
    새 spec 은 **종료 시 골든 재기준 1회**로 이를 상환한다.
    원문: `battle-sim-extraction` M0 **앞**(사용자 결정 유지). 다만 rev 3 이 M0 골든에

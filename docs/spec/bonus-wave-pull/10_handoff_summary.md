...
- **R-별 헌터 필드 분리** [S] — `DefenderFieldSystem` 의 소스 반경 R 은 살아있는 헌터들 사거리의
  **min** 이라, 근접 보너스 적이 살아 있는 동안 R 이 1 로 내려간다.
  ⚠ **증상은 거의 없다 — 과장하지 말 것.** BFS 는 소스가 하나라도 있으면 걸을 수 있는 구역
  전체를 채우므로 R 은 «도착점»만 바꾸고 도달성을 바꾸지 않는다. 사거리 긴 보스는 FSM 이
  소스 도달 전에 자기 사거리에서 먼저 멈추므로(계약 6) 행동도 사실상 같다.
  보스가 사냥을 포기하려면 **소스가 통째로 0** 이어야 하고, 그건 모든 방어유닛이 인접 8칸에
  걸을 수 있는 칸을 하나도 안 가진 «벽에 파묻힌» 배치일 때뿐이다.
  **Duel 에서는 구조적으로 불가능하다** — 230칸 중 224칸이 Walk 다(Env 기둥 6칸만 벽).
  착수 조건: 벽이 많은 맵이 라이브 풀에 들어오고, 거기서 실제로 보스가 골로 새는 것이 관측될 때.

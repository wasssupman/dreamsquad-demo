...
1. **3층 분리** (CLAUDE.md 제약 10). ① 정의 계층 `Wassup.Data` — `UnityEngine` 만 참조, `Entities`/`Battle` 타입 0(`DcMechanic.cs` 선례). ② 로직 계층 — plain 값 in/out 순수 static, 아키텍처 타입 0. ③ 아키텍처 계층 — 후보 수집·엔티티 생성·컴포넌트 쓰기. **①②는 Mono 이식 시 그대로 살아남고, 아키텍처 스왑은 ③만 다시 쓴다.** 기계 검증: unit 0 신규 파일에 `using Unity.Entities` 0건.

...
- ~~**Play 육안 확인**~~ — **2026-08-13 사용자 확인 통과.** 세부 체감(컨셉 간 구분·「공습」의 스킬 지불감·예고 반응 시간)에 대한 개별 코멘트는 남지 않았으므로, 밸런스 조정 시 그 항목들을 다시 물어볼 것.
...
- v5 덱 11개 전부 `waveGeneratorVersion 6`. **waveSeed 불변.**
- 알려진 구멍: `ClampGroupCounts` 는 슬롯당 상한(의미는 종류당) — 소비자 0 이라 방치, 종류합 단언이 알람.

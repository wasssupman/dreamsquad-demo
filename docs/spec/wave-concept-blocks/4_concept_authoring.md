...
- **공습** — Air 슬롯 하나. Air 는 Skimmer **1종뿐**이라 속도 2.5 단일 = 완전히 뭉친다. 소수(0.3)·저HP(80)·균일 속도가 계약 10 의 «스킬 한 발 값» 안전장치다.
...
**결과적으로 Skimmer 는 「공습」에서만 등장한다.** 「중장」은 `Tanker`, 「벌떼」는 `Runner`, 「원거리」는 `Shooter` 필터라 Skimmer(class `Bruiser`)를 뽑을 수 없고, 「평소」는 `Ground` 로 막혀 있다. 어느 컨셉도 `Bruiser` 를 쓰지 않는 것은 의도다 — Bruiser 는 속도 폭이 2.0~2.5 로 넓고 Skimmer 가 섞여 있어 «성질로 좁혀 뭉친다»는 계약 3 을 만족하지 못한다.

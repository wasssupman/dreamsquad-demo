...
3. waveCount 확정 후 `waveIntervalSec = timerDurationSec / (waveCount - 1)` 로 계산한다.
4. wave trigger time 은 `waveIndex * waveIntervalSec` 이다. 마지막 wave 는 180초에 예약된다.
...
```text
10 waves: interval = 180 / (10 - 1) = 20.00s
15 waves: interval = 180 / (15 - 1) = 12.86s
```

이 방식이면 10~15개 wave 모두 3분 안에 자동 호출된다. `Next Wave` 는 다음 예정 wave 를 현재 시점으로 앞당길 뿐, 총 wave 수를 늘리지 않는다.
...
`waveSeed == 0` 은 session seed 에서 파생한다. 재현성을 위해 최종 resolved seed 를 로그에 남긴다.
...
- unit pool 이 2종 미만이면 명확한 error log 후 legacy `spawns` fallback 또는 battle start 중단.
- 모든 wave 가 2종 유닛을 가진다.
- 모든 wave totalCount 가 10~15 범위다.
- 모든 wave triggerTimeSec 가 0~180초 안에 들어온다.

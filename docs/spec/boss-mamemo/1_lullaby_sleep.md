...
```
안쪽 반지름 = 마메모의 사거리(타일) + 1   ← 사거리 링까지 안 재운다
바깥 반지름 = payload.tileRange           ← 여기까지 재운다
```

> **`+1` 이 load-bearing 이다** (리뷰 H1, 2026-08-11). 공격 판정은 `tileDist > tileRange` 면
> skip 이라 **경계 링 자체가 사거리 안**이고, `SelectRing` 의 min 은 inclusive 다. `+1` 을
> 빠뜨리면 사거리 링이 재워지는데, 마메모는 방어유닛을 사냥해 **붙는** 보스라 대상이 스스로
> 그 링에 선다 — 즉 **최빈 케이스가 곧 자기무효화**다. 게다가 cap 선별이 가까운 순이라
> 자장가 3자리를 그 링이 먼저 채운다. `AuraPulseTests.Ring_MinMustBeAttackTilesPlusOne_*`
> 와 `BossLullabyTest` 의 «사거리 링은 안 잔다» 가 이 산수를 고정한다(둘 다 `+1` 을 빼면 빨개진다).
...
마메모 사거리 2 → 실효 도넛 = Chebyshev **3~4** (56칸).
...
- **`tileRange`** 는 사거리보다 **최소 2 커야** 도넛에 칸이 남는다. bake 가 빈 도넛을 거절한다.
  5 → 3(사용자 체감) → **4**(H1 수정으로 안쪽이 3이 되어 3은 빈 도넛이 됐다).
...
- [ ] **README 계약 10 이 요구한 항목(미이행)** — «1회 조우에서 자장가 N회 이상 발동».
      현재 이걸 재는 테스트도 관측도 없다. 추산으로는 생존 7~13초 / 주기 3.5초 ⇒ 2~3회지만,
      마메모 이속이 1.4(3보스 중 최저)라 방어유닛까지 걸어가는 시간이 앞의 1~2회를 먹어
      **실발동이 1~2회로 떨어질 수 있다.** unit 5 의 관측 항목으로 이관한다 — 이 축이 무너지면
      "구현은 됐는데 게임에서 안 보인다" 가 정확히 재현된다.

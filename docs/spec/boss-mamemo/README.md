...
> 상태: **units 0~4 구현 완료 · 사용자 Play 확인 2026-08-11 · 투트랙 코드리뷰 반영 완료.**
> 남은 것은 unit 5(밸런스 튜닝 관측) · 6(인계). 설계 근거·접은 대안은
> `docs/plans/2026-08-11-boss-mamemo-design.md`.
>
> **리뷰에서 잡힌 실버그 1건**: 자장가 도넛의 안쪽 경계가 사거리 링을 포함해
> **자기 평타가 자기가 재운 유닛을 깨우는** 자기무효화가 있었다(`+1` 누락). 마메모는
> 방어유닛을 사냥해 붙는 보스라 그게 최빈 케이스였다 — unit 1 문서 참조.
>
...
| 5 | 검증 | `5_play_validation.md` | Play e2e + 밸런스 튜닝 루프 (**관측 1번 = 웨이브 소요 시간**) |
...
4. **자장가는 도넛이다 — 마메모의 사거리 안은 재우지 않는다.** (unit 1 구현에서 기준 확정)
   wake-on-hit(`DamageApplicationSystem.cs:221-230`)에는 진영 게이트가 없어 방어유닛도 맞으면
   깬다(PlayMode 로 이미 고정 — `ActionLockTest`). 그런데 마메모는 `BossTag` 이라 방어유닛을
   사냥해 붙어서 때린다 → **자기가 재운 유닛을 자기 평타로 즉시 깨운다.**
   안쪽 반지름 = `GridMath.RangeToTiles(AttackState.range)`(FSM 이 "때릴 수 있나"를 재는 것과
   **같은 변환**), 바깥 = `payload.tileRange`. "앞은 때리고 뒤를 재운다".
   ~~자기 **공격 대상**을 제외~~ 초판 기준은 `AttackState.committedTarget` 을 전제했는데 그건
   START→RESOLVE 1회 수명이라(그 밖에선 항상 비어 있다) 안정된 기준이 못 된다 — 의도는 같고
   기준만 거리로 옮겼다.

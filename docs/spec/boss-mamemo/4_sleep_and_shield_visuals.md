...
**쏘는 위치가 다르다 — 이게 계약이다** (리뷰 H2, 2026-08-11 정정):

| arm | 어디에 | 몇 개 |
|---|---|---|
| 꿈의 장막(self) | host 위치 | 1개 (수혜자 == host) |
| 악몽의 가호(반경) | **수혜자 각각의 위치** | **부여 성공한 대상 수만큼** |

초판은 가호도 host 에서 1개만 쐈다. 그러면 "보스가 반짝하고 호위 실드는 소리 없이 생긴다" 가 되어
**같은 채널을 재사용한 이유("같은 사건은 같은 그림")가 정작 깨진다.** 선례인 가디언
(`ShieldCastSystem`)이 수혜자 위치에 대상 수만큼 쏜다 — 거기 맞춘다.
`poolTransforms` 가 이미 그 자리에 있어 추가 조회는 0이다.

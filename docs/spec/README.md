...
- ~~**[높음] `UltimateLeap` 착지 예고가 «사각» 인데 피해는 «원»**~~ → **해소 2026-09-07**
  ...
  **착지 슬램의 형이 «자리에 떨어지는 것»으로 확정됐고**(사용자 결정 — 운석과 같다), 그래서
  ...
  예고의 **대상 항은 표준 방어유닛 몸(1.0)** 이다 — 「이 칸이 비었나」가 아니라 **「이 칸에 유닛을
  놓으면 맞나」**를 그린다. 칸 반폭(0.5)이면 도달이 정확히 3.0 이 돼 축에 **뿔 4개** 난 모양이
  나온다(사용자 지적: *「완전한 원이어야지」*) — 격자에서 반경이 정수에 딱 떨어지면 늘 그렇다.
  ⚠ **「모서리가 사라졌다」는 거짓이었다**(리뷰 정정): 사각 모서리는 **N ≥ 4 부터** 빠지는데
  라이브 반경은 2 다 — 모서리는 지금도 칠해진다. 실제 변화는 옛 사각 25칸 ⊂ 새 원 37칸
  (**빠진 칸 0**)이고, 고쳐진 것은 「맞는데 안 칠해지던 자리」 쪽 하나다.
  ⚠ 남는 한계: 예고는 칸을 칠하고 피해는 **유닛의 몸**을 잰다. 짝수 폭 유닛은 sim 위치가 칸
  경계라 **점유 칸이 하나도 안 칠해졌는데 맞는** 자리가 남는다 — 라이브 전수 2×2 0 · 배스티온
  **1** · 캐논 0(대상 항 0.5 시절엔 2·3·2). `TileAoeTests` 가 그 하나를 핀으로 박았다.

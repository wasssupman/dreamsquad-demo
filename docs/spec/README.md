...
- ~~**[높음] `UltimateLeap` 착지 예고가 «사각» 인데 피해는 «원»**~~ → **해소 2026-09-07**
  ...
  **착지 슬램의 형이 «자리에 떨어지는 것»으로 확정됐고**(사용자 결정 — 운석과 같다), 그래서
  ...
  예고는 **「점 + 거리」**다(사용자 결정) — 착지 좌표를 중심으로 한 **원 링** 하나이고,
  **운석과 같은 함수**(`PinSkillTelegraph`)를 부른다. 칸 열거(`BuildZoneCells`)는 **철거**했다.
  ⚠ **원인은 반경이 아니라 매체였다.** 칸 열거는 「점 + 거리」를 이산 격자로 **양자화**해서,
  원 가장자리에 걸치는 띠가 「칠해짐/안 칠해짐」 이진값으로 접혔다 — 그래서 몸 있는 유닛이
  예고 밖에서 맞는 자리가 생겼다(무경고: 옛 사각 24 · 원 타일 32). 링에서는 **「내 그림자가
  링에 닿나」가 판정식 `|Δ| ≤ N+0.5+대상몸` 과 동치**라(사용자 결정 2026-09-02 D6) 오차가
...
### 적 감지 반경 — 남은 것 (`enemy-detection-range` — **spec 완료 2026-09-08**)

- ~~**「발견」 표식 프리팹 미저작**~~ → **완료** (`8cf313e1`, unit 9 — 「!」 + 몸 플래시).
  ⚠ 슬롯 null 이 이제 `LogError` 다(unit 5 의 「미할당 = 정상」 예외 폐기).
- ~~**Play 육안 4종**~~ → **전항 통과 2026-09-08.**
- ~~**비행이 배치 구역으로 파고드는 새 성질**~~ → **사용자 수용 2026-09-08.** 되돌리기는 저작
  한 줄(`Enemy_Skimmer`·`Enemy_Dragon` 의 `detectionRange = 0`). 「비행 감지는 두되 배치 구역만
  못 넘게」는 **새 개념이라 별도 spec** — 값 조정으로 착각해 끼워 넣지 말 것.

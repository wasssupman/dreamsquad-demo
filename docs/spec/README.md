...
- ~~**[높음] `UltimateLeap` 착지 예고가 «사각» 인데 피해는 «원»**~~ → **해소 2026-09-07**
  ...
  **착지 슬램의 형이 «자리에 떨어지는 것»으로 확정됐고**(사용자 결정 — 운석과 같다), 그래서
  ...
  ⚠ **「모서리가 사라졌다」는 거짓이었다**(리뷰 정정): 원 도달이 N+1 이라 사각 모서리는 **N ≥ 3
  부터** 빠지는데 라이브 반경은 2 다 — 모서리는 지금도 칠해진다. 실제 변화는 옛 사각 25칸 ⊂ 새 원
  29칸(**축 방향 4칸 증가, 빠진 칸 0**)이고, 고쳐진 것은 「맞는데 안 칠해지던 자리」 쪽 하나다.
  ⚠ 남는 한계: 예고는 칸을 칠하고 피해는 **유닛의 몸**을 잰다. 짝수 폭 유닛은 sim 위치가 칸
  경계라, 「반 칸 차이」가 아니라 **점유 칸이 하나도 안 칠해졌는데 맞는** 자리가 생긴다
  (라이브 기준 2×2 앵커 2곳 · 배스티온 3곳 · 캐논 2곳 — `TileAoeTests` 가 반례를 핀으로 박았다).

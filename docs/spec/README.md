...
#### 세 번째 보스 마메모 (boss-mamemo — units 0~4 완료 2026-08-11, 사용자 Play 확인 + 코드리뷰 반영 완료)

웨이브 회전을 멈춰 점수를 깎는 보스. 아무도 안 죽이면서 시간을 가져간다.
신규 맥락·시스템·채널 0. 상세: `docs/spec/boss-mamemo/5_handoff_summary.md`.

- **자장가 발동 횟수 관측** [S] · README 계약 10 이 요구했으나 **아직 아무도 안 잰 유일한 축**. 마메모 이속 1.4(3보스 중 최저)라 방어유닛까지 걸어가는 시간이 앞의 1~2회를 먹어 실발동이 1~2회로 떨어질 수 있다. 이게 무너지면 "구현은 됐는데 게임에서 안 보인다". 관측 1번 = **마메모 웨이브 소요 시간**. (boss-mamemo)

...
- ~~**[높음] `UltimateLeap` 착지 예고가 «사각» 인데 피해는 «원»**~~ → **해소 2026-09-07**
  (`distance-based-range` unit 24). 예고가 판정과 같은 술어(`ReachFromCell`)를 지난다.
  **착지 슬램의 형이 «자리에 떨어지는 것»으로 확정됐고**(사용자 결정 — 운석과 같다), 그래서
  unit 23b 가 슬램에 실어 둔 보스 몸을 **양쪽에서 걷어냈다**(예고 · 생산자 2곳). 라이브 영향 0 —
  슬램 저작이 짱쎈 하나뿐이고 그 몸이 정확히 0.5 = 칸 반폭이다.
  예고는 **「중심점 기준 N거리, 몸체 크기 무관」**이다(사용자 결정 — 운석과 같은 메커니즘).
  도달 = `N + 칸 반폭`. 표기에 몸을 끌어들이면 「누구 기준이냐」가 생겨 화면이 유닛마다 다른
  규칙을 말한다 — 시행착오로 두 번 넣었다가(보스 몸 · 표준 유닛 몸) 둘 다 걷어냈다.
  부수 효과로 도달이 **정수가 될 수 없어** 축 뿔이 구조적으로 안 생긴다(몸을 섞었을 때 도달이
  3.0 이 되어 5×5 에 뿔 4개가 섰던 것이 사용자 지적 *「완전한 원이어야지」* 의 원인).
  라이브(반경 2) 기준 옛 사각 25칸 → 새 원 **21칸**, 한 일은 **모서리 4개 제거** 하나다.
  ⚠ **대가는 양방향이다.** ① 예고가 몸을 모르므로 몸 있는 유닛이 **예고 밖에서도 맞는다**
  (점유 칸이 하나도 안 칠해진 앵커: 2×2 8 · 배스티온 11 · 캐논 8 · 버스터즈 5). ② 반대로
  **안 맞는데 칠해지는** 자리도 있다(캐논 6 · 버스터즈 2). `TileAoeTests` 가 ①의 표본 2건을 박았다.
  ⚠⚠ **①은 unit 24 이전(사각 25칸)의 6/9/6/3 보다 «나쁘다»** — 「모서리는 안 맞는데 칠해졌다」는
  **대상을 점으로 놓고 잰 결과**였고, 실제로는 모서리 칸에 선 유닛이 전부 맞는다(지운 4칸은
  거짓 예고가 아니라 **진짜 경고**였다).
  ⚠⚠⚠ **그러나 원인은 «반경»이 아니라 «매체»다**(2026-09-07 재진단). `N + 0.5` 는 자리형 표기의
  **게임 전역 승인 계약**이고(사용자 결정 2026-09-02 D6 — `PinCenteredRange`), 운석·착탄 예고가
  같은 수를 쓴다. 그쪽이 성립하는 이유는 **SDF 링**으로 그려서 「**내 그림자가 링에 닿나**」를
  읽을 수 있기 때문이다 — 그 판독이 `|Δ| ≤ N+0.5+대상몸` 과 **수학적으로 동치**다.
  착지 예고만 **이산 타일 채움**(`SetTelegraphCells`)이라 그 어포던스가 없고, 링 가장자리에
  걸치는 띠가 「칠해짐/안 칠해짐」 이진값으로 접혀 통째로 «안 칠해짐» 으로 떨어진다.
  → **고칠 것은 반경이 아니라 표시 매체다**(링 추가). 반경을 몸에 맞추는 방향은 운석과 달라져
  「같은 메커니즘」을 오히려 깬다.
- **[중] 착지 슬램의 «형»이 데이터로 안 실린다** (2026-09-07, `distance-based-range` unit 24 리뷰) —
  ...
  셀 열거를 손으로 다시 돌지 말라고 하면서(*「두 계산의 우연한 일치에 기대게 된다」*) 정작 형은
  그렇게 두고 있다. 오늘은 슬램 저작이 **짱쎈 하나**고 값이 상수 0 이라 소스 정규식으로 충분하다.
  **활성화 트리거**: ① 몸형 착지 슬램을 저작하는 날 ② 일반 도약에 예고를 붙이는 날.
  그때 `UltimateLeapState.slamOriginBodyRadius` 를 실어 예고가 `ReachFromImpact(…, 그 값, 0f)` 를
  지나게 하면 소스 정규식 그물 3개가 함께 소멸한다. ⚠ **그 전에 만들지 말 것** — 값이 상수 0 인
  필드는 제약 8 의 「나중을 위한 확장 포인트」이고, unit 24 가 방금 같은 이유로 진입점
  `ReachFromUnitToCell` 을 철거했다.
...
- **[높음] 골든 코퍼스의 조건이 «머신 상태»를 상속한다** (2026-09-07 발견) — 코퍼스 전건이
  빨간 것을 추적한 결과 **코드가 아니라 조건**이었다. 방어 덱이 **`profile.json` 의 선택된
  스쿼드**를 따라간다: `_sceneDefaultPool` 캡처가 `PrepareDraftMap`(프로필 스쿼드 적용) **뒤**라
  이름은 「씬 기본 풀」인데 값은 프로필 덱이다. 덱을 명시한 `summoner` 만 통과하는 것이 그 증거.
  같은 계열이 반복된다(`DevMapOverride` 09-04 · 시트 in-memory 값 · 우발적 meta GUID 깨짐) —
  **핀은 아는 축만 막는다.** 처방: ① 모든 시나리오가 덱 명시 + 암묵 상속 폴백을 셋업 실패로
  ② **조건 지문**(맵 인덱스·스테이지·덱 id)을 골든에 함께 굽고 대조 — 지금 `configHash` 는
  「다르다」만 말하고 무엇이 다른지 안 말한다 ③ 그 뒤 전량 클린 재베이크.
  상세: `docs/spec/battle-sim-extraction/7_corpus_wide_body_scenario.md`. (battle-sim-extraction)
...
- ~~**「발견」 표식 프리팹 미저작**~~ → **저작·배선 완료 2026-09-07**(unit 9, 「!」 + 몸 플래시).
  남은 것은 **Play 육안**뿐.

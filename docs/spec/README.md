...
- ~~**[높음] `UltimateLeap` 착지 예고가 «사각» 인데 피해는 «원»**~~ → **해소 2026-09-07**
  (`distance-based-range` unit 24). 예고가 판정과 같은 술어를 지나고 **내리찍는 몸**까지 받는다
  (진입점 `ReachFromUnitToCell` 신설 — 원점은 인자, 칸 반폭은 함수의 성질). 두 방향 거짓이 함께
  사라졌다: 모서리는 겁주고(안 맞는데 칠함) 몸 큰 보스는 안심시켰다(맞는데 안 칠함).
  ⚠ 남는 한계: 예고는 「칸 위 **1×1**」에 정확하고, 몸이 큰 방어유닛은 칠해진 칸 바깥에서도 맞는다.

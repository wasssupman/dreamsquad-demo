...
자동·시각 검증으로 검증 질문 3개를 고정하고, `PresetApply` 삭제로 stale 해진 문서 포인터를 정정한다.
...
**Live 관찰 시나리오** (비차단 후속 — 실서버 계정 히스토리에 남의 참가가 있어야 한다):

1. 로비 → 히스토리 → 남의 행 덱보기 → `스쿼드 프리셋 저장` → 스쿼드 페이지에 `"{이름}의 덱"` + 유닛·스톤 채워짐 + dirty. `[저장]` → 목록 셀 썸네일이 채워지고 dirty 꺼짐
2. 같은 행 `드림캐쳐 프리셋 저장` → 드림캐쳐 페이지에 새 덱 프리셋 + 카드 채워짐 + dirty. `[되돌리기]` → 빈 덱, dirty 꺼짐
...
- [x] EditMode 전체 그린 (1,739 total / 1,737 pass / 2 existing ignored / 0 fail, 관련 47 pass)
- [x] 릴리스 검증 처분 완료
  - 자동화 가능한 코어·팝업·라우팅·페이지 픽업은 관련 47개 테스트로 고정했다.
  - PlayMode 시각 스모크로 분리 버튼의 동일 폭·독립 라벨을 확인했고 `$visual-verdict` 96/100 pass.
  - 명시적 `DeckInfoPresetApplyLiveE2ETest`는 현재 계정에 적용 가능한 외부 참가자 덱이 없어 실제 적용 assertion까지 진입하지 못했다. 외부 데이터 의존 관찰은 기능 완료를 막지 않는 후속으로 분류한다.

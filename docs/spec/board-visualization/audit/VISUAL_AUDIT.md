...
- **22 palette pass** (2026-04-25, 커밋 `6c88007`): `MapThemeData` 에 `placeBaseTint` / `walkBaseTint` / `envBaseTint` / `propGlobalTint` 4 필드 추가, `forest.asset` 에 보드게임 톤 값 입력, `MapView.CreateTileTopMaterials` 가 zone tint 를 `RuntimeMaterialFactory` 로 전달하도록 변경. **rev4 종료 시점 잔존 이슈 (2026-04-27)**: 사용자 Play 보고 — 틴트 컬러가 화면에 거의 반영되지 않고, 엣지/코너 overlay 가 시각적으로 보이지 않음. 코드 트레이스 의심선: (1) `RuntimeMaterialFactory.CreateTransparentTexture` 가 URP Unlit transparent surface mode (`_Surface=1`, `_SrcBlend`/`_DstBlend`/`_ZWrite`, `_SURFACE_TYPE_TRANSPARENT` 키워드, render queue 3000) 미설정으로 overlay 가 opaque 렌더될 가능성; (2) `Tile_Unlit` 셰이더의 `DOTS_INSTANCING_ON` 경로가 `material.SetColor("_BaseColor", tint)` 우회 가능성; (3) edge/corner overlay quad 들에 `sortingOrder` 미부여로 다른 quad 에 가려질 가능성. **결론**: rev4 의 시각 개선은 추가 ROI 부족으로 종료. 본 finding 은 후속 spec (rendering-pipeline-fix 류) 에서 root cause 부터 다룬다.

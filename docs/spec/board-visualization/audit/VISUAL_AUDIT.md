...
- **22 palette pass** (2026-04-25, 커밋 `6c88007`): `MapThemeData` 에 `placeBaseTint` / `walkBaseTint` / `envBaseTint` / `propGlobalTint` 4 필드 추가, `forest.asset` 에 보드게임 톤 값 입력, `MapView.CreateTileTopMaterials` 가 zone tint 를 `RuntimeMaterialFactory` 로 전달하도록 변경. **rev4 종료 시점 잔존 이슈 (2026-04-27)**: 사용자 Play 보고 — 틴트 컬러가 화면에 거의 반영되지 않고, 엣지/코너 overlay 가 시각적으로 보이지 않음.
- **2026-04-27 정밀 코드 트레이스 결과 root cause 갱신**: 처음 적은 의심선 (URP transparent surface mode 누락 / DOTS_INSTANCING 우회 / overlay sortingOrder 누락) 은 **검증 결과 모두 무관**. `RuntimeMaterialFactory.CreateTransparentTexture` 는 line 72-83 에서 `_Surface=1`, `_SrcBlend`/`_DstBlend`, `_ZWrite=0`, `_SURFACE_TYPE_TRANSPARENT` 키워드, render queue Transparent 를 모두 설정. `DOTS_INSTANCING_ON` 은 ECS RenderMesh 경로에서만 활성, Mono `MeshRenderer.sharedMaterial` 인 본 프로젝트 타일에는 무관. overlay y 좌표 (region 0.002 / hover 0.006 / edge 0.022) 는 transparent + ZWrite off 환경에서 자연스럽게 정렬됨.
  - **갱신된 의심 대상 3개**: (A) `MapView._tileTextureMaterials: Dictionary<Texture2D, Material>` 캐시 키가 텍스처 only 라 zone 별 tint 가 첫 zone 만 반영됨 — `CreateTileTopMaterials` line 124-136. (B) `MapView.cs` line 242 `int edgeMask = visualCell.envNeighborMask;` 로 Place ↔ Walk 경계 fringe 가 절대 안 그려짐. (C) `placeEdgeOpacity = 0.25` (27 단계 조정), `placeOuterCornerOpacity ≈ 0.42` 로 알파가 너무 낮아 베이지/녹색 텍스처 위에서 fringe 가 시각적 인지 한계 미달.
  - **결정 실험**: forest.asset `placeBaseTint = (1,0,0,1)` 빨강. Place 빨개지면 Bug A 영향 없음 → Bug B + C 만 작업. 안 빨개지면 Bug A 발화.
  - **결론**: rev4 의 시각 개선은 본 spec 에서 종료. 후속 spec (`palette-and-overlay-fix` 류) 에서 위 결정 실험 + Bug A/B/C fix 부터 시작.

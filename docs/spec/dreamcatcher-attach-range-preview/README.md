...
> 상태: **완료 2026-09-03** — 구현 + 투트랙 리뷰 반영(`abff12fd`) + 골든 정본 조건 A 확정·재베이크 +
> unit 4 육안·실기기 사용자 확인 통과(스크린샷 면제 — 사용자 결정). 인계: [`5_handoff_summary.md`](5_handoff_summary.md).
...
- **환경 사실**: 방어유닛 27종 중 24종이 2×2, 캐논 2×3 · 배스티온 3×2 · 버스터즈 1×2(`7738f4b8`, 2026-09-02). **1×1 은
  없다.** ~~몸 중심은 앵커 + ((W−1)/2, (H−1)/2)~~ **rev 2026-09-03(베이스 통일)**: sim 위치 = 베이스(발밑 = 하단 행
  가로 중앙, `anchor + ((W−1)/2, 0)`) — 캐릭터가 서는 그 점. 링은 sim 위치를 그대로 받으므로 **링 중심 = 캐릭터 발**.
...
### 2026-09-03 (후속 버그 — 「부착 후 맵 위에 타일이 남는다」)

- 공격폭증·속사 시전 뒤 남는 것은 active-ally-zone unit 2 의 **장판 바닥 점등**((2N+1)² 사각, 수명 6~8초)이었다.
  조준은 0b 로 원이 됐는데 시전 순간 사각 타일로 바뀌어 남았다. 사용자 결정: **제거**. 장판은 바닥 타일을
  칠하지 않는다(active-ally-zone 계약 5 은퇴 · 핀 `ActiveAllyZoneTest.ZoneCast_PaintsNoBoardTiles`).
  수명 표시가 필요해지면 원 링/VFX 로 재설계 — Backlog 「장판 수명 표시 재도입」.

...
> **spec 완료 2026-09-03** — 골든 조건 A 확정·재베이크, unit 4 육안·실기기 사용자 확인, README 「완료」 선언까지 마감.
> 계약은 README, 단위 계약은 번호 문서가 정본이다. 이 문서는 지도다.
...
1. ~~**골든 정본 조건 결정**~~ — **완료 2026-09-03**(사용자 확정: **A** BattleScene 직접 Play·디스크 SO). 최초 커밋
   ...
   ③ **「새 Play 세션 첫 동작으로 굽는다」는 장식이 아니다** — 재생성 전에 스크립트 프로브 판을 한 번
   돌렸더니 매치 조건 롤(기믹 등 전역 랜덤 축)이 바뀐 채 구워졌고(245d9a63), 다음 재생성에서 이벤트
   대변동으로 드러났다. 첫 동작 베이크는 세션을 건너 바이트 재현됨을 2세션 교차 Verify 로 실측(2026-09-03).
2. ~~**unit 4 육안·실기기**~~ — **완료 2026-09-03**(사용자 Play·실기기 확인, 스크린샷 면제 — 사용자 결정).
   베이스 통일 이후 기준(링 중심 = 캐릭터 발밑) + 사망폭발·퇴직위로금 원 노출 포함.
3. ~~README 「완료」 선언~~ — **완료 2026-09-03**.

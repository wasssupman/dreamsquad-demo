...
> 구현 커밋 `6ba61bec`(2026-09-02). 채움 펄스는 **기본 off**(`RangeRingStyle.pulse`) — 사거리 채움 펄스가 사용자 요청으로
> 제거된 이력(`TilemapMapView.Update` 주석)이 있어 노브만 두고 켜지 않았다. 아래 Play 항목은 unit 4 에서.

- [ ] Play: 액티브 셀 조준(`TileStatBurst` 계열, 토네이도)이 조준 셀 중심 **원**으로 뜨고 채움(정적 0.4)이 읽힌다.

...
1. **출혈 = 누적 → 임계 발동 → 갱신형 도트**(`StackModifier_Bleed`). 타격당 1스택,
   `atStack 5 · Consume` 로 5타에서 발화하고 0으로 리셋. 발동하면 **5초간 0.5초 간격 10틱 ·
   틱당 5 · 1회분 총 50**(리그 실측 정확히 50.00).
   ...
   - ⚠ **`duration` 을 `tickInterval` 의 정확한 배수에 걸치지 말 것.** 첫 틱은 즉발이고
     ...
     틱 수가 프레임레이트에 따라 흔들린다. **`duration 5.0` 은 쓰지 말 것** — 11번째 틱 자리가
     만료와 정확히 겹쳐 10틱(50)과 11틱(55) 사이에서 흔들린다. 그래서 `4.85` 다: 마지막 틱 4.5s
     이후 0.35s, 유령 틱 5.0s 앞으로 0.15s 여유. 틱 수 = `floor((duration − ε) ÷ tickInterval) + 1`.
   - **지속(4.85s)이 발화 주기(1.5s)보다 길다 — 의도된 것이다.** 계속 맞는 적은 폭발이 겹치면서
     출혈이 끊기지 않고 이어진다(`CcEffectMerge` 가 `remainingTime = max(기존, 신규)` 로 갱신하고
     `tickTimer` 를 보존하므로 틱 리듬도 유지된다). 그래서 **총 50 은 "한 번 물리고 벗어난 적"이
     받는 양**이고, 붙잡힌 적은 0.5초마다 5씩 지속해서 받는다. 2026-07-29 이전의 "폭발 지속 <
     주기" 규칙은 이 변경으로 폐기됐다.
   - ⚠ 단, `CcEffectMerge` 는 피해자당 kind 슬롯 하나만 두고 scalar 를 덮으므로 **여러 난도질꾼이
     한 적을 물어도 출혈은 합산되지 않는다**(합산은 코드 결정 — 후속).
...
· `StackModifier_Bleed`: `atStack 5 · Consume` · 틱 5 / 0.5s · 지속 4.85s (**10틱 · 1회분 50**)

**붙잡힌 단일 대상 DPS 18.9** = 직격 8.9(2.67 ÷ 0.3) + 출혈 10.0(5 ÷ 0.5s, 지속 갱신되므로 상시).
**한 번 물리고 벗어난 적**은 출혈 50 을 마저 받는다(5초에 걸쳐).
⚠ 공속을 바꾸면 발화 주기(= `atStack` × 쿨다운)가 따라 움직인다. 지속이 주기보다 길기만 하면
출혈 DPS 자체는 `틱당피해 ÷ tickInterval` 로 공속과 무관하지만, **발화까지 걸리는 시간**(첫 출혈
지연)은 공속에 비례한다 — 공속 0.3 이면 1.5초, 시트의 0.9 면 4.5초.
...
- **다중 공격자 출혈 합산** [M] → `docs/spec/cc-effect-flavor/` 후속 후보로 이관. 그 spec 이 도트에
  출처 flavor 를 실어 **파이프라인 간** 충돌(출혈 vs 화염 장판)은 해소하지만, 난도질꾼 2기처럼
  **같은 flavor 끼리**는 여전히 한 슬롯을 공유한다(flavor 는 "무엇"이지 "누구"가 아니다).

...
- 같은 스탯 스톤 4개도 stackId 가 각각 달라 additive 슬롯 4개로 공존(+30%). 버프(≥1)=additive, 감소형(<1)=multiplicative — 기존 `modifier-additive-authoring` 정책·one-directional 채널 불변식 그대로 (스톤은 percent > 0 강제라 1.0 경계를 넘지 않음).

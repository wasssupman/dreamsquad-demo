...
> 완료 확인(5차) 2026-07-04 — 육안 피드백 반영: 피커에서 장착 중 항목 딤드 (유닛 = 딤드+선택불가, 스톤 = 딤드 표시만·선택 유지 — 중복 장착 계약 보존) + 스톤 아이템 2줄 라벨(displayName + 요약)로 기성 16종 에셋 정체 노출. 리그 게이트 5/5 PASS (딤드 계약 어설션 포함).
> 완료 확인(최종) 2026-07-04 — 사용자 육안 확인 OK. 피커 상호작용/렌더 계약은 리그 물리클릭·상호작용 진단으로 게이트화되어 회귀 방지.

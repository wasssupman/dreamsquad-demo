...
- **시트 `frenzy` 행 — 생성됨**(2026-08-17 SO→시트 push, `DcCards` 51행 · `visible 1` ·
  `tileRange 10`). 이제 값의 정본은 시트다.
- ⚠ **디스크 `.asset` 이 시트보다 낡다** — 짱빠른·짱쎈버서커의 새 이름/값(50%·3초)은 Unity
  **메모리에만** 있고 `.asset` 파일은 아직 「킬속 8%/4초」다. `DreamcatcherCardNameTests` 가
  디스크 값을 핀하므로, 누가 에셋을 저장하면 그 테스트가 빨개진다. 저장할 때 이름 핀도 같이
  고칠 것.

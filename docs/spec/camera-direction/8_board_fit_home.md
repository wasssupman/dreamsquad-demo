...
## HUD 클리어 오프셋 (2026-07-24 추가)

fit 만으로는 보드 중심이 화면 정중앙(하단 vpY≈6%)이라 하단 HUD(트레이)에 보드 아랫부분이 가린다. `CameraDirectionConfig` 에 두 노브를 더해 홈 포즈를 보정한다(margin 과 독립):

- **`boardFramePullback`**(4) — forward 반대로 추가 후퇴(줌아웃). 보드가 작아져 양 끝이 화면 중앙으로 모인다.
- **`boardFrameRaiseY`**(2) — **보드를 화면에서 위로** 올리는 양(양수). ⚠ pitch 60° 로 내려다보므로 화면 상승 = **카메라 하강**(월드 -Y)이다. 카메라 Y 를 직접 올리면 시선이 밀려 보드가 오히려 **내려간다**(실측: raiseY +3 → 하단 6.2%→0.6%). 부호는 "보이는 결과" 기준으로 잡았다 — 양수 = 보드 상승.

실측 튜닝(Coil 15×12): pull4/raise2 → 보드 하단 vpY 6.2% → **18.6%**, 상단 82.9% → 85.9%. 다른 맵(15×11)에서도 하단 19.5%/상단 86.2% 로 일관.

## 후속 후보

- 세로/가로 aspect 별 margin 분리 — 실기기 aspect 편차가 크면.
- HUD 실제 높이를 읽어 raiseY 자동 산출 — 현재는 눈으로 맞춘 상수.

...
| 채널 | 이후 | 스위치 |
|---|---|---|
| 킬 스트릭 셰이크 | **켜짐** | 진폭 0.25 / 0.9 (unit 17 에서 인상 — 아래 ⚠) |
...
회전(`shakeMaxRotAmp`)은 도 단위라 프레이밍 무관이다 — 큰 맵·좁은 화면비에서는 위치 성분만
줄어들어 셰이크가 **거의 순수 roll 로 성격이 바뀐다**. 환산을 넣을 때 같이 봐야 한다.

⚠ **천장은 두 채널이 공유한다 — 지속 쪽은 이 값으로 확인된 적이 없다.** `ShakeWeight` 가
`max(한 방, 지속)` 이라 진폭 한 쌍을 임펄스와 킬 스트릭이 같이 쓴다. 한 방은 envelope 으로
0.35초에 잦아들지만 **킬 스트릭은 감쇠 없이 지속**되고, `ScoreHudView` 의 heat 상한이
`soundPitchMax - soundPitchBase` 로 나뉘어 **정확히 1.0 까지 간다**(연속 처치 시 saturate).
즉 웨이브가 몰리면 임펄스 최대 진폭이 계속 유지된다. 코드 리뷰가 이것을 지적해
2026-08-21 사용자가 **실전 킬 스트릭 구간을 따로 확인했고 문제 없음**으로 판정했다 —
그래서 지속 입력에 별도 스케일을 두지 않는다(안 겪은 문제에 노브를 만들지 않는다).
**다시 과해지면** 진폭 자체를 내리지 말고 지속 입력에만 배율을 준다 — 진폭을 내리면
배치 한 방이 같이 죽는다.

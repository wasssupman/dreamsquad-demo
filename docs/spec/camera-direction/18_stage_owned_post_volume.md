...
- [x] 코어 EditMode 테스트 통과 — 2362건, 실패 1건은 별건(`LegacyTraceV0Tests`: `core.autocrlf=true` 가 골든 코퍼스를 CRLF 로 풀어 매직이 `LTV0` + CR 이 된다. 이 unit 과 접점 0)
- [x] `dof` 식별자가 `Assets/_Project/Scripts` · `Tests` · `Editor` 에서 0건
- [x] Play: 마음 스트레스 1단계 이상에서 비네트가 보인다 (사용자 확인)
- [x] Play: 스테이지 교체 후에도 비네트 유지 (사용자 확인)
- [x] 콘솔 에러 0

확인 2026-08-27 · 구현 `21e9eed4` (배선·DoF 제거·문서) + 마무리 커밋(에디터 러너 클램프 주석).

...
| | |
|---|---|
| `b696ba41` | units 10~14 — 상태별 독립 레시피 재설계 (코드·테스트·spec) |
| `8552a193` | 화각·흐림세기 + 배틀 씬 블룸·그레이딩 (사용자 튜닝) |
| `cf8cec74` | 디버그용 `CharacterTest` 오브젝트 끔 |
| `93c1cc4c` → `71688c0a` | `fovMin` 인하 + 흐림 임계값 이관 → **되돌림**(아래 «되돌린 이유») |
| `eec00f34` | 전투 카메라를 `(0, 22, -18.5)` 로 역산 |

GitHub `main` 푸시 완료(`3dd15dc7..eec00f34`). GitLab 미러는 미반영.
...
|---|---|---|
| `pitchDeg` | 70 | **55** |
| `fitMargin` | 1.15 | 1.01037 |
| `screenY` | 0.554 | 0.50239 |
| `fov` | 31.2 | 25 → **런타임 31**(클램프) |
| 흐림 | 끔 | `1.0 ~ 1.6` · 반경 1.5 |

`fovMin 31` · `fovMax 60` · `placementFocusLead 0.15`.
판 23×10 · 화면비 2.0556 에서 배치 `(0, 23.24, -9.26)` / 전투 `(0, 23.24, -16.31)`.

### 되돌린 이유 (같은 실수를 반복하지 말 것)

전투 저작 화각 25 는 `fovMin 31` 클램프에 걸려 **런타임 31 로 동작한다.** 이것을 «저작값이
안 먹는 결함» 으로 보고 `fovMin` 을 20 으로 내렸더니, 화각이 31 → 23.6 으로 바뀌면서 카메라가
약 4 유닛 물러나 **눈으로 맞춰 둔 프레이밍이 통째로 사라졌다**(사용자 롤백 요청).

교훈: **클램프에 걸린 값을 «푸는» 것은 숫자를 고치는 일이 아니라 그림을 바꾸는 일이다.**
사용자가 눈으로 맞춘 결과는 «클램프 이후» 상태이므로, 클램프를 건드리면 그 결과가 무효가 된다.
정리하고 싶다면 `fovMin` 을 내리는 게 아니라 **저작 `fov` 를 31 로 적는 쪽**(동작 변화 0)이다.

같은 이유로 **프로파일의 `gaussianStart/End` 를 손으로 고쳐도 효과가 없다** — 흐림 임계값은
상태 레시피가 소유하고 Director 가 매 프레임 덮어쓴다. 프로파일에서 조정하려다 값이 안 먹으면
그건 버그가 아니라 소유권이다. 레시피의 `dofStart`/`dofEnd`(보드 깊이 정규화)로 적어야 한다.

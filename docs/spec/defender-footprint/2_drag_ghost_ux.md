...
- **사거리 링 양보 (rev 2)**: 사거리 표시 중인 칸은 전역 불가 표시(빨강/노랑)가 비켜 간다(`IsPlacementRangeCell` read seam) — 링 위에 얹히면 링이 붉게 물들어 사거리 읽기가 흐려진다. footprint 칸은 예외(유효성 = 핵심 신호).
...
- [x] 육안 Play: 2×2 테스트 유닛 드래그 시 고스트 4색·자석·착지 중심 확인 (**사용자 확인 대기 축**)

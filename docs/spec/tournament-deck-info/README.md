...
> 상태: units 0~2 완료 2026-07-30 · **units 4~5 완료 2026-07-31** (0점 마감 덱 전송 · 스톤 raw id 보존, 커밋 `73deadbb`)
...
- `TournamentEntry` 엔티티에도 컬럼이 있다 = **엔트리(참가)당 1개**. 엔트리 하나로 플레이하는 판이 한 판이므로 이는 곧 **그 판의 덱**이다 (서버 문구의 "최고 점수를 기록한 판"이 그 한 판을 가리킨다 — 아래 미확인 #2 참조).
...
| 4 | 0점 마감 덱 전송 | `4_zero_score_deck_info.md` | 나가기·reconcile 도 **그 attempt 의** 덱을 싣는다. pending 레코드에 덱 스냅샷 |
| 5 | 무성 실패 제거 | `5_stone_raw_id_and_warn.md` | 스톤 미해석 id 보존(계약 3 정렬) + 덱 없이 점수만 제출될 때 warning |
...
4. **모든 마감이 덱을 기록한다 — 단 "그 attempt 의" 덱만** (rev unit 4, 2026-07-31). 구 계약은 "0점은 최고점 후보가 아니다"를 이유로 0점 마감 두 경로를 빈 문자열로 두었다. 그런데 **엔트리 하나 = 판 하나**다(2026-07-31 사용자 확인) — 나가기·하드킬로 끝난 판도 자기 엔트리로 히스토리에 남고, 덱이 비면 그 엔트리의 덱보기는 영원히 "덱 정보가 없습니다"다. 즉 덱을 싣는 이유는 덮어쓰기 방어가 아니라 **그 판의 기록을 남기는 것**이다. 지금은 세 경로 모두 덱을 싣되 **출처가 경로마다 다르다**: `ReportResult`·`AbandonMatch` 는 호출자가 넘기는 라이브 로거의 덱(그 판이 아직 살아 있다), `ReconcilePending` 은 **`PendingMatchRecord.deckInfo`** — 로비에는 로거가 없고, 지금 메모리의 덱을 붙이면 남의 판에 엉뚱한 덱을 기록하기 때문이다. 판 중에 두 번(캐리인·배치 진입) 레코드에 적어두고, 그 저장은 attemptId compare-and-write 다.
...
1. ~~**덱이 실제로 왕복하는가**~~ — **확인 완료 2026-07-30** (`tournament-history-deck-view` unit 4, dev 서버 계정 `wassup`). 히스토리 덱보기에 유닛 7 + 드림스톤 4 + 카드 10 이 전부 카탈로그로 해석돼 표시됐다. **선물 카드 분리도 실증** — 같은 판 로컬 로그의 `deckCardIds` 는 12장인데 화면은 10장이었다(`baseDeckCardIds` 가 의도대로 동작).
2. ~~**0점 마감이 기록을 덮어쓰지 않는가**~~ — **질문 자체가 성립하지 않는다 (2026-07-31 사용자 확인).** `entryId` 하나로 플레이할 수 있는 판은 **한 판**이다. 그래서 0점 마감이 마감하는 것은 자기 엔트리이고, 거기에는 덮어써서 잃을 다른 점수·덱이 없다. **서버 최고점 가드도 필요 없다.**

   구 서술("엔트리가 여러 attempt 를 누적하고 서버가 최고점 것으로 갱신한다")은 swagger 필드 설명 *"최고 점수를 기록한 판의 덱 정보"* 를 1:N 으로 읽은 **검증되지 않은 추정**이었다. 반대 증거가 더 구체적이다 — `unclaimed` 의 엔트리가 각자 `score`·`rank` 를 갖고, 히스토리 목록은 참가를 행으로 쌓으며 같은 날 참가를 시각으로 구분한다(`tournament-history-deck-view` 계약 5). 판이 하나면 그 문구는 참이면서 무의미하다.
...
- ~~**드림스톤 캐리인 로그가 미해석 id 를 버린다**~~ — **unit 5 에서 해소** (raw id 보존 + warning).
- **스톤 `slotIndex` 가 payload 에서 유실된다** — 배열은 압축이라 "3번 슬롯만 장착"이 수신 측엔 0번으로 보인다. 넣으려면 포맷 v2 + 수신 UI 까지 함께 판단해야 한다. Scope: S

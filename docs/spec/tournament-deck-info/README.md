...
4. **모든 마감이 덱을 기록한다 — 단 "그 attempt 의" 덱만** (rev unit 4, 2026-07-31). 구 계약은 0점 마감 두 경로를 빈 문자열로 두었는데, 서버가 엔트리 컬럼을 최고점 가드 없이 대입하면 그 빈 값이 좋은 판의 덱을 지운다(아래 미확인 #2). 지금은 세 경로 모두 덱을 싣되 **출처가 경로마다 다르다**: `ReportResult`·`AbandonMatch` 는 호출자가 넘기는 라이브 로거의 덱(그 판이 아직 살아 있다), `ReconcilePending` 은 **`PendingMatchRecord.deckInfo`** — 로비에는 로거가 없고, 지금 메모리의 덱을 붙이면 남의 판에 엉뚱한 덱을 기록하기 때문이다. 판 중에 두 번(캐리인·배치 진입) 레코드에 적어두고, 그 저장은 attemptId compare-and-write 다.
...
2. **(unit 4 로 무력화, 잔여만 미확인) 0점 마감이 기록을 덮어쓰지 않는가** — 원래 위험은 "0점 complete 의 빈 `deckInfo` 가 좋은 판의 덱을 null 로 덮는다"였다. 계약 5 의 키 생략은 완화일 뿐 차단이 아니었고, unit 4 가 세 경로 모두 **그 attempt 의 덱**을 싣게 해 서버 가드 유무와 무관하게 엔트리에 실제 덱이 남게 했다. **남은 잔여**: 프리셋을 바꿔 재플레이한 뒤 낮은 점수/0점으로 마감하면, 가드 없는 서버에서는 엔트리의 점수(최고점 판)와 덱(마지막 판)이 어긋난다. 이건 클라가 해소할 수 없다 — 서버에 최고점 가드를 요청하는 쪽이 맞다.

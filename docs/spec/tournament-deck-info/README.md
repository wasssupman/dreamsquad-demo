...
2. **(여전히 미확인) 0점 마감이 기록을 덮어쓰지 않는가** — 좋은 판을 친 뒤 나가기(또는 다음 로비 reconcile)로 0점 complete 가 나갔을 때, 엔트리의 `deckInfo` 가 유지되는가. 계약 5 의 "키 생략"은 이 위험을 줄이는 조치일 뿐 **차단하지는 못한다** — 서버가 컬럼을 무조건 대입하면 null 로 덮인다. 덮이는 것으로 판명되면 0점 경로에서 complete 자체를 바꾸는 게 아니라 서버에 최고점 가드를 요청하는 쪽이 맞다(클라는 이미 보낼 것을 안 보내고 있다).

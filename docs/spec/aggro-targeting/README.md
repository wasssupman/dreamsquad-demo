...
4. **sticky / primary override.** 어그로된 적은 기존 타게팅을 전부 버리고 **primary 타겟**=링크된 가디언으로 고정(사거리 밖이면 미발사). 해제 전까지 불변. ★**광역 «폭» 은 이 계약의 대상이 아니다** — `attackTargetCount` 는 어그로와 무관하게 저작값대로 적용된다(`elite-whirlpot` unit 0, 2026-08-14). unit 8 의 MEDIUM 2 가 폭까지 1 로 접었으나 그것은 이 계약(«타겟» 단수)의 확장 해석이었고, **어그로가 적의 공격 «형태» 를 바꿔 붙잡힌 광역 적이 오히려 덜 때리는** 부작용을 만들어 철회했다. 고정 = `AggroAoeWidthTests`.

...
핵심 루프: **가디언 근접 → 어그로 획득 → 적이 스스로 가디언으로 보행 → 가디언 타일에 겹쳐 정지 → 가디언 sticky 공격 → 가디언 사망 시 해제 → 출구 복귀.**

## 검증 질문

> "가디언 근처(1타일)에 들어온 적이, capacity 상한 안에서, 스스로 가디언 타일로 걸어와 겹쳐 멈추고 가디언이 죽을 때까지 가디언만 공격하는가? 가디언이 죽으면 모인 적이 출구로 다시 흩어지는가?"
...
- **"시간당 어그로 수량" 파워 스탯 정식화** — 공격당 대상수×공격속도 rate 모델. 본 spec 은 capacity 상한 + 근접 즉시 배정으로 단순화. 별도 단위.

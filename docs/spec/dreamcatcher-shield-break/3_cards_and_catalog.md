...
- **B 고요한 파문**(`id=shield_lull`): payload=AreaSleep(kind 16), magnitude=3(M)·tileRange=2(N)·duration=2.5(L)·projectile 없음. 실드 파열 → 가까운 3명 2.5초 수면.
...
- 수치는 열린 결정 기본값(A 80/1, B 2/3/2.5) — Play 후 SO 튜닝.

...
4. **전이 규칙**: `Aggroed` 있음 → 가디언 tile-Chebyshev 사거리 내 `Standoff`, 밖 `Chasing`. `Aggroed` 없음 → **공유 `TrySelectTarget` 헬퍼(FocusUntilDead 락·필터·우선순위·사거리 포함)가 타겟을 찾으면** `Engaging`, 없음 `Marching`. 전이 판정 = AttackSystem fire 판정(동일 헬퍼)이라 상태=Engaging ⟺ fire 타겟 존재.

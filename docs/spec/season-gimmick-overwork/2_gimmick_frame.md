...
2. **OverworkGimmickData**: `fatigueStack`(StackModifierSO 참조 — maxStack/perAppDuration 원천), `fatigueInterval=10`, `fatigueAmount=1`, `redbullSpawnInterval=5`, `lastRunAttackSpeedMul=1.5`, `lastRunDuration=5`, `lastRunDamageFraction=0.5`(최대체력 50% 피해). 하드코딩 금지 계약의 수치 원천.

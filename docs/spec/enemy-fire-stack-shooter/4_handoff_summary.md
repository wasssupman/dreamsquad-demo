...
- **`ApplyStat` 은 일부러 안 고쳤다.** 같은 결함이지만 `Enemy_Debuffer` 의 `DamageMul ×0.6` 이
  ...
  클램프 `[0.2, 5]` 가 현재 병리를 경계하고 있다. 사유는 코드 주석 + `docs/spec/README.md`
  Follow-up Backlog.
...
- **다중 킨들러의 화상이 합산되지 않는 것은 사양이다(사용자 결정 2026-07-30).** 스택은
  `(source, kind)` 라 사수별로 따로 쌓이고(합쳐서 5가 되지 않는다), 화상은 `(origin, element)`
  라 한 슬롯으로 접혀 `remainingTime = max` 갱신만 된다. 결과적으로 **화상 화력은 마릿수에
  비례하지 않고 4.0 DPS 가 천장**이며, 2기 이상이면 펄스 공백(1.15s)이 메워져 상시 화상이
  된다. 둘 다 알고 받아들인 것 — "버그처럼 보인다"고 되돌리지 말 것. 뒤집으려면 README
  계약 2·6-1 을 인용하고 재승인.
...
- ✅ **사용자 Play 확인 통과 (2026-07-30)** — 파이어볼 외형(진행 방향 정렬·트레일·보드 접지) ·
  히트 VFX 가독 · `StatusFxKind.Fire` 오라 점등/소등 · 데미지 폰트 1초 간격 "4"×5 ·
  펄스 리듬 · 어그로 예외 · 레인저 부재 시 통과. 콘솔 에러 0.
- ✅ **씬 리로드 확인** — `stackModifierAuthoring` 4칸(`StackModifier_Fire` 포함), dirty 아님.
- ✅ **파이프라인 맵 대조** — 구조 변경 없음. 신규 아키타입·정거장·앵커 파일·이벤트 채널이
  전부 0이고 Enemy/Projectile 아키타입의 기존 정거장만 채웠으므로
  `docs/reference/object-pipeline-map.md` 는 **갱신 불요**.
- **밸런스 미검증** — 수치는 전부 placeholder. 화상 1회분 20 은 아처 HP 293 의 약 7%.
  킨들러 1기 총 ≈7.5 DPS(직격 4.2 + 화상 평균 3.3) → 아처 1기 처치에 약 39초.
  ⚠ 화상은 **1마리 기준**으로 계산할 것(마릿수에 비례하지 않고 4.0 DPS 가 천장 — 계약 6-1).
- 후속 후보 5건은 `docs/spec/README.md` Follow-up Backlog 로 이관했다.

...
- [x] 시트 importer 를 태워도 SO 값이 유지된다 — `kindler` 행이 없으므로 구조상 unmatched
...
### 사용자 Play 확인 — **2026-07-30 통과** (프레젠테이션, 배치로는 판정 불가)

- [x] **어그로 예외**: 가디언이 킨들러를 때려 어그로하면 조준이 가디언으로 넘어간다
- [x] **레인저 부재**: 레인저를 치우면 아무도 안 쏘고 통과한다(계약 9)
- [x] **틱 수·총량**: 데미지 숫자가 **1초 간격 정수 "4" × 5회**. 초당 수십 개 스팸 없음
- [x] **오라**: 아처에게 `StatusFxKind.Fire` 오라 점등 → 화상 종료와 함께 소등.
      **Stack origin 으로 이 오라가 켜진 첫 사례** — `dot-effect-extraction` unit 1 도 겸해 실증됐다
- [x] **펄스 리듬**: 화상이 4.85초 뒤 꺼지고 다음 발화(≈6초)까지 **1.15초** 공백(계약 2)
- [x] 콘솔 에러 0

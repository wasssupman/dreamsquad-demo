...
> **상태: 완료 2026-08-26** (슬라이스 2a~2g · Play 확인)
...
## 2f 에서 나온 것 (2026-08-26) — **밸런스 변경**

레거시는 탄을 안 쐈다: 폭 1.2타일 통로를 훑어 **즉발 피해**를 줬다. 이제 4기
(머신거너·마크스맨·피어서·스나이퍼)가 **진짜 관통탄**을 쏜다(사용자 결정 「(a)」).

바뀐 것: 탄이 **날아가는 시간**이 생겼고, 사거리 끝에서 소멸하며, 화면에 탄이 보인다.

**바뀌지 않게 지킨 것 둘**:

1. **「앞으로 쏜다」.** 4기 중 셋은 자기 공격탄이 방향 궤적이 아니라(조준이 안 걸린다),
   각자 자기 탄을 복사해 방향 궤적 전용 배럴을 만들었다. 안 그러면 조준이 사라져
   레거시의 「전방」이 통째로 없어진다.
2. **레인 커버리지.** 첫 실행에서 「레인 오프셋만큼 옆으로 선 적」이 빠졌다 — 탄이
   통로보다 얇아서다. 그 폭은 **실측으로 고쳤던 결함**이라(±0.5 로 선 적 두 마리가
   빠지던 것) 되돌리면 안 된다. 탄 피격 반경을 통로 반폭과 **같은 값(0.6)** 으로 맞췄다.

**관통 99** 인 이유도 같다 — 레거시 통로는 그 안 «전원» 을 때렸다.

⚠ **그물의 전제가 또 바뀌었다.** `StartBattle()` 이 필요해졌고(투사체 드레인이
...
- [x] Play 육안 확인 — 사용자 확인 2026-08-26

**완료 2026-08-26** — 슬라이스 7개:
`5007c970`(2a) · `021dc7bf`(2b) · `cdf1e9fd`(2c) · `8c2d7493`(2d) ·
`be2dcb69`(2e) · `0044745b`(2f) · `ad5c50c6`(2g)

콘텐츠 변경 3건 육안 통과: 브루저 충격파(짱쎈 진동 파형 축소 재사용) ·
전방 4기의 관통탄 · 오라 저작 퍼센트화.

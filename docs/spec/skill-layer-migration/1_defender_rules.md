...
⚠ **같은 unit 이 두 payload 에 반대 판단을 내린 이유**(ECS 리뷰 M-1): 발사 명세는
「두 벌로 두느니 arm 을 지운다」였는데 도발은 arm 을 남긴다. 갈림의 근거는 **강제 조건의
유무**다 — 발사 명세는 조준 규칙이 도메인으로 이사하면서 Burst arm 이 그걸 **부를 수 없게**
됐다(선택지가 「지운다」와 「규칙을 복제한다」 둘뿐이었다). 도발엔 그런 강제가 없어
spec 의 기본 계획(arm 은 unit 8 까지 산다 · 매 커밋이 작고 되돌릴 수 있다)을 따른다.

⚠ **다만 두 벌은 이미 동치가 아니다.** 새 skill 은 후보를 64개로 자르고(arm 은 무상한),
`SimEntityId` 미발급 후보를 버린다(arm 은 안 버린다). 유닛 능력 슬롯은 라우팅 분기가
앞이라 arm 에 도달하지 못하고 카드는 이 payload 를 저작하지 않아 **오늘은 잠재적**이지만,
카드가 도발을 처음 저작하는 날 조용히 다른 규칙이 돈다. **unit 8 은 이 divergence 를
「arm 을 지운다」로 닫아야지 「둘을 맞춘다」로 닫으면 안 된다.**

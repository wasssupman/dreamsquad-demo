...
### 결정 (2026-08-26 사용자) — **(a) 이름표는 남는다**

완료 기준을 고친다: **「`SkillEffectType` 삭제」가 아니라 「그 이름표가 더는 실행을 정하지
않는다」**가 이 unit 의 기준이다. 없애려던 진짜 병(스킬 하나 추가할 때마다 분기문이
자라는 것)은 해결됐고, 남은 것(색·문안)은 그 카드가 **무엇인가**이지 **무엇을 하나**가 아니다.

### 7e — 「포탈같이 특수한 액티브는 각 액티브 스킬 **명세와 execute** 에서 처리한다」

같은 결정에서 나온 구조 지시다. 포탈의 특수성이 흩어져 있었다:

| 흩어진 곳 | 지금 |
|---|---|
| `IsPortal => effect == Portal` | **저작 필드** `needsTwoTiles` — 명세가 선언한다 |
| UI 조준 문안·두 번 탭 판정 | 같은 필드를 읽는다 |
| 브리지 커밋 가드(`effect != Portal`) | 같은 필드를 읽는다 |
| 입구≠출구 규칙 | 두 칸 조준에 딸린 규칙으로 남는다(입력 규칙) |
| 링크 생성 | `PortalSkill.Execute` |

⚠ **필드를 안 켜면 조용히 한 칸 스킬이 된다** — 포탈은 입구만 찍히고 카드가 안 먹는 것처럼
보인다. 컴파일러도 런타임도 그 연결을 안 잡아서 `ActiveSkillAimingTests` 가 잡는다.

**남은 것**: 이 unit 의 완료 기준 중 「enum 삭제」는 위 결정으로 대체됐고, 나머지는 충족됐다.

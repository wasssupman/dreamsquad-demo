...
**볼리 2에셋은 「스킬인가」가 먼저다.** 머신거너의 10연발은 그 유닛의 **공격 그 자체**다
(`DirectionalVolleyAbility` 는 「이 유닛이 어떻게 공격하나」를 말하고, 이미 `PatternSlot` 0 번에
구워져 emitter 가 전개한다). 스킬 레이어로 옮기면 유닛의 공격 모델이 스킬 디스패치 뒤로
간다 — 그게 맞는지가 사양 질문이지 이전 작업이 아니다.

...
| **5b** | 실드 셔틀 1에셋 — **전용 시스템 은퇴** | 완료 |
| **5c** | 볼리 2에셋 | **범위 밖 확정** (2026-08-26 사용자 결정) |
| **5d** | 폭탄 1에셋 | **범위 밖** — 볼리와 같은 이유 |
...
## 5b 에서 나온 것 (2026-08-26)

**전용 시스템·상태·규칙 3파일이 사라졌다** — `ShieldCastSystem` · `ShieldCastState` ·
`ShieldTargeting`. 주기 트리거는 이미 있고(`DcTriggerSlot.periodSeconds`) 실드 부여도 이미
있어서(`GrantShield`), 남은 것은 **축을 맞추는 일**뿐이었다.

**저작 에셋은 안 건드렸다.** `ShieldCastAbility` 는 그대로고 **bake 가 규칙 슬롯으로 굽는다**.
저작 표면을 옮기는 것은 이전과 별개의 일이고, 이 unit 의 질문은 「실행이 한 레이어에 있나」다.

**축은 다섯이었다** — 오라에서 배운 대로 세고 시작했다:

| 축 | 카드(악몽의 가호) | 능력(셔틀) |
|---|---|---|
| 반경 | `tileRange` | `attackRange` 재사용(계약 5) |
| 자기 포함 | **제외** | **포함**(계약 6) |
| 대상 수 | 전부 | `targetCount` |
| 우선순위 | 없음 | 가까운 순 / 다친 순 |
| 만충 skip | 있음 | 있음 |

⚠ **자기 포함을 filter 로 접으면 안 된다.** 같은 filter 라도 두 경로가 갈리고, 그 이유가
「이 host 에 자기 실드를 주는 능력이 또 있나」(→ 병합 키를 공유해 상시 실드로 붕괴)라
**저작자가 아니라 bake 만 아는 사실**이다. 그래서 저작 필드가 아니라 bake 가 정한다.

**선택 규칙은 도메인으로 이사했다**(`SkillShieldSelect`) — `SkillAim` 과 같은 이사다.
규칙·상수·부등호 그대로, 그릇만 `NativeArray` → plain 배열. 기존 8단언이 그대로 정본이다.

⚠ **포트에 `EffectiveHpRatio` 를 열었다.** 원시 세 값(체력·최대체력·실드합)이 아니라
**파생값 하나**인 이유: 스킬이 묻는 것이 「누가 제일 다쳤나」이고, 실드 합산 규칙은
어댑터(`ShieldMath`)가 소유한다.

⚠ **enum 에 값을 넣고 어댑터 구현을 빼먹으면 스킬이 통째로 죽는다.** 미배선 스탯은
`NotWired` 를 던지고 디스패처가 그 발동을 통째로 버린다 — 「실드가 아예 안 나간다」로
나타났다. 그리고 그 질의는 **`AttackState` 게이트보다 앞**이어야 한다(안 때리는 아군이
전부 0 으로 동률이 된다).


## 5d — 폭탄도 기본공격이다 (범위 밖)

사용자가 그은 경계선을 그대로 적용한 결과다. 폭탄 투척의 발동 조건을 보면:

- **쿨다운이 차면** 던진다(`attack.cooldownRemaining <= 0`) — 조건이 없다
- 유닛의 **공격 사양**을 그대로 쓴다(`attack.range` · `targetMask` · `targetTraversalLayers`)
- 평타 분기를 **대체**한다(`continue`) — 폭탄맨은 이것 말고 때리는 방법이 없다

`BombThrowAbility` 는 「이 유닛이 **어떻게 공격하나**」를 말한다. 옮기면 유닛의 공격 모델이
스킬 디스패치 뒤로 가고, 없어지는 arm 사본은 0이다.

⚠ `BombLauncherState` 는 그대로 남는다 — 은퇴 대상이 아니라 **그 유닛의 공격 파라미터**다.

## unit 5 종결

**이전 대상은 8이 아니라 5였다.** 셋(볼리 2 · 폭탄 1)은 기본공격이고, 그 셋이 이 spec 이
그은 경계선의 반대편을 정의한다:

> 「발사 명세를 트리거한다」만으로는 스킬이 아니다. 그 위에 **조건**(N회째 · 처치 시 ·
> 경계 돌파 · 주기 · 부착 순간)이 얹힐 때 스킬이 된다.

남은 다섯은 전부 옮겼다 — 하자드 4(캐스트 seam) · 실드 1(주기 슬롯).

### 완료 기준 대조

- ✅ 이전 대상 5가 concrete + 저작 SO 로 존재하고 전용 arm 이 죽었다
  (`ShieldCastSystem`·`ShieldCastState`·`ShieldTargeting` 3파일 삭제)
- ✅ `CastEventsSingleton` 의 같은 프레임 소비 유지 — 캐스트 seam 이
  `[UpdateAfter(HazardCastSystem)] [UpdateBefore(AttackSystem)]` 로 그 구간에 꽂힌다
- ✅ 진행형 상태는 남아 있다
  ⚠ 단 **이 문서의 전제가 하나 더 틀렸다**: `ShieldCastState` 는 진행형 상태가 아니라
  **쿨다운 + 저작값 운반체**였다. 굴리는 일은 `DcTriggerSlot.elapsed` 로 옮겨갔고 그것도
  여전히 컴포넌트+시스템 소유라 계약 5 는 지켜진다. 진짜 진행형 상태(`BombLauncherState`)는
  기본공격이라 애초에 대상이 아니다.
- ✅ 자는 캐스터 동작 무변경 — CC 게이팅을 어느 쪽도 건드리지 않았다
  (`BossPeriodicTriggerSystem` 도 `WithNone<DeadTag>` 뿐이라 셔틀은 예전처럼 자면서도 시전한다)
- ✅ 그물 초록 / ⬜ Play 육안

...
- [ ] 컴파일 에러 0 · **기존 EditMode 전량 그린 + 기대값 갱신 0건** — 경로 없는 맵은 슬롯 1개로 현행과 byte-동치여야 한다. 기대값을 고치고 싶어지면 동치가 깨진 것이다
- [ ] 슬롯 조회 테스트: (골, Path)=0 · (웨이포인트, Path)=해당 슬롯 · 미등록 조합 = PrimarySlot
- [ ] 재빌드 테스트: 장애물 추가 → 골 슬롯과 웨이포인트 슬롯의 `dist` 가 **각자의 목적지 기준**으로 재계산됨(웨이포인트 슬롯 dist[웨이포인트 셀] == 0)
- [ ] 중복 제거 테스트: 두 경로가 같은 셀 공유 → 슬롯 수 증가 없음

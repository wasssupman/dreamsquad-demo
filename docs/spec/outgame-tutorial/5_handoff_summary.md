...
- **사용자 Play 체감 확인** — dim 톤(`UiOverlay.Dim` 알파 0.92)이 로비 배경 위에서 과한지,
  문구 가독성, 12px 스트립이 실제로 두 버튼을 분리해 보이게 하는지. ScreenSpaceOverlay 는
  카메라 스크린샷에 안 잡히므로 육안 확인이 유일한 수단이다.
- Android 가로 실기기: 노치 영역 dim 커버리지 + **링/홀 정렬**.

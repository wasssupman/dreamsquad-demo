...
**완료 확인 2026-08-01** — 사용자 Play 확인 통과. 커밋 `f0d05bd7`. 리셋 → A → 첫 판 → 복귀 B → 패널 열고 닫기 → **C 노출** → World 캐릭터 드래그 → 즉시 종료 → 재진입 미노출. **패널을 여는 순간에는 C 가 뜨지 않음**(`ClosePanels(restoreLobby)` 회귀 — 스쿼드·드림캐쳐·테스트모드·히스토리 4개), dim 탭 무반응, 8초 뒤 건너뛰기까지 확인.

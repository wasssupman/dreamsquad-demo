...
- **텔레포트** = `HealthThreshold(-10%마다)` × `SelfBlink(위협 리더 근처)` — 임계치 트리거 + 위치 페이로드 (신규)
  - **착지 정책은 은퇴했다 (2026-07-29, `boss-jjangssen` unit 4).** `SelfBlink` 의 착지 앵커가 "위협 리더 근처 → 최근접 폴백" 에서 **"방어유닛 밀집도 최대 셀"** 로 **교체**됐다. 이 정책의 라이브 authoring 사용처가 0이었기 때문(나이트메어 에셋의 mechanic 3개는 전부 `PeriodicTimer`)에 셀렉터 필드를 두지 않고 갈아냈다. `ThreatEntry` 버퍼와 threat drain 은 별 책임이라 그대로 살아 있다 — 위협 테이블을 다시 착지에 쓰려면 정책을 되살리는 것이 아니라 새로 선택축을 설계해야 한다.

...
- [x] 로비 스쿼드 페이지에 투머치토커 노출(카탈로그 직독) + 스쿼드 편성 가능.
- [x] Play: 배치 → 적 히트 → 대상 zzz + 이동/공격 정지 확인.
- [x] Play: 혼자 때리는 동안 대상이 계속 잠금 유지(수면 3.5 ≥ 쿨다운 3.0), 파이터가 안 때리면 3.5초 후 자연 기상.
- [x] Play: 잠든 적을 다른 타워가 히트 → 즉시 기상(wake-on-hit) 확인.
- [x] 콘솔 에러 0.

확인 2026-07-21 · 사용자 Play 검증 통과 (EditMode 1135/1135 동반 확인)

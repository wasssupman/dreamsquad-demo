...
8. **회오리 VFX 는 「누가 하는 것인가」가 읽혀야 한다.** 혼동 후보가 둘이다 —
   ...
   초판은 `Tornado_SKELETON.prefab`(=②)을 그대로 재사용했고 잠정이라고 적어 뒀다.
   **2026-08-17 에 분리했다** — `Assets/_Project/VFX/Whirlpot_Whirl_SKELETON.prefab`.
   이제 플레이어 스킬과 프리팹이 다르므로 한쪽 튜닝이 다른 쪽을 흔들지 않는다.
   - **색은 `startColor`(main 모듈)로 갈랐다.** ⚠ 이 계약의 초판 서술(「색이 3개 시스템의
     Color-over-Lifetime 그라디언트에 있다」)은 **불완전했다** — Shuriken 은 최종색을
     `startColor × Color-over-Lifetime` 으로 곱하므로, 그라디언트를 건드리지 않고 `startColor`
     하나로 전체 톤을 옮길 수 있다. 그라디언트 키 재작성보다 훨씬 안전한 레버다.
   - 팔레트: 창백한 시안/민트/청회색(플레이어 톤) → **보라 계열 악몽 톤**
     (`OuterRotatingGale` 0.45/0.22/0.68 · `InnerCounterGale` 0.24/0.10/0.40 ·
     `DustColumn` 0.32/0.26/0.42).
   - 밀도 +40%(방출 42/32/24 → 60/46/34). `maxNumParticles` 1000 은 그대로라 상한 여유 안이다.
   - ★**머티리얼은 공유 그대로다** — 프리팹만 복제했고 `.mat` 은 손대지 않았다.
   - 최종 룩 판정은 육안 몫이다(아래 후속 후보 참조).
...
- **회전 예고(telegraph)** — 즉발이고 공격 애니가 없어 신호가 VFX 하나다. 드래곤 브레스와
  ...
  ⚠ **2026-08-16 실제로 안 읽혔다** — 「팽이가 셀프 데미지를 입는 것 같다」는 제보. 자기 피해는
  EditMode·라이브 양쪽에서 0 으로 확인됐고(반격 불가 상태에서 팽이 320→320 · 방어유닛 135→60),
  원인은 **피드백 비대칭**이다: ① 플로팅 데미지 숫자는 `AttackUnitTag` 게이트라 **적 전용**이므로
  갈리는 방어유닛에는 숫자가 안 뜬다 ② 팽이는 자기가 적이라 방어유닛의 반격에 **숫자가 뜬다**
  ③ 공격 애니가 빈 값이라 「내가 때리는 중」이라는 표시가 몸에 없다 ④ 회오리는 팽이를 감싸는
  4타일 폭이라 팽이가 그 효과의 시각적 중심이다. 결과적으로 **회오리 안에서 움직이는 숫자는
  전부 팽이 위에 뜨고 전부 피해**라, 회오리가 자기를 갈고 있는 그림이 된다. 고치는 lever 는
  「피격자 쪽 타격 표시(가장 싸다)」 또는 「예고(위)」이고, 숫자를 방어유닛까지 확대하는 것은
  이 spec 이 아니라 전역 프레젠테이션 결정이다.
...
- **회오리 룩 육안 검수** [S] · 2026-08-17 변경분(사거리 1 · 화면 스케일 1.5 · 악몽 팔레트 ·
  밀도 +40%)은 **기계로 검증할 수 없는 부분이 남는다.** 볼 것: 보라 톤이 번아웃 먹구름(방어유닛
  *상태*)과 헷갈리지 않는가 · 3×3 판정 대비 회오리가 과하게 크지 않은가 · 팽이 몸이 묻히지
  않는가. 크기는 `attackVfxScalePerTile` 한 값으로 즉시 조정된다.
- **회오리 프리팹 경량화** [S] · `Whirlpot_Whirl_SKELETON` 은 파티클 시스템 3개(각
  `maxNumParticles 1000`)이고 방출 총 140/초다. 회오리는 **상시 2개가 겹치므로**
  (`VfxSpawner.unitAttackAoeSustainMul` = 2) 실효 280/초다. 모바일 예산(일반 50 / 임팩트 100)과
  어긋나므로 프레임 저하가 보이면 **밀도부터 되돌린다**(60/46/34 → 42/32/24 가 원래 값).
  전용 프리팹이 생겼으니 이제 플레이어 스킬에 영향 없이 깎을 수 있다.

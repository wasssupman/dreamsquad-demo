...
**완료 기준 확인**: 에셋·카탈로그 등록 `68d2f35c`(2026-08-03) · 라이브 e2e `7f936b46`(2026-08-03) · 밸런스 튜닝 `69cb1dda`(2026-08-09) · 시트 밸런스 패스 `27cb9794`(2026-08-12).

PlayMode `PatrolDefenderPlayTest` 2건이 실 `BattleScene` + 라이브 에셋으로 배치 SO → 소환 → 지원 편입(힐/실드) → 사망 → 재소환 → 소환사 사망 시 연쇄 소멸·뷰 회수를 태운다 (`Summoner_SpawnsOnePatrol_ThatReceivesSupport_AndRespawns` · `SummonerDeath_RemovesItsPatrolAndView`). 계약 11 의 「효과 타일은 위치 기반이라 걸릴 수 있다」는 unit 0 의 진영 게이트(`d09cd75d`)가 적 대상 존에 한해 닫았다.

**미판정 — spec 종료 시점(2026-08-12)에 기록 없음**:

- **검증 질문** 자체에 대한 사용자 판정. 이 spec 의 고유 질문이라 다른 unit 의 Play 확인으로 대체되지 않는다.
- **영구 봉쇄가 성립하지 않는다** · **보스가 순찰병을 사냥 대상으로 삼고 눌러앉지 않는다** — 관찰 기록 없음. 둘 다 `docs/spec/README.md` Follow-up Backlog 로 이관.

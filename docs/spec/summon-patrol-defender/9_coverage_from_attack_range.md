...
- [x] compile 에러 0 · **EditMode 32/32 · PlayMode 2/2**
...
- [ ] Play(육안): 소환물이 소환사 **옆** 칸에 스폰되는지
- [ ] Play(육안): 소환사를 재배치하면 구역과 집이 따라오는지
- [ ] 데이터 결정: 순찰병 사거리(시트 4) vs 담당 구역(시트 4) — 마중 나가게 하려면 사거리를 낮춘다

...
- [ ] Play(육안): 소환사를 **재배치**하면 구역과 집이 따라오는지 — 이 축은 아직 확인 안 됨
- [ ] 데이터 결정: 순찰병 사거리(시트 4) vs 담당 구역(시트 4). 지금 조합은 «소환사 옆에서 원거리 커버»다. 마중 나가는 그림을 원하면 시트의 `patrol_soldier.attackRange` 를 1~2로 낮춘다

---

**완료 기준 확인**: 2026-08-10 · 커밋 `515e5f00` · 사용자 Play 확인 완료(스폰 위치·이동). 재배치 축과 사거리 저작 결정은 미결로 남김.

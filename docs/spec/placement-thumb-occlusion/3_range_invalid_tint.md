...
(2026-07-31 유효 색이 라임이 되며 색상 거리는 커졌지만, 초록↔적색은 적녹 색약에 최악 쌍이라
**플래시가 색약 안전 성분이라는 사실은 더 중요해졌다** — 끄지 말 것.)
...
- `:44` 의 "**노란** 범위" 는 stale 이다(당시 라이브는 주황 `1,0.55,0.12` · 현재는 라임 `0.55,1,0.25`).
  같은 커밋에서 교정(critic L7).

## 완료 기준

- 컴파일 에러 0.
- 에디터 Play 실드래그: 배치 가능 → 유효색(당시 주황, 2026-07-31 이후 라임), 불가로 이동 →
  **적색 전환 + 전이 1회 밝게 번쩍**, 복귀 시 유효색.

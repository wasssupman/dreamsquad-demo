...
### 민감도 실측 (2026-07-29 · `31a79042`)

에디터 1080 세로, 스윕 해상도 1%:

| ratio | 오프셋 | 단언 B | 비고 |
|---|---|---|---|
| 0.06 | 65px | 통과 | 최초 확정값 |
| 0.10 | 108px | (미실행) | **2026-07-31 라이브** — `e45ee478`. 테스트 재실행 없이 사용자 Play 로 하단 도달 확인 |
| 0.18 | 194px | 통과 | |
| 0.20 | 216px | **실패** | row 0 조준 최고점 119px = 화면 11% < 안전선 12% |

첫 실패 ≈ **0.19**. 0.06 은 약 3배, 라이브 0.10 은 약 1.9배 여유. 가드가 실제로 무는 것이 증명됐다
(어떤 값에서도 통과하면 가드가 아니라는 조건 충족).

**단서**: 안전선 `BottomSafeRatio = 0.12` 는 트레이 높이의 근사다. 실기기에서 트레이가 화면의 12%
보다 두꺼우면 이 가드는 낙관적이고, 실제 상한은 더 낮다 — 실기기 항목 ②가 최종 판정이다.

...
> 상태: 구현 완료 2026-07-13 (units 0~5 + 코드리뷰 반영). 트위닝 시각 최종검증은 사용자 포커스 Play 대기. handoff: `6_handoff_summary.md`
...
- **연출 상세조정**(별도 후속 스펙, 사용자 결정 2026-07-13) — 타이밍/이징/촤라락 셔플 비주얼·임팩트, 카드 아트/프레임, 각성 버튼 fly 정밀 정렬(현재 `FlyTarget` 고정 근사), 인트로 텍스트 연출. `GiftConfig` 로 1차 튜닝 가능하나 시각 완성도는 별도 패스.

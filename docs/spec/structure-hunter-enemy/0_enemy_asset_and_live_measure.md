...
**덱 `attackUnitPool` 편입은 이 unit 에서 빠졌다** — 웨이브 재추첨 회귀 때문에 unit 1(밸런스)로 이관했다. 근거는 README §덱 편입이 왜 unit 1 인가. 카탈로그 등재는 웨이브 생성에 영향이 없으므로 여기 남는다.
...
| `killScore` | **3**(엘리트) | 저작 티어가 «일반 1 / 엘리트 3 / 보스 10» 이다. 400HP 에 유일한 대응이 처치인 적이 Tanker(3)보다 낮으면, 이 적을 우선 처리한 플레이어가 잡몹을 파밍한 플레이어보다 손해를 본다 — 검증 질문이 원하는 인센티브의 반대 |
| `idleAnimation` / `walkAnimation` | **`Idle` / `Walk` 분리** | 이 적은 골 앞에서 **분 단위로 정지**한다(계측 5,605프레임 Engaging). `Enemy_Basic` 처럼 `idle=Walk` 단일 루프를 복사하면 제자리 걷기가 된다. 같은 스켈레톤을 쓰는 `Boss_Jjangssen` 이 이미 이 분리를 쓴다 |

**시각은 최소 구분만 한다** — Spine 은 기존 스켈레톤 재사용, 슬롯 컬러(보라)+스케일만 다르게. **정식 구분은 unit 1** 이다(동작이 확인된 뒤에 아트를 붙인다).

⚠ **현재 `partSkins` 가 `Enemy_Basic` 과 완전히 동일하다** — 실루엣이 같아서 잡몹 무리 속에서는 사실상 **리컬러**다. 다른 적 7종은 전부 다른 조합을 쓴다. unit 1 의 첫 항목.

**YAML 저작 주의**: 한글 `displayName` 은 Unity 가 `\uXXXX` 로 직렬화한다. 손으로 raw UTF-8 을 쓰면 다음 Unity 저장 때 그 줄이 재작성돼 **남의 커밋에 무관한 diff 로 섞인다**.
...
**미확인 (사용자 Play 확인 대기)**: 실제 플레이에서 「막을 수 없는 적」의 체감. **단 지금은 어떤 라이브 덱에도 들어 있지 않아 정상 플레이에 등장하지 않는다** — 체감 확인은 unit 1(덱 편입 + 밸런스) 이후에 가능하다.

## 리뷰 반영 (2026-08-11, 커밋 04a63dd8 이후)

코드 리뷰가 잡은 것 중 이 unit 에 반영한 것:

- **덱 편입 되돌림** — 웨이브 재추첨·초반 편향·`Deck_Endless` 누락·동시 등장 상한 부재. unit 1 로 이관(README 참조)
- **`killScore` 2 → 3** — 티어 스케일 밖이었고 난이도 순서를 뒤집었다
- **`idleAnimation` 분리** — 정지가 잦은 적인데 제자리 걷기였다
- **`displayName` 이스케이프** — raw UTF-8 은 다음 Unity 저장에서 재작성된다
- **`stabilityDamage` 를 knob 목록에서 제거**(README) — `canSiege` 경로가 그 값을 읽기 전에 빠져나가 이 적에겐 죽은 값이다
- **「골 락 금지 가드」 주장 철회**(README 결정 3) — 그 가드는 `battle-structures` unit 0 이 제거했고 `AttackSystem` 에 거점 인지 코드가 0건이다. 계측의 «고착 0» 은 관측이지 보장이 아니다
- **「①몸으로 막기 무력화」 문구 정정**(README) — 무력화되는 것은 **유인 하나**뿐이고 방벽·CC 는 통한다

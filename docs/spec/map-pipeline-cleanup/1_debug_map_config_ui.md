...
- [ ] DraftView·SquadPrepView 에서 `mapSettings` 필드+호출 제거, `MapSettingsPanelView.cs` 삭제
- [ ] DraftController 맵-config 표면(:45/:132/:174 포함) 제거, drafting 로직 무변화
- [ ] BattleScene(+SquadPrepView 씬) 패널 와이어링 제거, missing-script/reference 경고 0
- [ ] compile 0 error, EditMode green(삭제한 draft-map 테스트 제외)

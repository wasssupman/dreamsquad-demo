...
- **진짜 버텍스 커브(②-A) + 꼬깃꼬깃 펴짐(③)** → `docs/spec/card-crumple-unfold/` (계획 확정 2026-07-13, plan
  critic 반영). 둘 다 서브디바이드 메시 토대 공유 — ②-A 는 같은 메시 저주파 케이스로 흡수. 이 spec 의 안착 flex 는
  4버텍스 squash-stretch(②-B)로 마감. 구현 대기(실기 perf spike 게이트).

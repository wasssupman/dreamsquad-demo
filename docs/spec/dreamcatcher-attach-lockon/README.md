...
6. **유닛 몸체 틴트 = 상태색(락온 defender만·presenter 소유).** 락온 신호 1차 = 리티클(위치) + 콜아웃(정체), 몸체 틴트는 2차 보강. **처음엔 제거**했다 — 기존 `SetDefenderHoverHighlight` 의 무조건 빨강 repaint 는 밀집에서 안 통함 + dim 없이 곱셈 틴트라 유닛이 더 어두워짐. **rev(포커스 유닛 틴트 · F 블록): dim 오버레이가 생긴 지금은 락온 defender 에 상태색 곱셈 틴트를 다시 얹는다** — valid=시안 / invalid=붉음(화살표·리티클과 같은 언어). Spine R/G/B 는 곱셈이라 "밝힘" 불가지만 dim 된 전장 위 **색조(hue) 대비**로 pop. `DreamcatcherFocusPresenter.UpdateFocusTint` 가 소유, 유닛 전환 시에만 on/off(락온 유닛의 valid 는 드래그 중 불변 — `_attachable` 스냅샷). AttachAim/DefenderCast 만 — **EnemyMark(적)는 health-tint 합성 문제로 비적용**. 틴트 노브 = `DreamcatcherFocusConfig` F 블록(`enableFocusTint` 로 끔). flash×hover stray-tint 레이스는 `SpineUnitView.FlashRoutine` 이 hover 활성 중 `_savedTint`(진짜 base)로 복귀해 차단.

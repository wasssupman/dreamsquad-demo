...
> **은퇴 (2026-08-23, unit 7).** 이 장치는 배럴의 **시한**을 예고하려고 존재했고, 그 시한이
> 사라지면서 할 말을 잃었다. 「언제 터지나」의 답은 이제 남은 체력이고 그것은 머리 위
> 바가 말한다(unit 8). 아래는 **당시 기록**이며 현재 코드에는 없다 — 되살릴 거라면
> `MaterialPropertyBlock` 함정(벤더 메시가 머티리얼 하나를 500여 개와 공유)과 지수 역수
> 함정이 그대로 유효하므로 먼저 읽을 것.

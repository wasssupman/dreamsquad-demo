...
- ⚠ **「배럴에 깔린 적」 위험의 유일한 완화책을 없앴다** — 그리고 초판은 이걸 안 적었다.
  스폰 검증은 적이 서 있는 칸을 거르지 않는다(`EffectSpawner.ValidateCellsForBlockingHazard`).
  Runner·Swift 는 `outputs` 가 비어 배럴을 **못 부수고**, Runner 는 속도가 빨라 최근접이
  되기 쉽다. `4_assets_and_ability.md` 는 이 위험을 「1칸이고 **수명이 유한**해 영구 교착은
  아니다」로 수용했는데, 그 수명을 이 unit 이 지웠다. **unit 7~9 사이 커밋 구간에서는
  실제로 영구 교착이었다.** unit 9 의 노후화가 시간 상한을 되돌려 복구했다 — 노후화를
  끄려면 이 위험이 함께 되살아난다는 것을 먼저 처리해라.
- **배럴이 영구물이 됐다**(unit 9 이전 한정). 아무도 안 때리면 판 끝까지 남아 그 칸을 막는다. 배럴은

...
- ⚠ **상태 해시의 제외 축 2건**(사용자 결정 2026-08-05, 근거는 [`18_context_port.md`](18_context_port.md)):
  `LocalTransform.Scale`(← `HitFlash` discard, **필드 단위** 제외 — 위치·회전은 exact 유지) ·
  `ThreatEntry` 버퍼(← discard, 호출처 실측 0). 둘 다 뷰 소유거나 아무도 안 읽는 값인데 해시가
  컴포넌트를 통째로 직렬화해서 실린 것이다. **제외는 상수 목록으로 선언하고 매 실행 로그로 찍는다**
  — 조용한 제외는 parity 초록을 거짓으로 만든다. 목록 밖 불일치는 기존대로 실패다(제외가 "해시
  비교를 느슨하게" 로 번지지 않게). **목록에 항목을 추가하는 것은 이 unit 의 완료 기준을 낮추는
  것**이므로 근거를 `18_context_port.md` 에 함께 적는다.

...
### F7. 청크 순회 → 생성 순서 전환의 동률 위험 (MEDIUM · 미해결)

구 ECS 쿼리는 **아키타입-청크 순서**, `SimWorld.With<T>()` 는 **생성 순서**. 둘 다 결정적이지만
**서로 다를 수 있다.**

`simId` tie-break 가 대부분을 잡지만, 그 함수들에 **후보 스냅샷을 먹이는 순서** 자체가 다르다:
`BounceRetarget.FindNext`(등거리 시 첫 후보) · `PatternTargeting.Select`(index 기반 선택).

**처분**: 등거리 2체 후보를 둔 EditMode 테스트로 tie-break 계약을 박제한다. 골든이 이 분기를
밟으면 18-K 에서 드러나지만, **밟지 않으면 잠복**한다 — 테스트가 더 싸다.

### F8. 파괴 예정 엔티티에 write-back (MEDIUM) ✅ 수정함

`ProjectileMoveSystem` 이 switch 뒤에서 **모든** 투사체에 `world.Set` 을 두 번 했다.
파괴 예정도 포함. 구 sim 은 `RefRW` 로 arm 안에서만 선택 기록.
결과는 같지만 투사체 수 × 2 의 순수 낭비 — `destroyed` 플래그로 건너뛴다.

### F9. 자기 게이트 누락 — "증발" 이 아니라 "이사" 여야 했다 (MEDIUM) ✅ HazardCast 수정함

분류 D 게이트의 처분 원칙은 *"루프 밖 부수효과가 없으면 증발"* 인데, **루프 밖에 스냅샷 구축이
있으면 그건 부수효과다.**

| 시스템 | 구 게이트 | 캐스터/대상 0 일 때 |
|---|---|---|
| `HazardCastSystem` | `RequireForUpdate<HazardCastState>` | 매 틱 전 유닛 후보 리스트 구축 ✅ 수정 |
| `PatrolFieldSystem` | `RequireForUpdate<PatrolAnchor>` | 매 틱 grid BFS + 적 스냅샷 — **18-K 성능 항목으로 이관** |

배치 전 구간이 통째로 이 상태였다. 정확성 무관, 모바일 예산 문제.
...
- **ECS 경계**(CRITICAL 0 · HIGH 0): 맥락 경계 위반 **0**. `world.Set`/`RemoveComponent`/`AddBuffer`
  전수를 폴더별로 대조한 결과 소유권이 지켜진다. 등록된 32 시스템의 캡처 번호·phase 가 청사진과
  정확히 일치. 같은-틱/1틱-지연이 뒤바뀐 채널 쌍 **0**. 게이트 53 처분 중 잘못 버린 것 **없음**.
  추가 확인: 구 sim 의 ECB 사용 28 시스템이 전부 `Allocator.Temp` + 같은 `OnUpdate` Playback
  이었고 신 sim 이 그 배치를 보존(교차 시스템·교차 틱 ECB 신설 0) · tie-break ⑥ 역-삽입순 보존 ·
  채널 27 중 **22 존재**(누락 5는 전부 미이식 시스템 소유).

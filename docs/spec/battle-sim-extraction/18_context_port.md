...
> **실행 계획은 [`m1_unit18_plan.md`](m1_unit18_plan.md) 가 소유한다** (rev 2, critic REWORK 반영).
> 조각 배정·세션 분할·증인 명세·중단 기준은 그쪽을 읽는다. 이 문서는 계약과 정찰 결과다.
>
> rev 2 가 이 문서에서 정정한 것 2건:
> - **`m1_salvage_matrix.md` 의 `UnitLifecycle` = rewrite 판정은 무효다.** 근거였던 *"신 sim 은
>   객체 제거가 즉시"* 가 청사진 ③ 의 **1틱 "죽었지만 아직 있는" 창(핀 3개)** 과 충돌한다.
>   청사진이 이긴다 — 이 unit 은 새 설계를 하지 않는다.
> - **아래 §"규모 실측" 의 "42%(5,382줄)의 상당분은 unit 17 이 가져간다" 는 사실이 아니다.**
>   unit 17 은 **코드 변경 0** 으로 끝났고 지목된 conform 유틸은 전부 아직 `Scripts/Battle/`
>   아래다. 실질 이식 대상은 7,304줄이 아니라 **12,839줄**이다.

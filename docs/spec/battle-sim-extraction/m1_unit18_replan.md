...
| 18-K/2 | 타입별 렌더러 21종 + 엔티티 블록 순회(`SimEntityId` 오름차순) | ← **다음** |
| 18-K/3 | **P0/P13 흡수** — 커맨드 반입 · 읽기 모델 스탬프 · 도약 드레인 | |
| 18-K/4 | **분류 C 게이트 13건** — 월드 싱글턴 준비. 기동 순서 문제라 **조립 지점이 소유** | |
| 18-K/5 | **그림자 A/B 무장** + 게이트 53 계정 감사 | ⚠ 여기서 라이브 코드를 만진다 |
| 18-K/6 | **F5** `SimTransform` 팩토리 강제 · **D3** 동률 예외 목록 재유도 · 성능 재측정 | |

**18-K/2 를 시작할 때 알아야 할 것**: 21타입 중 **필드 집합이 갈리는 것은 `SimTransform` 하나**다
(나머지 20 은 이름까지 동일 — `SimLegacyTraceContractTests` 가 구 타입에서 유도해 대조한다).
그래서 렌더러는 **필드 이름을 ordinal 정렬한 순서**로 기계적으로 쓰면 되고, 확인은 그 테스트가 한다.

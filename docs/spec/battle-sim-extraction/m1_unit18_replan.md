...
| ~~18-K/5c/3..~~ | **하위번호 폐지** — 남은 작업은 평평한 유닛으로 재발급 (아래 표) | → **18-M** |

### 작업 단위 재발급 (2026-08-06 — 구조 판정)

**하위번호가 예측을 깨는 메커니즘이었다**: 작업량 증가가 상위 라벨(`18-K`) 안에 숨어 유닛
개수는 그대로인데 내용만 불었다. 반면 **커밋 단위는 지금대로 둔다** — 작은 커밋이 이번
세션에만 실수 3건을 EditMode 에서 잡았고 전 커밋이 초록이었다. 고치는 것은 장부다.

1. **새 작업 = 새 문자.** 하위번호 금지 — 증가는 표에 행이 늘어나는 것으로만 드러난다.
2. **유닛 = 한 세션.** 한 세션에 안 끝나면 유닛이 아니라 트랙이고, 착수 전에 쪼갠다.
3. **번호는 셀 수 있는 사이징과 함께만 발급.** 못 세면 첫 유닛이 "세는 작업"이다 —
   18-K 는 사이징 없이(토픽 5줄) 유닛 행세를 했고, 그것이 이번 예측 붕괴의 뿌리다.
...
| **18-N** ✅ `d74b300c` | 스폰 미러링 — **bake 가 아니라 presence-driven copier**(아래 판정). `mirrored == ordinals == liveCounter` 전 시나리오 일치 | 컴포넌트 20종 | EditMode · 골든 불변 |
| ~~18-O~~ ~~18-P~~ | **18-N 에 흡수됨** — copier 가 라이브 아키타입을 훑으므로 방어유닛·해저드·장애물·투사체가 자동 포함된다(`dreamcatcher_heavy` 67 엔티티 미러, 미매핑 0). 경로별 손 코드가 필요 없었다 | — | — |
| **18-Q** ✅ `776a4bf4` | A/B 상태 대조기 — **2/14 세션 EXACT**. 갈림의 원인이 남은 유닛을 정확히 가리킨다(아래) | 축 1 | 골든 + 비교 로그 |
| **18-R** | **D3** 제외 목록 기록기 재유도(제외 축 2건 처분 — 사용자 확인 1줄 필요) · 성능 장기측정. **F5 는 18-N~P 의 완료 기준으로 흡수** | 판정 2 | EditMode · 측정 |
| **18-S** ✅ `12d06851` | Bridge 산출 이벤트 미러링 — 짝을 잃지 않는 발행 지점 5종. EXACT 2→6 세션 | enqueue 9 지점 | 골든 + A/B |
| **18-T** ✅ `9cdc25b3` | 기믹 config 싱글턴 3종 미러링 — **EXACT 6→12 세션 = 6/7 시나리오** | 싱글턴 3 | 골든 + A/B |
| **18-U** | 잔여 divergence 1건 추적 — `normal` 의 단일 `Health` 라인(아래) | 값 1 | A/B EXACT 7/7 |
| 18-L | 합류 4건 (기존) — ⚠ 17-F 타입 졸업은 sim-owned 스폰 타이밍의 선행 조건. ⚠ `Vector2Int` **260곳/28파일** 실측(계획서의 "9 사용처" 는 과소계상) | 항목 4 · 260 참조 | 골든 + A/B |

#### 18-Q 의 A/B 장부 — 갈림은 이식 오류가 아니다 (2026-08-06 실측)

**`restart` 의 재시작-후 매치가 문자 단위 EXACT**(2 런 모두). 상태 원문 전체가 구 sim 과
완전히 일치했다 — 44 시스템 이식이 실제 판에서 동치일 수 있다는 첫 실증이다.

| 시나리오 | live/shadow 줄 | 성격 | 원인 → 소유 유닛 |
|---|---|---|---|
| normal · boss_wave · multi_goal · dreamcatcher_heavy · forced_wave | 105/99 · 256/241 · 90/86 · 87/85 · 176/166 | shadow 가 **적다** | sim 밖 규칙 → **18-L** |
| simultaneous_death | 116/**246** | shadow 가 **많다** | 전달되지 않은 커맨드 → **19** |
| restart(2번째) | — | **EXACT** ✅ | — |

**shadow 가 적은 쪽의 추적**: 첫 불일치가 `StatModifierSlot[0]=` **부재**(live 는 빈 버퍼로
존재)이고 `Health.value` 가 live 101.5 vs shadow 75 다. 양쪽 `ModifierApplySystem` 이 **첫 적용
시에만** 버퍼를 만들므로 부재 = *"그림자에서 stat 모디파이어가 한 번도 적용되지 않았다"* ⇒
생산자가 sim 밖이다: **배치 스킬(`ApplyOnPlaceEffect` 8분기)·카드 bake·규칙 실행 위임 4채널**
(`HazardSpawnRequest`·`MeteorBarrageRequest`·`ShieldBreak`·`DefenderDeath` — 세션 계약 M9 가
*"사실이 아니라 Bridge 에게 내리는 명령"* 이라 적은 그것).

**shadow 가 많은 쪽의 추적**: `simultaneous_death` 는 `EnqueueHarnessSimultaneousDeaths` 가
라이브 버퍼에 `float.MaxValue` 를 **직접 꽂는다** — 커맨드이고 그림자에 전달되지 않으므로
라이브만 유닛이 죽는다.

⇒ **비교기의 diff 는 이식이 틀린 증거가 아니라, 아직 옮기지 않은 규칙과 아직 전달하지 않은
입력의 목록이다.** 18-L·19 를 끝내면 이 표가 EXACT 로 수렴해야 하고, **수렴하지 않는 잔여가
곧 진짜 이식 버그**다 — 그 분리가 이 유닛이 만든 값이다.

⚠ 그러므로 **18-L 의 완료 기준에 A/B 를 넣는다**: 골든 byte diff 0 **+ 위 5 시나리오의 shadow
줄 수가 live 에 수렴. 규칙을 옮겼는데 수렴하지 않으면 그 자리가 이식 버그다.

#### 수렴 이력 — 18-S·18-T 가 표를 닫았다 (2026-08-06)

| | 18-Q | 18-S | 18-T |
|---|---|---|---|
| EXACT 세션 | 2/14 | 6/14 | **12/14** |
| EXACT 시나리오 | 1/7 | 2/7 | **6/7** |

`boss_wave`·`multi_goal`·`dreamcatcher_heavy`·`forced_wave`·`simultaneous_death`·`restart`
= **상태 원문이 문자 단위로 같다.** 44 시스템 이식이 구 sim 과 행동 동치라는 것이 6 시나리오에서
실증됐다. **위 장부의 예측이 맞았다** — 갈림은 이식 오류가 아니라 안 옮긴 규칙·안 준 입력이었다.

**어떻게 좁혔나(추측이 아니라 분류)**: Bridge 의 sim 컴포넌트 직접 쓰기 58 곳을 스폰
내부/이후로 갈랐다. 52 곳은 스폰·셋업 함수 안이라 copier(18-N)가 이미 덮고, 스폰 이후는 **6 곳**
— 그 중 4 곳이 `CreateGimmickConfigIfActive` 였다. 58 을 다 보는 대신 6 을 봤다.

⚠ **분류 B 게이트가 두 벌이라는 것을 놓쳤었다**: `ClockOut` 은 `SimConfig` 로 이사했지만
레드불·온천·야근은 **컴포넌트 싱글턴**으로 남아 있다. 18-K/5c/1 이 config 주입을 하면서
`SimConfig` 쪽만 챙겨 그림자가 세 기믹 없이 돌았다 — 부재 = 비활성이라 **조용히** 꺼졌다.

### 18-U — 잔여 divergence 1건 (다음)

`normal` 만 갈리고 성격이 완전히 다르다:

- **줄 수 동일**(105/105) — 구조적 결손 0(부재 버퍼·유령 엔티티 없음)
- **단일 라인**: 한 엔티티의 `Health` — live `260.199982` vs shadow `301.2`(max 338)
- 그림자가 **덜** 아프다(+41.000018)

⇒ 1차 용의자는 **지속 피해 계열**: live 값의 float 잡음(`260.199982`)이 작은 증분 누적
(DoT/리젠 틱)을 시사한다. `DotEffect` 의 틱 수·요율·`tickTimer` carry-over(청사진 ③ §6 이
*"병합 duration 정책은 경로마다 다르다 — 통일하지 말 것"* 이라 경고한 그 비대칭)를 먼저 본다.

2차 용의자는 `ApplyDreamcatcherCardToUnit` 의 큐 없는 직접 쓰기 2 곳(`LethalTimer`·
`DreamCocoon`·`FrontmostAttackLock`) — 다만 `dreamcatcher_heavy` 가 **이미 EXACT** 이므로
그 코퍼스는 해당 분기를 밟지 않는다(우선순위 낮음).

⚠ **착수 전에 비교기를 한 단계 정밀화할 것**: 지금 리포트는 줄 번호만 준다. 어느 `entity+N`
블록인지와 그 `tag=` 를 함께 찍으면 아키타입이 즉시 특정된다 — 그게 없으면 105 줄에서
사람이 세어야 한다(값 1 개를 쫓는 데 골든 5 분이 든다).
...
(실측: `IEnableableComponent` 구현체는 Battle 전체에서 **이것 하나**다.)

#### 결정 정정 — bake 가 아니라 **copier** 다 (2026-08-06, 구현 착수 시)

착수 노트는 이 유닛을 *"유닛 데이터→컴포넌트 세트 bake"* 로 잡았다. **철회한다.**

**이유는 진단 순수성이다.** bake 는 스폰 로직 ~150줄을 sim 쪽에 다시 쓰는 일이고, 그러면
A/B 가 갈렸을 때 **"이식한 규칙이 틀렸나 / 내 bake 가 틀렸나"** 를 구분할 수 없다 — 그건
A/B 비교의 존재 이유를 무너뜨린다. copier(라이브 엔티티의 값을 그대로 복사)는 그림자가
**정확히 같은 초기 상태**에서 출발하므로, 이후 모든 불일치가 **이식된 규칙**의 것이다.

부수 이득: 경로별 차이가 "어떤 컴포넌트가 붙어 있나" 뿐이라 **presence-driven copier 하나가
18-N/O/P 를 전부 덮는다**(7 경로 × 손 변환 코드가 사라진다).

**bake 는 죽지 않고 미뤄진다** — config-드라이브 최종 형태(청사진 ③ · unit 20)에서 필요하고,
그때는 **규칙이 이미 parity 로 증명된 뒤**라 bake 버그와 규칙 버그가 섞이지 않는다.
⇒ bake 는 unit 19~20 또는 M2 의 몫으로 이관한다.

⚠ 이 판정은 리플렉션 금지 원칙과 충돌하지 않는다. 그 금지의 근거는 *"신 타입에 리플렉션을
걸면 **타입 이름**이 신 것으로 나온다"*(트레이스 키)였고, copier 는 **값**을 필드 이름으로
옮긴다. 이름 일치는 18-M 이 155쌍에서 증명했다 — 그 오라클이 이 copier 의 전제다.
...
**정정 (2026-08-06 같은 날, 재실측)**: *"118 타입 변환면"* 은 **과대계상**이었다. 118 은 전수
census 이고, 그중 캐리어·이벤트·내부 상태는 T1 이 이미 옮겨 **스폰과 무관**하다. 스폰이 실제로
붙이는 것은 **아키타입당 컴포넌트 ~14종**(defender 실측: Health·AttackState·FactionTag·
DefenderTile·DefenderCcData·AggroCapacity·ShieldCastState·HazardCastState·DefenderClassTag·
태그류 + 버퍼 3종). 대조 오라클(18-M)은 리플렉션 자동 발견이라 118 이어도 비용이 같다 —
비싼 것은 변환이고, 그 변환면이 5 배가 아니라 T1 한 조각 수준으로 줄었다.

**끝 그림도 정정**: 청사진 ③(*"신 sim 의 Sim.Tick 은 P0 의 웨이브 스케줄·스폰 게이트를
흡수한다"*)과 unit 20(*"같은 MatchConfig·seed·입력 스케줄로 각각 구동"*) 기준, 최종 형태는
**config-드라이브**이지 write-mirroring 이 아니다. 다만 bake(데이터→컴포넌트 세트)는 어느
쪽이든 동일한 코어라 **live-타이밍 미러로 먼저 세우고**, 18-L(17-F 타입 졸업 —
`MatchWaveSchedule` 이 `Wassup.Data` 의 엔진 타입에 걸려 있다) 뒤 sim-owned 타이밍으로
전환한다. bake 는 재사용되고 타이밍 소유만 이동한다.
...
⇒ **18-R 의 D3 에서 제외 목록을 기록기로부터 재유도**할 때 이 두 항목을 함께 처분하고,
`20_ab_parity_swap.md` 의 해당 문단을 고친다. ⚠ 이 처분은 **기록된 사용자 결정(2026-08-05)의
전제가 죽은 것**이므로, 실행 전에 사용자 확인 1줄을 받는다 — 제외를 좁히는 방향(더 엄격한
parity)이라 논쟁적이지는 않지만, 결정의 소유자는 사용자다. (P5 의 다른 "제외 축" 주장도 같은 의심 대상 —

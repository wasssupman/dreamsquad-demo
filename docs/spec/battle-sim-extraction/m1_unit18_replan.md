...
**지금 할 일: #33 `AttackSystem` 의 arm D.** 아래 [남은 arm 지도](#남은-arm-지도-구-attacksystemcs-라인-기준) 표에서
`713–798` 구간(**START** — 애니 + 쿨다운 리셋 + 지연 세팅, 86줄)이다. 어휘 토대(`29953628`)와
arm B(`5f0216d4`)가 끝났으므로 **arm 은 전부 이미 있는 타입 위에 선다 — 새 타입을 만들 일이
거의 없다.** 시스템 껍데기(`Sim/Lib/Combat/AttackSystem.cs`)와 후보 스냅샷도 이미 서 있으니
`Run` 안에 arm 을 하나씩 더한다.
...
44 시스템 중 **33 이식**(#33 은 arm 단위로 채우는 중) · EditMode **2604 passed / 0 failed /
1 skipped / 1 inconclusive**. 그 inconclusive 는 **의도된 것**이다 — 이 런타임에서 할당 카운터가
항상 0 이라 프로브가 거짓 통과 대신 판정을 비운다(N2 참조). 초록으로 만들려고 손대지 말 것.

✅ **F6 의 절반은 닫혔다** — `AttackCluster`(`Sim/Lib/Combat/AttackSystem.cs` 하단)가 #18·#33 을
등록한다. 나머지 절반({1..44} 전수 등록 단정)은 18-K 몫이다.
...
| 순서 | 단위 | 규모 | 새로 들어간 것 |
|---|---|---|---|
| 1 | **18-I/2** #33 `AttackSystem` — arm B·G 완료 | 1,729줄 | **F6** — #18 등록을 완료 기준에 (`AttackCluster` 로 닫힘) |
...
| # | 구간 | 줄 | 내용 |
|---|---|---|---|
| A | 25–171 | 147 | `OnCreate` 쿼리 + `OnUpdate` 선두 lookup·스냅샷 구축 — **대부분 증발**(아래) |
| ~~B~~ | ~~172–228~~ | ~~57~~ | **완료** (`5f0216d4`) — 캐스트 사건 드레인 |
...
| ~~G~~ | ~~1618–1664~~ | ~~47~~ | **완료** — `SpawnNeedleCarrier` 는 arm B 가 호출처라 함께 옮겼다 |

G 의 나머지 순수 헬퍼는 `AttackMath` + `NearestTargeting` 이 이미 덮는다. 단
**`PickFallbackTarget`(후보 조립)은 그 셋에 없었다** — 초판이 `NearestTargeting` 으로 덮인다고
셈했지만 실제로 옮겨진 것은 랭킹(`SelectNearest`)뿐이었고, 진영 마스크·자기 제외·그리드 변환·
`PastGoal` 조립은 arm B 가 옮겼다.

권장 순서는 **~~B~~ → D → E → C → F → A** 다. B·D·E 는 경계가 뚜렷하고 계약이 좁아 먼저 초록을
만들 수 있고, A(스냅샷 구축)는 C·F 가 무엇을 요구하는지 안 뒤에 짜야 헛수고가 없다.

**arm A 는 실측상 147줄이 아니다.** 그 대부분(lookup 27종 hoist)은 신 sim 에서 `world.Has/Get`
직접 호출로 **증발한다**. 남는 것은 ① 후보 스냅샷 ② 그리드 폴백 ③ 채널 writer 인데,
①②는 arm B 가 구 쿼리 조건 그대로 이미 세웠고 ③은 채널이 항상 존재해 사라진다
(`SimChannels` 주석). ⇒ **arm A 에 남은 일은 C·F 가 요구하는 스냅샷 축을 더하는 것뿐**이다.

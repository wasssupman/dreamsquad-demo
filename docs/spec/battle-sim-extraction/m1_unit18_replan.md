...
**지금 할 일: #33 `AttackSystem` 의 arm B.** 아래 [남은 arm 지도](#남은-arm-지도-구-attacksystemcs-라인-기준) 표에서
`172–228` 구간(캐스트 사건 드레인, 57줄)이다. 어휘 토대는 끝났으므로(`29953628`) **arm 은 전부
이미 있는 타입 위에 선다 — 새 타입을 만들 일이 거의 없다.**
...
44 시스템 중 **33 이식** · EditMode **2587 passed / 0 failed / 1 skipped / 1 inconclusive**.
그 inconclusive 는 **의도된 것**이다 — 이 런타임에서 할당 카운터가 항상 0 이라 프로브가
거짓 통과 대신 판정을 비운다(N2 참조). 초록으로 만들려고 손대지 말 것.

⚠ **#18 `HazardCast` 는 이식됐지만 아직 어느 클러스터에도 등록되지 않았다.** #33 클러스터를
만들 때 함께 넣는다(F6) — `SimPipeline` 은 번호 중복만 막고 **누락은 못 막는다**.

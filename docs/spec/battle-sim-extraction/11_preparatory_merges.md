...
- **`Golden` 을 PlayMode 실행 직후에 이어 붙이지 말 것.** 실측(2026-08-05): PlayMode 스위트가
  끝난 직후 `Golden` 트리거를 쓰면 에디터가 아직 Play 를 빠져나오는 중이라 재생성이 **기동하지
  못한다**. 트리거는 소비되고, 골든 파일은 안 쓰이고, 그 상태에서 백업 대비 `cmp` 를 돌리면
  **7/7 IDENTICAL 이 나온다** — 파일이 안 바뀌었으니 당연하다. 승격을 확인하지 않으면 이 공허한
  초록을 증거로 오독한다. 그래서 판정 순서는 ① staged 실행본 mtime 변화 = **기동 확인**
  ② 골든 mtime 변화 = **승격 확인** ③ 그 다음에야 `cmp`. 골든은 **단독으로** 돌린다.

### ⚠ PlayMode 스위트는 그 세션의 골든 검증을 오염시킨다 (2026-08-05 실측)

드림캐쳐 PlayMode 군(33건)을 돌린 뒤 골든이 **3회 연속** 같은 사유로 실패했다:
`normal two-run trace diff was non-zero`. 두 실행의 트레이스를 직접 비교해 원인을 특정했다 —
갈린 곳은 이벤트 스트림이 아니라 **헤더의 `configHash`** 였다:
...
두 실행이 서로 다르고 **둘 다 기준선과도 다르다** = 매치 설정 자체가 움직였다.
`git status Assets/_Project/Data/` 는 **비어 있다** — 디스크는 온전하다.

**시도한 복구와 결과** (모두 실패, 그리고 매번 해시가 **새 값**이었다):

| 시도 | 결과 | run1 / run2 configHash |
|---|---|---|
| 그냥 재실행 | FAIL | `c85c3208` / `bbdbcfd0` |
| 도메인 리로드 후 | FAIL | (동일 사유) |
| `Assets/_Project/Data` 강제 재임포트 후 | FAIL | `7ccadf0d` / `c68c22bf` |

⇒ **"한 번 변조돼 틀린 값으로 고정" 이 아니다. `configHash` 가 실행마다 달라졌다.**

### ✅ 해소 (2026-08-05) — 원인 2개, 재기준선 없음

**결론부터**: 골든 7종 **two-run diff 0 · 커밋된 코퍼스와 byte 동일**. 재생성이 필요 없었다. 로그
전체에서 기준 해시 `d3ded5d0` 은 **26회** 재현되고 흔들린 구간은 오염된 그 한 창뿐이다. unit 13 C3 의
"골든 미검증" 도 이때 해소됐다.

**결정적 도구**: 러너가 canonical blob 을 `Library/LegacyTraceV0/{scenario}.run{N}.blob.txt` 로
덤프한다. **두 블롭을 diff 하면 범인 줄이 즉시 나온다** — 위의 "다음 후보" 추론(정렬·반사 순회·
`PresentationFields`)은 전부 헛다리였고, 블롭 diff 는 한 번에 `skillLoadout` 을 지목했다.
*불안정한 해시를 만나면 추론하지 말고 블롭을 뽑는다.*

원인 ① **근인 = 진입 경로 갈아타기**. 선행 PlayMode 스위트가 `PlayerProfileSO` 를 메모리에서 스쿼드
보유 상태로 남겨, 하네스가 draft 대신 squad 경로로 진입했다. 그 경로만 `SetSkillLoadout` 을 부르므로
draft 에서 `null` 로 캡처되던 필드가 실제 스킬 2개로 채워졌다. 판별: 정상 세션 로그에는
`[DraftController] Roll 완료` 가 있고 실패 구간에는 **0개**였다.

원인 ② **잠재 결함 = 벽시계 시드**. squad/TestMode 경로의 스킬 로드아웃이 `DateTime.UtcNow.Ticks`
로 굴렀다. 오염이 이 결함을 **만든 게 아니라 노출**했고, squad 는 실제 게임 경로다. 수리는
`MatchSeed.DeriveSkillSeed` + `GameManager.NextSkillRollSeed()` — 상세는
[`3_canonical_match_config.md`](3_canonical_match_config.md) "골든 게이트 정지 사건".

- **위생 규칙**: 골든으로 검증할 세션에서 PlayMode 스위트를 먼저 돌리지 않는다. 이미 돌렸으면
  러너 `ReimportData` 로 SO 를 디스크에서 재적재한 뒤 골든을 돌린다. **도메인 리로드로는 낫지
  않는다** — 에셋 인스턴스는 리로드를 넘어 산다. (당시 재임포트 직후 실행도 실패한 것으로 기록했지만,
  그 실행은 재임포트가 반영되기 전 상태였다. 이후 같은 세션에서 정상 복귀했다.)
- **오독 주의 — 판별법**: 골든 실패를 볼 때 `configHash` 를 먼저 본다. 해시가 골든과 다르면
  **설정**이 움직인 것(환경/unit 3), 해시는 같은데 스트림이 다르면 **sim** 이 움직인 것. **후자만
  회귀다.** 이 구분이 없으면 멀쩡한 커밋을 되돌리게 된다.
- **이 게이트가 멎으면 M1 의 남은 유닛(14~20)은 진행 불가**다 — 그 전부의 완료 기준이 "골든 byte
  diff 0" 이고 스펙이 "골든이 그 증인이다" 라고 못박았다.

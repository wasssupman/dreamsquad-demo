...
⇒ **"한 번 변조돼 틀린 값으로 고정" 이 아니다. `configHash` 가 실행마다 달라진다.** 즉 정규화 입력에
실행마다 변하는 값이 섞여 있다 — **unit 3(`MatchConfigSnapshot`) 의 결함**이며 unit 13 과 무관하다.
같은 밤 앞선 실행들(로그 PASS @639241 까지)은 정상이었으므로 **조건부**로 발현한다.

조사 배제 결과: 정렬 2곳은 총순서다(`effectTiles` 는 (y,x), 필드는 이름 ordinal) — 불안정 정렬이
아니다. 다음 후보는 `CanonicalWriter` 가 반사로 훑는 값 중 실행마다 변하는 것, 또는
`PresentationFields` 제외 목록이 놓친 휘발성 필드다.

- **이 게이트가 멎으면 M1 의 남은 유닛(14~20)은 진행 불가**다 — 그 전부의 완료 기준이 "골든 byte
  diff 0" 이고 스펙이 "골든이 그 증인이다" 라고 못박았다. 규칙 적출을 증인 없이 하는 것이 정확히
  스펙이 금지한 것이다.
- **다음 세션의 첫 할 일**: 갓 시작한 에디터에서 골든을 돌려 ① 통과하면 세션 오염(위생 규칙으로 종결,
  PlayMode 스위트를 골든 앞에 돌리지 않는다) ② 실패하면 `MatchConfigSnapshot` 의 불안정 입력을
  찾아 고치는 것이 **unit 14 보다 앞선다**.
...
- 러너에 `ReimportData` 모드를 추가했다(디스크에서 SO 재적재 = 에디터 재시작 대용). 이번엔 듣지
  않았지만 메모리 변조형 오염에는 유효한 도구다.

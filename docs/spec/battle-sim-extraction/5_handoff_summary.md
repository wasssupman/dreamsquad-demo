...
## ⚠ 골든 코퍼스가 갱신됐다 — M1 을 시작하기 전에 읽을 것 (2026-08-31)

`distance-based-range` spec 이 **사거리 술어를 바꿨고**, 그 결과 이 코퍼스의 기준선이 다시 구워졌다.
M1 의 A/B 대조는 **지금 기준선**을 쓴다.

| 무엇 | 어디 |
|---|---|
| 자가 어떻게 바뀌었나 | `docs/spec/distance-based-range/4a_reach_predicate.md`(rev 2 절 포함) |
| 광역 멤버십 | 같은 폴더 `4b_area_membership.md` |
| 획득/유지 히스테리시스 | 같은 폴더 `4d_hysteresis.md` |

**코퍼스 자체에 생긴 변화 셋:**

1. **시나리오가 8건이 됐다** — `summoner` 추가. 씬 기본 덱에 소환사가 없어 **순찰병(이 프로젝트의
   유일한 연속 이동 아군)이 구조적으로 코퍼스에 못 들어왔었다.** `Scenario.defenderIds` 로 그
   시나리오만 덱을 갈아끼운다.
2. **게이트 2종이 늘었다** — 셋업 실패(덱을 못 만듦)와 빈 트레이스는 **저장하지 않는다.**
   그리고 `Bake Missing Goldens Only` 메뉴가 생겼다: 시나리오를 **추가**할 때 기존 기준선을
   덮지 않는다(전체 재생성은 기존 감시를 그 자리에서 무효로 만든다).
3. **`PatrolSpawn` 채널의 페이로드가 바뀌었다** — 기록 시점이 `CreatePatrolEntity` **뒤**로
   옮겨졌고(전엔 스냅 실패로 소환이 취소돼도 찍혔다), `b` 에 순찰병 SimEntityId 가 실린다.

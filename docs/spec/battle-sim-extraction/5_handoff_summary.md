...
4. **28채널을 단일 이벤트 스트림으로 붕괴하지 마라.** 같은 틱 소비 계약인 내부 phase 메시지(CastEvents→AttackSystem, BlinkRequests)가 있다 — 이벤트 3분리(내부 phase queue / AMR / presentation projection)가 계약이다.
...
7. 점수는 전부 int — parity 비교는 exact(±ε 금지). `RequireForUpdate` 게이트는 39개 시스템의 **행동**이다(IncomingDamage 0이면 Regen 힐도 정지) — 항상-틱으로 바꾸면 조용히 깨진다.
...
- main HEAD에 CardBuffs PlayMode 사전 실패 존재(가디언 dmgTaken 여분 ×1.25) — unit 4에서 수리 또는 명시 제외 결정.
...
- M0 완료 시 M1 units(6~)를 설계 정본의 M1 절 기준으로 분해해 이어 쓴다.

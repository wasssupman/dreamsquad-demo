...
| ~~**18-E**~~ | ✅ **7/8 완료** — 필드·존·해저드. **#18 `HazardCast` 는 18-I 로 이관**(아래) | 7 | 686 | 특성화 4 선행 + 복제 | tie-break ⑥ 양쪽 박제 | — |
...
| **18-I1/I2** | 공격 루프 (후보·타겟팅 / 출력해결·발사) **+ #18 `HazardCast`**(18-E 에서 이관) | 2 | **1,870** | 6파일 | adapt | 단일 파일 최대. 동률 tie-break |
| **18-J** | 기믹·보스·임계·도약 + **`_meteorRng`** | 9 | 1,171 | **최다 결손** | rewrite 2 | 테스트 부채 |

#### T1 순서 (rev 3.1 — 재검토 후 **원안 유지**)

`E → F → G → H → I → J` 를 `F → G → E …` 로 바꾸려다 **철회했다.** 기록으로 남긴다 —
같은 오판을 다음 사람이 반복하지 않게.

**재정렬 근거였던 것**: `LocalTransform` 이 25파일, `Health`·`DeadTag`·`IncomingDamage` 가
42파일에 걸쳐 남은 6조각 전부가 쓴다. 그 소유자가 F(#17 "위치 갱신 단일 권한")·G(#41
"유일한 파괴자")이니 앞으로 보내야 한다고 봤다.

**왜 틀렸나**: 그 축이 약하다. 위치는 **리프 타입**이고, 리프 타입은 *먼저 필요한 쪽이
정의한다* 는 정책이 이미 두 번 적용됐다(`Health` ← 18-C/5, `IncomingDamage`·`BossTag` ← 18-D).
정의 비용이 20줄이라 소유 시스템을 기다릴 이유가 없다.

**진짜 축은 반대 방향이다**: `MovementSystem` 이 방향을 정하는 데이터 —
`FlowField`·`DefenderField`·`Obstacle`·`PatrolStep`·`AggroChaseCell`·`PortalLink`·`TornadoField` —
를 **전부 18-E 가 굽는다.** 캡처 번호가 이미 그렇게 말하고 있었다: **E 는 #1~#7(P1),
Movement 은 #17(P4)**. E→F 는 생산→소비다. 순서를 뒤집으면 F 가 스텁 위에 서고,
그 스텁이 곧 "오라클 없이 발명한 타입" 이 된다 — 내가 E 를 두고 걱정한 바로 그것이다.

⇒ **원안 유지.** 다만 탐색에서 건진 것 둘은 남긴다:
- **리프 타입 정책**을 명문화한다(위 문단). 조각 경계로 타입을 미루지 않는다.
- **사망 릴레이의 재설계 위험은 착수 전 판단보다 낮다**(실측 후 하향) — 마킹(#11 P3 / #34 P9)
  → 관찰(P10) → 파괴(**#41 P12 단독**)이고, `SimWorld.Destroy` 의 "P12 만 부른다" 가 이미
  그 계약을 표현한다. 18-G 를 앞으로 당길 이유가 없다.

#### #18 `HazardCast` 의 18-E → 18-I 이관 (rev 3.2, 실측 근거)

그 시스템의 캐스트-이벤트 arm 이 `DcTriggerSlot` 의 **버퍼 존재**를 본다("카드가 붙은 캐스터만").
그 타입은 **25필드 + `Wassup.Data` enum 4개**(`DcTriggerKind`·`DcPayloadKind`·`DcGateKind`·
`DcGateSubject`)에 걸쳐 있고, 자기 주석이 *"owned write: AttackSystem 전용"* 이라고 못박는다.

⇒ **존재 확인 하나 때문에 18-I 의 타입을 추측으로 먼저 옮기지 않는다.** 필드 하나만 틀려도
상태 라인이 갈리고, 그건 `AttackState` 를 9필드 전부 옮긴 것과 같은 이유다(부분 이식 금지).
나머지 의존(`HazardCastState`·`HazardSpawnRequest`)은 Effects 것이라 18-I 가 함께 가져간다.

이 누락이 **사고가 아니라 결정**임을 코드에도 남겼다 —
`SimEnvironmentClusterTests.HazardCastIsAbsent_BecauseItMovedTo18I` 가 캡처 번호 집합을 단정한다.

...
## 🚨 다음 세션의 첫 할 일 — 골든 게이트가 멎어 있다

**갓 시작한 에디터에서 골든을 먼저 돌린다.** unit 14 를 시작하기 전이다.

`normal` 시나리오의 두 실행이 서로, 또 커밋된 골든과도 다른 `configHash` 를 낸다. 재실행·도메인
리로드·`Assets/_Project/Data` 강제 재임포트 3회 모두 실패했고 **매번 해시가 새 값**이었다. 갈린 곳이
이벤트 스트림이 아니라 **헤더의 configHash** 이므로 **설정**이 움직인 것이고 sim 회귀가 아니다.
증거·시도 표·판별법은 `11_preparatory_merges.md` 의 "PlayMode 스위트는 그 세션의 골든 검증을
오염시킨다" 절에 있다.

- 통과하면 → 세션 오염이었다. **위생 규칙**으로 종결: 골든으로 검증할 세션에서 PlayMode 스위트를
  먼저 돌리지 않는다. C3(`52454aa4`)의 골든 미검증도 그때 해소한다.
- 실패하면 → `MatchConfigSnapshot`(unit 3) 의 불안정 입력을 찾아 고치는 것이 **unit 14 보다
  앞선다**. 정렬 2곳은 총순서임을 확인해 배제했다(다음 후보는 `CanonicalWriter` 의 반사 순회 값과
  `PresentationFields` 가 놓친 휘발성 필드).
- **왜 이것이 선행인가**: units 14~20 전부의 완료 기준이 "골든 byte diff 0" 이고 스펙이 "골든이 그
  증인이다" 라고 못박았다. 규칙 적출을 증인 없이 진행하는 것이 스펙이 금지한 바로 그것이다.

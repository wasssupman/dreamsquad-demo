...
## ✅ 골든 게이트 — 해소됨 (2026-08-05)

블로커였던 `configHash` 불안정은 종결됐다. **골든 7종 two-run diff 0 · 커밋된 코퍼스와 byte 동일**
(재기준선 없음). C3(`52454aa4`)의 "골든 미검증" 도 이 실행으로 해소됐다. units 14~20 은 증인을 갖고
진행할 수 있다.

원인 2개 — ① **근인**: 선행 PlayMode 스위트가 `PlayerProfileSO` 를 메모리에서 스쿼드 보유 상태로
남겨 하네스가 draft 대신 squad 경로로 진입했다(그 경로만 `SetSkillLoadout` 을 부른다). ② **잠재
결함**: squad/TestMode 경로의 스킬 로드아웃이 벽시계로 굴렀다 — 오염이 만든 게 아니라 노출했고,
squad 는 실제 게임 경로다. 상세·증거는 `3_canonical_match_config.md` "골든 게이트 정지 사건".

**남은 운용 규칙 3개**:

- **위생**: 골든으로 검증할 세션에서 PlayMode 스위트를 먼저 돌리지 않는다. 이미 돌렸으면 러너
  `ReimportData` 후 골든. 도메인 리로드로는 낫지 않는다(에셋 인스턴스가 리로드를 넘어 산다).
- **판별법**: 골든 실패 시 `configHash` 를 먼저 본다. 해시가 다르면 **설정**(환경/unit 3), 해시는
  같고 스트림이 다르면 **sim**(진짜 회귀). 이 구분이 없으면 멀쩡한 커밋을 되돌린다.
- **불안정 해시는 추론하지 말고 블롭을 뽑는다**: `Library/LegacyTraceV0/{scenario}.run{N}.blob.txt`
  두 개를 diff 하면 범인 줄이 나온다. 이번에 정렬·반사 순회를 의심하며 세 번 오진했다.
...
| `e41a30a4` | unit 13 **C2** — 배치·방향·재배치를 커맨드로 (골든 초록 확인) |
| `52454aa4` | unit 13 **C3** — 카드 4변종 커맨드화 (**⚠ 골든 미검증** — 위 블로커) |

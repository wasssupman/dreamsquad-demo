...
> **현재 상태(2026-07-30): 미착수.** units 0~3 은 코드·테스트 완료지만 이 unit 은 **Unity 에디터가 필요**하다. 이 세션에는 Unity MCP 브리지가 없고 `OutgameScene.unity` 가 dirty(사용자/병행 세션 WIP)라 씬 YAML 직접 편집은 하지 않았다 — 에디터가 열고 있는 씬을 밖에서 고치면 임포트 전까지 무효이고, 사용자가 저장하는 순간 덮인다.
>
> **미배선이어도 기능은 동작한다** — 카탈로그가 null 이면 팝업이 전 항목을 raw id 로 그린다(계약 9). 배선은 이름·아트를 살리는 업그레이드다.

...
12. **"프리셋 적용" 버튼 영역을 잡는다 — 이번 spec 은 레이아웃까지다.** 목록 아래 고정 영역에 버튼을 두되 **비활성**(`interactable = false`)으로 둔다. (구현 정정: 탭마다 별도 버튼이 아니라 **두 탭이 공유하는 하단 영역 하나**다 — 탭을 오갈 때 자리가 뛰지 않아야 한다는 요구를 그쪽이 더 잘 만족한다.) 눌러도 아무 일도 안 하는 활성 버튼은 결함으로 신고된다. 자리를 지금 잡는 이유는 나중에 끼워 넣으면 두 탭의 레이아웃을 다시 짜야 하기 때문이다.
...
- **프리셋 적용 기능** (별도 spec) — 계약 12 가 잡아둔 버튼을 실제로 연결한다. 본 프로필의 선택 스쿼드·덱에 쓰는 것은 `PresetApply.WriteToProfile` 이 이미 한다. 판단이 필요한 지점: (a) 드림스톤 — 헬퍼가 안 건드린다, (b) 내 카탈로그에 없는 id 가 섞인 덱을 적용하면 어떻게 되는가, (c) 적용 후 저장 시점(`ProfileStore.Save`)과 확인 절차. Scope: S~M

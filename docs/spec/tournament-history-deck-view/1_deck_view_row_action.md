...
- [x] 컴파일 통과
- [x] 결과 화면(`ResultScreen`)의 랭킹 행 모양이 **변하지 않는다**. (구현 중 확인: `ResultScreen` 은 `Render` 를 아예 쓰지 않고 자체 행 페인팅을 갖는다 — README 계약 2 정정 참조. `Row` 에 필드를 늘린 것이 유일한 접점이므로 선택 인자 기본값으로 흡수했다)

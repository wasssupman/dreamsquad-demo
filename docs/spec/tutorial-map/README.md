...
| 10 | 30s | **Dragon ×1** + Vanguard ×4 + **Whirlpot ×1** | 비행 엘리트 + 호위 + 회오리 — 총복습 |

**총 240초 · 라이브 로스터 15종 전부 등장.** (Whirlpot 은 `elite-whirlpot` 세션이 로스터에 추가하며 웨이브 10 에 함께 넣었다 — 아래 EditMode 단언이 그 갱신을 요구한다.) 단순→복합, 지상→공중, 일반→엘리트 순으로 쌓는다. `timerDurationSec: 0` 이라 **시간 압박이 없다**(전 웨이브 처리 후 승리).

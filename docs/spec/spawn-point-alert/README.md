...
- **시뮬 무변경**. ECS·스폰 타이밍·웨이브 스케줄은 그대로. 얼럿은 플랜을 읽는 순수 프레젠테이션.
...
- **Wave 1 은 자연 스킵**: trigger 0초라 창이 배틀 시작 이전 → 특수 분기 없이 안 뜬다.
- **Next Wave 강제 호출도 자연 스킵**: 강제 시점 = 스폰 시점이라 창이 소멸. 명시 계약(버그 아님).
...
| 트리거 | N/A(큐 아님) — `BattleBridge` 웨이브 플랜 예보 폴링 | NextWaveDock 과 같은 read-only 폴링 |
...
- Wave 1 사전 얼럿 (배틀 시작 전 페이즈에 노출 — 시작 시점 예지가 필요해 별도 결정)

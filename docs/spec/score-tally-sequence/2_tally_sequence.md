...
| 구간 | 목적 |
|---|---|
| `preRollSec` 1.0 + `dimFadeSec` 1.0 (**동시**) | 마지막 킬을 눈으로 마무리하는 동안 딤이 서서히 깔린다 |
| **`PulseAttention()`** | 값 없는 배지 펀치 — 시선을 우상단으로 끌어온다 |
| `labelLeadSec` 0.25 | **라벨이 먼저, 숫자는 나중.** 동시에 하면 어디를 볼지 모르는 사이 롤업이 끝난다 |

가장 큰 개선은 **라벨 선행**이다. 실측에서 라벨(1.54s) → 숫자(1.78s) 로 0.24초 리드가 생긴다.

**딤을 여운과 겹친 이유**: 따로 두면 "아무 일 없음 → 갑자기 어두워짐" 두 사건이 계단처럼
끊긴다. 1초에 걸쳐 겹치면 "전투가 잦아든다"는 하나의 제스처가 된다. 램프가 길어 초반엔
거의 투명하므로(1초 중 0.3초 시점에 알파 0.17) 마지막 킬 VFX 를 가리지 않는다.

총 4.0초. 전부 인스펙터 값이라 조정이 싸다.
...
**종결됨** (README 결정 D, 2026-07-21 체감 확인): Tally 진입 즉시 전투 HUD 가 걷히는 것은 그대로 둔다. 여운의 정의를 "HUD 걷힌 전장의 마지막 킬 여운"으로 문서를 실제에 맞췄다. `SetPhase` 를 preRoll 뒤로 미루는 방향은 **기각** — 재시도하지 말 것.

확인: 2026-07-21

```
t=0.0~1.0   전투 여운 + 딤 서서히 (동시) → 배지 펄스
t=1.54      "시간  +11,500"      ← 라벨 먼저
t=1.78      HUD 6,400 → 17,900   ← 0.24초 뒤 숫자
t=2.77      "스트레스  +7,200"
t=3.04      HUD → 25,100
t=4.01      → onDone
```

> 딤 램프의 **모양은 직접 샘플하지 못했다.** `EditorApplication.update` 가 비포커스 구간에서
> 멈춰 1초 창을 세 번 놓쳤다. `Mathf.Lerp` 선형 보간이라 산술은 자명하고, 정작 확인이 필요한
> "마지막 킬이 가려지나"는 사람 눈으로 볼 문제다.

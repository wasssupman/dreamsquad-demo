# 10 — 진입 다듬기 (코스트 잠금 · 초상화 드래그 지름길)

## 목적

코스트가 없으면 **이동모드에 들어가지 못하게** 하고(들어가서 아무 데도 못 놓는 최악을 막는다),
트레이의 그 유닛 **초상화를 끌면** 버튼을 거치지 않고 바로 이동모드로 넘어가게 한다.
...
> **확인 2026-08-13** · 커밋 `1e7a3b9e`(코스트 잠금) · `088030d7`(표면 정정) — 사용자 Play 확인 완료.
> 자동 검증: PlayMode `RelocationMoveModeTest` 4/4 · 재배치+`BoardLimit*` 9/9.
> 이동모드 하이라이트가 안 뜨던 **선행 버그**(`5de9d07f`)를 같이 고쳤다 — 이 unit 밖의 결함이지만
> unit 9 의 소스 칸 어포던스가 그 기능 위에 얹혀 있어 죽어 있으면 반쪽이 된다.

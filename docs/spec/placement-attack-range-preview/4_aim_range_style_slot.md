...
> **갱신 2026-07-31 (`e45ee478`)**: 배치 단계 `rangeColor` 가 **라임 `(0.55,1,0.25)`** 이 되고
> `aimRangeColor` 는 주황으로 남았다. 아래 "색은 배치 단계와 같은 주황"·"두 단계를 가르는 신호는
> 색이 아니라 형태다" 는 **2026-07-28 시점 기록**이다 — 슬롯을 나눠둔 덕에 코드 변경 없이 갈라졌고
> (`:16-18` 이 예고한 그대로), 지금은 형태(outline↔solid)와 색이 함께 단계를 가른다.
...
확인 2026-07-28 (사용자 Play) · 커밋 `7456dedb`.

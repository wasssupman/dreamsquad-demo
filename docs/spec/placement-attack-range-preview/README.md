...
> **색 갱신 (2026-07-18, 브랜치 `feat/placeable-tile-highlight`)**: `rangeColor` 노랑 → **주황 `(1,0.55,0.12)`**.
> 배치 가능 하이라이트(쿨 시안)와 공존하도록 웜/쿨 보색 대비를 준다(사용자 렌더 비교 확정). 라이브 TileSet 에셋 4개에 반영.
> 상세: `docs/spec/placement-eligible-tile-highlight/`. 형태·펄스 계약은 불변.

> **색 갱신 (2026-07-31, `e45ee478`)**: 배치 단계 `rangeColor` 주황 → **라임 `(0.55,1,0.25)`**(사용자 결정).
> `aimRangeColor`(드롭 후 조준 페이즈)는 **주황 유지** — unit 4 가 형태(outline↔solid)로만 갈랐던 두 단계가
> 이제 색으로도 갈린다. 무효 적색과의 대비도 주황일 때보다 커진다. 형태·펄스·유효성 계약은 불변.
...
- **시각 형태 = 격자 outline (map 가시성)**: `rangeTile` 스프라이트는 **셀 테두리만 그리는 격자 outline**
  ...
  주황 solid 채움. 가림은 적용 구간을 조준 페이즈로만 좁히고 알파를 낮춰(내부 실효 0.5)
  완화한다. **배치 단계(드래그 프리뷰)·스킬 조준/텔레그래프는 outline 그대로** — 원 결정이 지켜지는
  구간이다. 두 단계를 가르는 신호는 형태였고, 2026-07-31 배치 단계가 라임으로 바뀌며 색도 함께
  가른다(그때까지는 양쪽 다 주황이라 형태가 유일한 신호였다). 현재 에셋

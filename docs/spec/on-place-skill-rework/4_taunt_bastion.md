...
> (아틸러리·폭탄맨만 지상 전용). 그래서 당시의 배스티온 도발은 **비행 적도 끌어왔다** — 게이트가
> 죽은 게 아니라 통과시키는 것이 의도였다.
> **rev 2026-09-03(bc37b57c)**: 근접(사거리 1)은 전원 지상 전용으로 재개정 — 배스티온 저작이
> `Path`(2) 로 돌아가 도발도 **공중을 다시 안 끌어온다**(사용자 확정: 어차피 근접이 못 때리는
> 대상을 부르는 건 무의미). 위 8/17 문단이 예고한 「지상 전용 유닛에 도발」 상태가 현재다.
...
## 연출 — 배치 VFX 는 `EarthAura`, 지속 표시는 이 spec 밖 (2026-08-16)

배스티온의 `placementVfxPrefab` 이 `FireAoeVFX` 였다(배치 효과가 「주변 폭발」이던 시절의 배선).
프로젝트의 배치 VFX 팔레트(PixPlays `ElementalAOE` 4종)는 **전부 밖으로 퍼지는 링·폭발**이라
「끌어온다」와 어휘가 정반대다. 오라 계열로 바꿨다 — 소용돌이가 유닛을 감싸고 도는 그림이라
「시선이 여기로 모인다」로 읽힌다. `EarthAura`(붉은 갈색 + 맴도는 돌덩이)가 탱커 톤에 맞다.
배제: `Tornado_SKELETON`(토네이도 스킬 전용 룩) · `EmpowerAura`(드림캐쳐 강화 오라로 의미 선점).

⚠ **이 슬롯으로 5초 지속을 표현할 수 없다.** `placementVfxPrefab` 은 배치 순간 원샷이고
`max(deploymentDuration,1)+0.25` 초 뒤 강제 파괴되므로 looping 오라가 ~1.2초에 잘린다.
도발 지속 내내 유지되는 상태 연출은 `aggro-targeting` 후속 후보 「어그로 아이콘 →
unit-status-fx 승격」의 몫이다(현 어그로 표시는 머리 위 "!" 플레이스홀더).

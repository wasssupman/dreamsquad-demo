...
- **캐논** — 반경 2 안 **모든 적에게 1:1** 낙하 미사일. 피해 총량은 기존과 동일(적당 80,
  `impactTileRange 0` 이라 겹침 없음). 바뀐 것은 예고 0.4초와 하늘에서 내려오는 그림.
...
- **`PatternScope` 는 셀 dedupe 를 하지 않는다.** 1:1 사양이라 같은 칸 적 둘이면 미사일도 둘.
...
- **전투 시작 전 배치는 캐논 미사일이 한 발도 안 뜬다.** 브리지의 `DrainProjectileSpawnRequests`
  ...
  만들어지고도**(실측 `maxCarrier=3`) 투사체가 0이다. 낭비가 아니라 **요청이 큐에 남는다**.

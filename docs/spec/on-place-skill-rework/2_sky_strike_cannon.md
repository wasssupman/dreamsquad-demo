...
| `shots` | 1발 · 간격 0 | 한 번의 일제사격. **동시 착탄**이다(캐스케이드는 후속 후보) |
...
- **동시 착탄이다.** 예고 0.4초 뒤 반경 안 미사일이 한꺼번에 터진다. 적이 그 사이 걸어
  나가면 빗나간다 — 조준 폭격의 성질이며 예고 시간이 Play 튜닝 대상이다.
...
- [ ] Play 육안: 적 무리 옆에 캐논 배치 → **적이 있는 칸마다 미사일이 하나씩 하늘에서
      내려와 동시에 터진다.** 빠진 칸이 없고, 한 칸에 겹쳐 떨어지지 않는다

...
README 후속 후보 「배치 페이즈 발동 정책」이 가리키던 비용의 실물이며, 예상보다 나쁘다 —
피해가 낭비되는 정도가 아니라 **요청이 큐에 쌓인 채 남는다**(전투가 시작되면 뒤늦게 터질 수
있다). 이 spec 에서 고치지 않고 후속 후보에 실측을 붙였다. 테스트는 `StartBattle()` 뒤에 배치한다.

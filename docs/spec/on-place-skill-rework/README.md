...
**캐논의 피해 총량은 바뀌지 않는다** — 적당 정확히 80 으로 기존과 같고(`impactTileRange 0` 이라
겹침 없음), 바뀌는 것은 예고 0.4초와 하늘에서 내려오는 그림이다. 사용자 의도(2026-08-16):
...
- **배치 페이즈 발동 정책** [M] · 전투 시작 전 배치는 적이 없어 셋 다 통째로 낭비된다
  ...
  캐논도 같다. 최소 대응 후보: 규칙 경로에 한해 「후보 0이면 시도를 소진하지 않는다」
  (레거시 5분기 회귀 표면 0) 또는 전투 시작 시점 지연. 육안 검증에 시나리오를 넣어 두었다.

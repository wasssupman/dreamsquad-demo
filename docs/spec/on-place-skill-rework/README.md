...
2. **`OnPlaceEffectType` enum 을 더 늘리지 않는다.** 신규는 전부 규칙으로 간다.
   ...
   - **예외 1회 (2026-08-19)**: `docs/spec/on-place-shuttle-shotgun/`(실드셔틀·샷건맨)이 이 선행
     조건을 **건너뛰고** 신규 2종을 얹었다. 근거 = 신규가 전부 규칙 경로라 과도기를 키우지 않고,
     오히려 샷건맨이 **`onPlacePush*` 의 마지막 소비자**여서 그 3필드와 `ApplyOnPlacePush` 는
     이제 **소비자 0** 이다(defender 27종 전수 확인). 이관 작업은 그 조각을 무비용으로 뗄 수 있다.
     enum 9개 소비자는 그대로 남아 있으므로 **선행 조건 자체는 유효하다.**

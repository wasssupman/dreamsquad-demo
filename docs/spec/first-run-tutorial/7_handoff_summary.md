...
- **첫 배치** 전투 N초 후 정지 → 캐논 셀 → **적이 내 목표 5칸 안까지 올 때까지 대기** →
  무기 소개(3타일 폭격) → 배치 → 정지 풀고 배치 스킬 관람 → 문구.
- **드림캐쳐 부착** 재개(플레이 구간) → **트레이 셀 탭으로** 유닛 선택 → 부착 가능한 Unit 카드만 열기 → 부착 → 마무리.
- **온보딩 전용 웨이브** `WavePlan_FirstRunTutorial`(60초 · 4웨이브 · 적 32, 1웨이브 15 동시).
  맵·덱은 라이브 그대로. 레인 8/7 분배는 라운드로빈이 자동으로 한다.
...
6. **부착 대상 = «트레이 셀로 선택할 수 있는» 배치 유닛**(소진 포함). 이게 B4 의 불변식이다 —
   보드 탭 선택이 꺼진 뒤로(`selection-entry-narrowing`) 진입구가 그것뿐이라, «배치돼 있다»
   만으로 고르면 조건이 영영 안 서고 타임아웃이 없어 멈춘다.
7. **B3→B4 고정 대기에는 차단막을 유지한다.** 열면 트레이의 캐논 셀(배치 후 «소진») 탭이
   곧 유닛 선택이라 손패가 미리 열린다. 차단막을 내리는 건 **부착 대상이 없을 때만** —
   그때는 플레이어가 더 놓아야 B4 가 성립한다.
...
11. **"3타일" 문구는 `Pattern_Cannon_Strike.scopeTileRange` 를 주장한다.** 그 값을 바꾸면
    문구도 같이 바꾼다.
12. **웨이브 그룹에 레인을 박지 마라.** 무지정이면 라운드로빈으로 양쪽에 갈리는데, 박으면
    한쪽으로 몰린다.

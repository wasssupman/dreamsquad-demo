...
- ⚠ **기준은 시간이 아니라 사건이다 — 적이 «강가»에 도달하는 순간.**
  판정은 `BattleBridge.AnyEnemyPastApproachLine(unit, riversideTiles)` **하나**다. 적 위치가
  ECS 에 있으므로 게이트웨이 안에 둔다(제약 1). 컨트롤러는 지형을 캐묻지 않는다.
...
- **열 좌표를 박지 않는다** — `Env` 타일 자체를 기준으로 삼으므로 강이 어디에 어떤 모양으로
  나 있든 같은 규칙이 선다.
- 강이 없는 맵의 폴백(배치 영역 진입)도 **그 메서드 안에** 있다. 폴백이 없으면 조건이 영영
  안 서고, 타임아웃이 없으므로(계약 11) 그대로 멈춘다. 강 유무는 맵당 한 번만 훑고 캐시한다.

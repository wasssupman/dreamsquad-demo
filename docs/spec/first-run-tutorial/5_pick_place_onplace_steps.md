...
### 3.1b 악몽이 내 영역 깊숙이 들어올 때까지

`"악몽이 배치 영역 안으로 들어오면!"` — **입력은 막은 채 시간만 흘린다.**

⚠ 이 대기가 없으면 다음 문구(«적들의 머리위에 배치해보세요»)가 적이 아직 멀리 있을 때 뜬다.
문장과 화면이 어긋나고, 그렇게 놓은 배치 스킬은 아무도 못 때려서 이어지는 «전황을 유리하게»
가 통째로 희석된다(실제 Play 에서 그랬다).

- ⚠ **기준은 시간이 아니라 사건**이고, 척도는 **내 목표까지의 거리**다
  (`BattleBridge.AnyEnemyWithinTilesOfGoal(approachGoalTiles)`). 적 위치가 ECS 에 있으므로
  게이트웨이 안에 둔다(제약 1).
- **왜 목표 거리인가**: «내 영역으로 얼마나 들어왔나» 를 직접 말하는 유일한 척도이고,
  목표는 모든 맵에 있어 «이 맵엔 기준이 없다» 가 생기지 않는다. 폐기한 두 후보 —
  ① 배치 영역 진입: Duel 은 배치 영역이 `x≤14` 라 스폰 직후 걸려 문구가 0초 노출된다.
  ② 강(`Env` 타일) 도달: 아직 멀어 배치 스킬이 한 무리를 못 덮고, 강 없는 맵을 위한
  폴백이 따로 필요했다(그 폴백은 ①로 떨어져 고친 버그를 되살렸다).
- **값**: `approachGoalTiles = 5`. Duel 기준 목표 `x=2`, 강 `x=10` → 5 는 **강 안쪽 3칸(x≈7)**.
  체비셰프 거리이고, 목표가 여럿이면 가장 가까운 것 기준이다.
- 정지를 푼다 — 멈춘 채 기다리면 적이 영원히 안 온다. 차단막은 유지(할 일이 없는 구간).
- 배치 영역 하이라이트를 켜둔다 — 문구의 «배치 영역» 이 화면에서 실제로 보여야 한다.

### 3.1c 무기 소개

`"캐논은 3타일 영역에 강력한 폭격을 합니다!"` — `skillHintSeconds`(2.5초) 동안 **정지한 채**
읽는다. 적이 몰려 선 화면이 배경이라 «저기에 놓으면 저게 다 맞는다» 가 눈으로 이어진다.

근거를 먼저 주는 자리다. 없으면 다음 문구가 «시키니까 하는» 지시로만 남는다.

⚠ **"3타일" 은 실제 값이다** — `Pattern_Cannon_Strike.scopeTileRange = 3`. 그 값을 바꾸면
이 문구도 같이 바꾼다(문구가 스펙을 주장하고 있다).

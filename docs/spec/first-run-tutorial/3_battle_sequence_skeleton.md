...
**차단막은 전 구간 떠 있다**(계약 5). 배틀에서는 **불투명도 0** — 입력만 막고 화면은 그대로 둔다.
...
스텝 = `(진입 연출, 완료 조건)`. 완료 조건은 기존 이벤트 구독이고 **타임아웃은 없다**
(계약 11) — 안내가 요구한 행동을 할 때까지 기다린다. 대신 **모든 조건 대기는 만족
가능해야 한다**: 선행조건이 없으면 기다리지 말고 진입 전에 건너뛴다. 건너뛸 때도
딤/구멍/정지 정리는 같은 해제 지점을 지난다.

남는 상한은 **카운트다운 홀드 하나**다(`introHoldMaxSeconds`). 성격이 다르다 — 컨트롤러가
죽어 Release 를 못 부르면 카운트다운이 3에서 멈춰 전투가 시작조차 못 하고, 그건 플레이어가
풀 수 없는 상태다. 스텝 대기는 플레이어가 언제든 풀 수 있다.
...
- 선행조건 부재로 구간을 건너뛰어도 딤이 걷히고 판이 정상 속도로 돌아간다.

...
7. TIME 배지 포커스와 문구를 정상적으로 연 순간 B5를 완료로 확정한다. 이 단계는 사용자
   행동을 요구하지 않으므로, 노출 대기 중 판이 끝났다는 이유로 전체 온보딩을 반복하지 않는다.
8. 문구 노출이 끝나면 포커스와 문구만 정리한다. 전투는 그동안에도 계속 진행된다.
...
- B3·B4가 완료되고 B5 안내가 정상적으로 열린 경우에만 `firstRunTutorialDone`을 기록한다.

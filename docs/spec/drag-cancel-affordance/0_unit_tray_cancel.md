...
- [x] Play — 유닛을 집어 트레이 위로 되돌리면 취소 예고가 뜨고, 놓으면 **코스트가 줄지 않는다**
- [x] Play (rev3) — 트레이 쪽으로 끌면 **덮는 UI 없이** 프리뷰 고스트 + 포인터 라벨만 나오는가
- [x] Play (rev3) — 코스트 물통과 슬롯 아이콘이 **끝까지 가려지지 않는가**
- [x] Play (rev2) — 집었다가 **그 자리에서 잠깐 머물면** 라벨이 뜨는가 (H2 — 가장 빠른 취소의 발견성)
- [x] Play (rev2) — 위로 빠르게 튕기는 드래그에서 라벨이 **깜빡이지 않는가** (H2 의 다른 쪽)
- [x] Play — 취소 존 안에서 보드 하이라이트·사거리·거부 라벨이 사라진다
- [x] Play — 취소 존을 나가면 하이라이트가 즉시 복구된다
- [x] Play — 큰 맵(Serpent/Twin/Spiral) **최하단 행에 여전히 배치된다** (도달성 회귀 없음)
- [x] Play — 탭 배치 비행이 트레이 위를 지나가도 취소되지 않는다(계약 5)

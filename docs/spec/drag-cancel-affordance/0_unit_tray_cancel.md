...
- **M1** 배너가 트레이를 통째로 덮어 코스트 물통·출발 슬롯을 가린다 → 얇은 스트립 + 출발 슬롯
  코랄 링. 시각 작업이라 분리.
- **M3** 최초 발견 경로가 없다(배너는 존에 들어간 뒤 뜬다) → 첫 배치 드래그 1회 힌트
  (`UserDragStarted` 훅 기존).
...
- [ ] Play (rev2) — 배너가 켜진 **그 자리에서** 떼면 실제로 취소되는가 (H1 — 예전엔 상단 65px 이 거짓)
- [ ] Play (rev2) — 집었다가 **그 자리에서 잠깐 머물면** 배너가 뜨는가 (H2 — 가장 빠른 취소의 발견성)
- [ ] Play (rev2) — 위로 빠르게 튕기는 드래그에서 배너가 **깜빡이지 않는가** (H2 의 다른 쪽)

...
- **rev2 (UX 리뷰 반영)** — 배너를 **트리거 위치**(트레이 rect − 포인터 오프셋)에 그려 보이는 밴드와
  히트박스를 일치시켰다(H1). 예고 게이트에 dwell 0.18s 문을 추가해 "집었다 그 자리에서 놓기"=가장
  빠른 취소가 발견 가능해졌다(H2). 라벨에 무차감 명시(M2).
...
- **배너는 트레이 rect 가 아니라 트리거 위치에 그린다.** 판정이 가상 포인터라 rect 를 그대로 그리면
  보이는 밴드와 히트박스가 오프셋만큼 어긋난다 — "배너가 켜졌는데 떼니 꽂혔다" 의 정확한 원인.

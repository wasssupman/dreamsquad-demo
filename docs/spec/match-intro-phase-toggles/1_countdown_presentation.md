...
**아웃트로** — `0` 은 표시하지 않는다. 남은 시간이 0에 닿으면 라벨을 `GO!` 로 바꾸며 같은 펀치를 한 번 더 치고, **기다리지 않고** `FinishPlacement()` 를 호출한다(계약 4). 패널 은닉만 `HideOverlay()` 로 위임해 자동 시작 모드에서는 GO! 트윈(~0.35초 스케일업 + 페이드)이 끝난 뒤 꺼진다. **입력 차단막은 `FinishPlacement()` 시점에 즉시 해제**한다 — 전투는 이미 시작됐고 GO! 는 잔상일 뿐이다.

...
6. **튜토리얼이 배치 창을 잡으면 자동 시작은 기다린다.** 두 겹이다. ① **첫 판**(`TutorialProgress.ShouldRunCore`)은 플래그 자체를 무시하고 현행 30초 배치로 간다 — 배치를 직접 가르치는 판이라 3초와 겹치면 온보딩이 통째로 깨진다(기믹 리빌이 첫 판만 건너뛰는 것과 같은 예외). ② **그 밖의 튜토리얼 게이트**는 카운트다운을 **멈춘다** — `ShouldRunCore` 는 첫 판만 참인데 튜토리얼 게이트를 쓰는 경로가 하나 더 있다(효과 타일 안내는 **두 번째 판 이후**). 멈추지 않으면 안내가 3초에 잘린 채 완료로 저장돼 플레이어가 영영 못 읽는다. 판정은 30초 경로와 **같은 술어**(`PlacementPhasePolicy.CanFinish`)를 쓴다. 이때도 배치 입력은 막힌 채다 — 안내 캔버스(1500)가 차단막(7) 위라 탭 진행만 살아 있다.

...
>
> 아트 기준 후속 변경: 방어 유닛 포트레이트의 현재 source는
> `docs/spec/defender-spine-portraits/`와 `Art/DefenderPortraits/spine/`이다.
...
게임의 밝고 읽기 쉬운 캐주얼 디펜스 아트 톤에 맞게 보이는가?"*
...
- 현재 아트 톤 기준은 Spine 방어 유닛 포트레이트다. 작은 크기에서 읽히는 단순한 실루엣,
  밝은 팔레트, 깨끗한 외곽선과 모바일 대비를 우선한다.
...
- **StartButton = 배경 이미지 + TMP 라벨.** Codex 생성 배경 그래픽을 버튼 Image 로 쓰고
  ...
  절차 플레이트로 폴백.
- **UI 아트는 Codex 로 생성**한다. 이 spec의 기존 생성물은 완료 당시 브리프를 보존하며,
  후속 생성은 현재 Spine 포트레이트의 캐주얼 디펜스 톤을 기준으로 한다. 생성물은 투명 배경
  PNG, `Assets/_Project/Art/UI/` 하위에 둔다(`UI/Cost/`, `UI/Buttons/`).

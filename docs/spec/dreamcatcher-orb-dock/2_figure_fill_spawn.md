...
**절차적 최종 확정**(2026-07-23 사용자 결정, 개정): 20단계 granularity 를 위한 피규어 지름
(~24px = 1080p 실기기 ~1.4mm)에서 Spine 스켈레톤은 특정 유닛으로 **읽히지 않아**(절차적 원형과
시각 동일) 20개 SkeletonGraphic 의 비용·복잡도를 정당화하지 못한다. README 가 이미 피규어를
"fill 질감, 개수 판독 아님"으로 강등한 것과 종합해 **절차적 피규어를 최종안으로 확정**한다.
- **2a (본 단위)**: 절차적 피규어 스프라이트로 풀·물리·게이지 구동. **이것이 최종.**
- **2b (후속 후보, 보류)**: Spine 스킨은 "적게·크게(maxFigures 6~8, ~44px)"로 재설계해야만
  식별 가치가 생기며 게이지 granularity 를 희생한다. 필요 시 별도 판단.

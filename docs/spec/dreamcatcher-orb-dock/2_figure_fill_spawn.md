...
**단계 이력**:
- **2a**: 절차적 원 피규어로 풀·물리·게이지 구동(초기). 원은 자리표시자였음.
- **2b (완료 2026-07-23, 사용자 재결정)**: "적게·크게 + 실유닛 Spine 미니어처". 원이
  "그냥 동그란 원"이라 재개. **maxFigures 8·radius 18·scale 0.34(~36px)** 로 축소·확대해
  SkeletonGraphic 이 읽히게. 게이지 granularity 는 8단계로 거칠어짐(피규어당 12.5). 포즈=Idle
  (서 있는 캐릭터가 통통 튕김과 어울림; Die 도 knob). 대표=Enemy_Boss_Nightmare(적 정체성
  이벤트 미제공 → 대표 스켈레톤). 미배선 시 절차적 원 폴백(무회귀).
- **추가(사용자 요청)**: 항아리 안 피규어가 **주기적으로 통통 튕김**(`JarFigurePile.JostleAll`,
  주기 상방 임펄스). 설계 리뷰의 "상시 움직임 산만" 우려보다 사용자 요청 우선.
- **후속**: 디펜더별 정확 스킨(DefenderDied 데이터 스레딩), 흡수 비행 고스트도 미니어처로.

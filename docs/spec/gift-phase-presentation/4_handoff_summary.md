...
- `3a9e20f4`·`c87f8060` unit 5 — 탭 스킵 2단(리빌 포커스 착지 → 배치행)
...
- 탭 스킵 2단(unit 5): 시퀀스를 전반부(~플립)/후반부(`PlayFromRevealFocus`: 홀드~흡수)로 분리 — 자연 진행과 스킵 착지가 후반부를 공유. 리빌 전 탭 = 리빌 포커스 직접 세팅 후 후반부 시작, 리빌 이후 탭 = `StopSequence`+배치. 착지마다 grace(`tapSkipGraceSec` 0.35s) 리셋 — 연타가 카드 확인까지 날리는 것 방지(의도, 되돌리지 말 것).

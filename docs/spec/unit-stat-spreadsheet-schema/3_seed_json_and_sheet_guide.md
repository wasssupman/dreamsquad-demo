...
- [ ] `3_seed_unit_stats.json` 이 25 유닛 전수를 담고, 값이 asset 원본과 일치 (2026-07-06 세션에서 enum 매핑 8종 전수 + 유닛 표본 교차 검증 완료)
- [ ] 사용자가 시드를 시트에 입력 완료 → unit 4 의 실 API 왕복 검증으로 최종 확인 (프리뷰/로그상 변경 0건이면 입력 무결)

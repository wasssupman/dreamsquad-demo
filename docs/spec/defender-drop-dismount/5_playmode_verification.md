...
2. **활성화 타이밍**: 커밋 시각 기록 → `PendingDeployment` 해제 시각이 `deploymentDuration ± 2프레임`(unscaled 대기 중 Battle 시계 주의 — 기존 `WaitUntilActivated` 패턴 재사용). 착지 시점(오버라이드 clear)이 활성화보다 늦지 않음(클램프 동작).

...
- `RunDeployment(unitData, cell, entity, presentAtLanding: bool)` 로 확장:
  ...
  - 착지 신호 전달은 단순하게: dismount 코루틴이 완료 시 콜백(Action) 호출. dismount 가 abandon 되면(binding 붕괴) 콜백 미발화 — 이 경우 `RunDeployment` 가 duration 만료 시점에 미발화 감지 후 스킵(엔티티가 이미 없거나 맵 teardown).
...
- facing 유닛(계약 8): aim 분기는 `RunDeployment` 를 타지 않는 현행 유지. 착지 훅은 dismount 코루틴이 직접 `PulsePlacementHover` + `PlayDeploy`(spineUnitPool 조회) 호출 — aim 확정 후 활성화 흐름은 무변경.

...
`PropData` Inspector 에 `Generate Billboard Prefab` 버튼을 추가하고, Sprite/Spine 중 하나를 자동 해석해 root + Visual 구조의 prefab 을 생성/갱신한다. Theme 경로의 최종 계약에서는 `Data/Theme/{themeName}` 의 SO 와 `Art/Theme/{themeName}` 의 동명 PNG 를 자동 매칭한다.
...
3. prefab 출력 폴더 생성
   - Theme 경로: `Assets/_Project/Prefabs/Props/{themeName}`
   - Prototype 호환: `Assets/_Project/Prefabs/Props`
...
9. `PrefabUtility.SaveAsPrefabAsset(root, outputPath)`
...
### Theme Image Matching

`PropData` 가 아래 경로에 있으면:

```text
Assets/_Project/Data/Theme/{themeName}/{PropData.name}.asset
```

generator 는 먼저 아래 이미지를 찾는다.

```text
Assets/_Project/Art/Theme/{themeName}/{PropData.name}.png
```

이 파일이 있으면 `sourceTexture` 로 취급하고 Sprite import 로 보정한다.
...
- Theme image matching 실패 시 prototype 호환 fallback 으로만 사용한다.

## 계약

- Generator 는 **idempotent**: 같은 이름 prefab 은 overwrite. prefab 내부 수동 수정은 초기 구현에서 보존하지 않는다.
...
- Generator 는 prefab 생성만 담당한다. 맵 타일 배치 여부는 `BackgroundPropPlacer` 책임이다.

## 완료 기준

- Sprite sibling PNG 만 있는 PropData 에서 버튼 1회 클릭으로 prefab + sprite import 동시 처리
- Theme Art PNG 만 있는 PropData 에서 버튼 1회 클릭으로 prefab + sprite import 동시 처리

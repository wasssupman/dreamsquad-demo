...
골드/네이비 스타일의 작은 배지를 추가하고 `누수 현재값 / 실제 최대 제한`을 표시한다.

검증 질문: *"플레이어가 전투 중 점수와 함께 현재 누수량과 패배 한계를 즉시 읽고, 다음 누수가
얼마나 위험한지 이해할 수 있는가?"*
...
| 1 | `1_score_companion_badge.md` | HUD 구현 | 점수 아래 `누수 current / limit` 배지와 위험 상태 표현 |
...
- 정보 구조는 한 줄 `누수 {current} / {limit}`이며, 점수 배지 바로 아래 우측 정렬로 붙인다.

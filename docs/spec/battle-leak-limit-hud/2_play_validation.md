...
누수 HUD가 실제 Battle 프레임의 점수 아래에서 읽히고, BattleBridge 상태 변화와 페이즈 수명주기를
정확히 반영하는지 확인한다.
...
- 1920×1080 Battle에서 점수 배지와 누수 배지의 정렬·간격·safe area를 캡처한다.
...
- 점수 아래 `누수 current / limit`이 겹침 없이 읽히고 점수보다 작은 위계를 유지한다.

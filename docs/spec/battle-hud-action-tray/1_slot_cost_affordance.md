...
- [ ] 1~5 비용 유닛이 실제 데이터와 같은 숫자를 표시.
- [ ] 현재 cost 경계를 넘나들 때 해당 슬롯만 즉시 available/unavailable 전환.
- [ ] 색을 구분하지 못해도 비용 숫자와 glyph로 상태 판독 가능.
- [ ] 포이즌/파이어/아이스 캐스터 연속 배치에서 이름 겹침·잘림 없음.
- [ ] 7슬롯 재빌드/드래프트 재진입 시 캐시 누수·stale 상태 없음.

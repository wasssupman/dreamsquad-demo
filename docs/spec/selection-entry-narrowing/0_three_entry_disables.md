...
- [ ] Play: 항아리를 눌러도 손패가 열리지 않는다. 손패가 열린 상태에서 항아리 위를 누르면 닫힌다.
- [ ] Play: 판 위 유닛을 탭해도 선택되지 않는다(손패가 열려 있었으면 닫힌다).
- [ ] Play: 하단 트레이 셀(배치 완료된 유닛)을 탭하면 그 유닛이 선택되고 손패가 뜬다.
- [ ] Play: 선택해도 카메라가 당겨지지 않는다. 방향 지정 배치의 셀 포커스는 **그대로 동작**한다.
- [ ] Play: 선택 중 부착(탭·드래그)과 액티브 카드 드래그가 종전대로 동작한다.

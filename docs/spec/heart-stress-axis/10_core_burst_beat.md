...
- [ ] 마음이 0 이 되면 그 자리에 붕괴 VFX 가 뜨고 프랍이 그을리며 주저앉는다
- [ ] 그 박자가 눈에 보인 **뒤에** 결과 화면이 뜬다
- [ ] 박자 동안 화면이 느려진다 · 끝나면 시간 배율이 원복된다(리스 누수 없음)
- [ ] 3분 만료·제출은 지연 없이 현행대로 결과 화면
- [ ] 결과 리본은 여전히 `"결과"` — 종료 사유를 말하지 않는다(의도)
- [ ] 서버 제출이 화면보다 먼저 나간다(순서 계약 유지)
- [ ] EditMode 신규 실패 0건

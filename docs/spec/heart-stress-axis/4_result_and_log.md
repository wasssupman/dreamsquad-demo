# 4 — 결과 화면·로그 어휘 정합

## 목적

**인게임을 스트레스 어휘로 바꾼 뒤 결과 화면만 옛 어휘로 남지 않게 한다.** 그리고 새 종료
라벨(`stress_full`)이 로그·기록에 정확히 남게 한다. **승/패는 여전히 만들지 않는다**(명제 3).

## 변경 대상

- `Assets/_Project/Scripts/Bridge/BattleBridge.cs` — `EndMatch("stress_full")` 로그 문구
- `Assets/_Project/Scripts/UI/ResultScreen.cs` — 「남은 마음」 줄(`:139`)
- `Assets/_Project/Scripts/Core/MatchTally.cs` — `Outcome` 주석(값 목록 3개로)

## 구현

**1. `Outcome` 은 라벨이지 판정이 아니다.** `complete` / `submitted` 옆에 `stress_full` 이
붙는다. ⚠ **라벨이 셋이라고 게임 종료가 셋인 것은 아니다** — 게임 규칙상의 통로는 2개
(만료 · 스트레스 100)이고 `submitted` 는 **절차 밖 탈출구**다(계약). UI·문구에서 제출을
「게임을 끝내는 방법」으로 승격시키지 않는다. `MatchTally` 에 승패 필드를 **만들지 않는다** — three-minute-kill-race 가 *"자리를 남기면
조용히 되살아난다"* 며 `Won` 을 은퇴시킨 이유가 그대로 유효하다. 주석의 「값은 둘」 서술을 셋으로 고친다.

**2. ⚠ 결과 화면의 「남은 마음」 줄.** 지금 `StatRow.State("남은 마음", StabilityText(Stability, StabilityMax))`
로 `650 / 1000` 같은 HP 를 그린다. 인게임이 스트레스(차오름)로 바뀌면 **어휘가 갈리고**,
`stress_full` 종료에서는 「남은 마음 0 / 1000」이 뜬다. 셋 중 하나로 정한다:
- **A(권장)**: 같은 줄을 **「스트레스」**로 바꾸고 `StressMath` 로 `0~100` 표기 — 인게임과 한 어휘
- B: 줄을 뺀다 — 점수(처치 수)만 남는다
- C: 그대로 둔다 — 어휘 분열을 수용

**3. `MatchTally.Leaks` 는 뜻이 바뀌었지 죽지 않았다** (unit 0 rev 3). 구 의미(«부서진 마음으로
적이 흘러듦»)는 구조적으로 발생 불가지만, 그 자리를 **돌격형 도달**이 물려받았다 —
「빠르게 달려와 마음을 치고 산화한 적 수」. 이 판에서 «놓쳤다» 로 셀 수 있는 유일한 사건이다
(공성형은 마음 앞에서 아직 잡을 수 있으므로 놓친 것이 아니다).

⇒ 결과 화면·로그에 이 값을 **쓸 수 있다**. 다만 라벨이 「유출」이면 거짓말이다 —
「직격당함」·「놓친 돌격」 계열 어휘로 맞춘다. 로그 스키마 키는 유지한다(서버·히스토리 소비처).

**4. 서버 제출값은 무변경.** `SubmissionScore => Kills`. 종료 경로가 셋이 되어도 **값을 만드는
곳은 하나**라는 계약(three-minute-kill-race)은 그대로 승계한다.

## 완료 기준

- [x] 컴파일 0 에러 · 콘솔 에러 0
- [x] EditMode 전체 완주, 신규 실패 0건
- [x] Play: 스트레스 100 종료 → 결과 화면에 **승/패 표기가 없다**
- [x] Play: 결과 화면 총점 = 그때까지 처치 수 (3분 완주·유저 제출과 같은 값 경로)
- [x] Play: 「남은 마음」 자리가 인게임과 **같은 어휘**다(스트레스)
- [x] 로그에 `outcome=stress_full` 이 기록된다
- [x] 서버 complete 에 실리는 값이 화면 숫자와 같다

**확인 2026-08-24** — 커밋 `f6c62e87`. 사용자 Play 확인 통과.
⚠ **결과 리본은 `"결과"` 고정이고 종료 사유를 말하지 않는다 — 의도된 현행**(사용자 결정
2026-08-24). 3분 만료와 파열이 화면상 같아 보이는 것은 버그가 아니다. unit 10 참조.
서버 제출은 `EndMatch` 안에서 **화면보다 먼저** 나간다(unit 10 이 화면만 늦추면서도 이 순서를 지켰다).

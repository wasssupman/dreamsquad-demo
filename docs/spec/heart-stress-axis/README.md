...
> · **unit 12 작성됨 2026-08-25** (마음 N개 공유 체력) — 착수 전. 확정 명제 10 을 뒤집는다.
> 잔여: **소리**(에셋 생성 필요 — spec 밖으로 미룸). 육안 확인은 units 9·10 까지 통과.
> **main 머지 완료 2026-08-24**(`410a2c9c`). units 0~10 은 `main` 에 있다 — 별도 브랜치를 만들지 않는다.
>
...
| 10 | ~~**마음은 무조건 1개**임을 깔고 진행한다~~ → **unit 12 에서 열림**(사용자 결정 2026-08-25). 마음은 **N개일 수 있고 체력을 공유**한다 — 어느 마음이 맞든 같은 저수지가 줄고 모든 마음이 같은 상태로 보인다. **정본이 `Health` 하나**라는 아래 핵심 구조는 그대로이며, 오히려 «어느 마음의 Health 냐»라는 모호함이 사라진다 |
...
- **마음 개수는 저작 사실이다 — 코드 불변식이 아니다.** units 0~11 은 「1개」를 전제로 만들어졌고
  (라이브 `MapDocument` 15종 전부 `goals` 길이 1, 2026-08-23 전수 확인), `OnValidate` 는 `goals < 1`
  만 잡고 `> 1` 하드 가드는 두지 않았다(`map-rework` 계약 3 = "멀티골 기계는 건드리지 않는다" —
  하드 에러를 세우면 제약 9 스코프 위반). 대신 런타임 경고 1줄로 저작 사고를 표면화했다.
  ⚠ **마음 1개에서는 「첫 붕괴」와 「마지막 붕괴」가 관측 불가능하게 동일**하다 — 계획된 테스트도
  Play 확인도 둘 다 통과하므로, unit 0 이 **2타워 픽스처**로 코드에 고정한다.
  → **[unit 12](12_shared_heart_pool.md) 가 이 전제를 연다**(사용자 결정 2026-08-25): 마음은 N개일
  수 있고 **체력을 공유**한다. unit 12 착수 시 그 런타임 경고를 제거하고, 「첫 붕괴 = 종료」 계약은
  그대로 유지한다 — 공유 풀에서는 모든 마음이 동시에 0 이 되므로 첫/마지막 구분이 소멸한다.
- **회복은 공유 풀에 1회 적용한다(마음 개수와 무관).** unit 11 까지는 「살아있는 마음 **전체**에 같은
  양」이었다 — 마음마다 독립 체력이던 시절의 규칙이고, 「킬 위치 최근접 하나」로 두면 2골 사고 시
  만피 마음이 회복을 흡수해 clamp 로 소멸시키기 때문이었다. **공유 풀에서는 그대로 두면 킬 1회가
  마음 수만큼 회복된다** — unit 12 가 진입점을 풀 1회 적용으로 바꾼다.

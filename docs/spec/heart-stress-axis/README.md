...
| 9 | **돌격형(`Runner`·`Swift`)은 마음에 피해를 주지 않는다.** 마음 근처까지 이동하고 끝 — `stabilityDamage` 소비를 끊는다 |
...
피해 : 공성하는 적의 **공격력만큼**만               ← 자폭 피해 축은 은퇴 (명제 9)
...
| 돌격형(`Runner`·`Swift`) | 마음 피해 **0** — 도달 후 소멸만 | 명제 9 (축 은퇴) |
...
- **게임이 끝나는 통로는 2개다**: 3분 만료 · 스트레스 100. **메뉴를 통한 제출은 허용하되 공식
  ...
  막는 장치는 그대로 필요하다).
- **마음에 피해를 주는 것은 공격력뿐이다.** 공성하는 적이 `outputs[].magnitude` 로 때리는 것이
  **유일한 경로**다. 돌격형(`Runner`·`Swift`)은 공격 수단이 없으므로(`attackMethod: None`)
  **마음에 아무 피해도 주지 않고** 도달 후 소멸한다 — 그들의 페널티는 기회비용
  (못 잡으면 점수도 각성치도 못 번다) 하나다. `stabilityDamage` 필드와 `EnqueueGoalTowerDamage`
...
- **돌격형에게 마음 위협을 되돌릴지** [S] — 명제 9 로 자폭 피해 축을 끊었다. `Runner`·`Swift` 는
  이제 마음에 **완전히 무해**하고 남는 것은 기회비용뿐이다. 플레이에서 「막을 이유가 없는 적」으로
  읽히면 되살린다. 그때는 **`stabilityDamage` 값만** 1000급으로 재저작하는 쪽이다
  (코드 0줄 + 소비 재개 1줄).
  ⚠ **「진짜 공격력을 줘서 공성시키는」 길은 함정이다** — 그 둘에게 `AttackState` 를 주면
  `canSiege=true` 가 되어 골에서 안 죽고 「필드에 적 0기」 판정을 막아 **웨이브가 안 넘어간다**
  (`battle-structures` unit 0 이 회귀로 규정해 제거한 이력).

...
- [x] Play(Isle/Ford/Duel): 본능이 살아 있으면 적이 **본능으로 걸어가** 그것을 팬다
- [x] Play: 마지막 본능이 무너지면 그때부터 마음이 깎이고 스트레스가 오른다
- [x] Play: 스폰 예고선이 **본능으로 가는 길**을 그린다
- [x] Play(Coil/Serpent 등): 방패가 한 번도 안 서고 현행과 같다

**확인 2026-08-24** — 커밋 `91122326`. 사용자 Play 확인 + 하네스 실측.
Duel 실측이 인과를 못박았다: **방패해제 172.2초 → 첫 스트레스 172.5초.** 그 전까지 마음은
표적조차 되지 않는다. 콘솔 `마음 방패 ON — 살아있는 방어 본능 2기`.
무방패 맵(Serpent·Zig)은 6~19초에 스트레스가 오르기 시작한다 = 방패가 안 서는 것도 확인.

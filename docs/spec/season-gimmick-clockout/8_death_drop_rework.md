...
- Play(ClockOut 기믹): 배치 유닛이 **전투 중 사망**하면 그 타일에 사직서(흰 종이)가 남는다. 10초 강제 퇴근은 더 이상 없다(가만히 둔 유닛은 사라지지 않음). 사직서 5장 → 메테오 순차 낙하(적만)는 그대로. 코스트 환급 없음.
- 메테오 발수 강화(`meteorCount` 3 → 10): 트리거 시 Walk 타일 여러 곳에 10발이 stagger(0.4s) 간격으로 순차 낙하(SO 값만 변경, cast 로직 불변 — `min(meteorCount, Walk타일수)` 로 안전 캡).

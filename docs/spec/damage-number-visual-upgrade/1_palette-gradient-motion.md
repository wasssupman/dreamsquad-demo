...
- **검증 2026-07-07**: compile 에러 0 · 팔레트 램프 라이브 확인(t0=청록 35E0D0 / t1=오렌지 FF6A2A). `EnsureDefaults` 게이트 버그 발견·수정 — 씬 gradient 가 `new Gradient()` 의 2키 흰→흰(=단조로움 원인)이라 구 `Length==0` 게이트가 영원히 미발동했음. pristine 흰→흰만 리셋하도록 교정(실제 튜닝 보존). 위치/색/모션/정지 육안은 Play 실시간 튜닝(값 직렬화).

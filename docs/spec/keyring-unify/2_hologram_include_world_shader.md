...
확인 2026-07-08 — compile·콘솔 클린. A/B diff 픽셀 0(최대 채널 오차 2 = fixed→float 허용
예외 수준), 레거시 사본 삭제. 월드 렌더 검증에서 **샘플 uv swap 누락 버그를 실제로 적발·수정**
(효과 축만 전치하고 텍스처 샘플을 안 바꾸면 빔이 어둡게 뭉개짐 — 이식 가이드 함정 목록 실증).
washout 판정: 중간톤(보드 근사) 선명·밝은 톤 파스텔화 — 가산 블렌드 유지, _Intensity 여유로
대응, 실보드 최종 판정은 unit 3. 리뷰 (none).

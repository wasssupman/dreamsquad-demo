...
- [ ] compile 클린.
- [ ] `ShieldMathTests` 그린: 같은 출처 max(잔량>B / 잔량<B) / 다른 출처 합산(100+100=200) / 부분 흡수(오래된 슬롯 우선) / 슬롯 경계 걸친 흡수 + 소진 슬롯 제거 / 완전 흡수(관통 0) / 슬롯 없음(전량 관통) / 과잉 데미지.
- [ ] 기존 EditMode 전체 그린 (버퍼 미부여 경로 무회귀).

...
**상태**: 계획 — 2026-05-09 brainstorming 완료, 작업 단위 0~6 정의됨
**구현 주체**: Codex (이미지 생성 + 코드/SO 채움). Claude 는 spec 만 작성하고 인계.

## 목표

토너먼트 시즌마다 맵 외곽을 풍성하게 채우는 백드롭 시스템을 도입한다. 시즌은 `MapTheme + Backdrop 1종` 을 묶어 활성화하고, 외곽은 ① 카메라 정면 일러스트 백드롭 quad ② 보드 둘레의 EdgeProp 8개로 구성된다. 본 spec 의 산출은 Forest 시즌 1개분.
...
| 6 | [6_play_verification.md](6_play_verification.md) | Play 진입 후 백드롭/EdgeProp 시각 검증 + 스크린샷 |
| 7 | 7_handoff_summary.md (구현 후 작성) | 인계 지도 |

## Feature-Wide 계약

- **Season → MapTheme + Backdrop 1종** 구조 (시즌 풀 X, 시즌당 정확히 1종).
- 시즌 SO 1개에 `MapThemeData` 와 `SeasonBackdropData` 가 함께 묶인다. `SeasonRegistry.activeSeason` 1개만 런타임에서 사용.
- 외곽은 두 레이어: ① 카메라 forward 거리 D 의 quad backdrop ② 보드 둘레 12 anchor 중 8개에 EdgeProp 인스턴스.
...
- 화면 비율 추종: backdrop quad 의 widthWorld 는 `heightWorld * camera.aspect`. 카메라 framing 변경 시 재마운트로 갱신.

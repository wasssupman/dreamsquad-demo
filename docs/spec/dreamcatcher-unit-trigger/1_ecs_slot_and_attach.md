...
- 가드: `binding != Unit` / entity 미존재 / mechanics 비어 있음 → false.
...
- [ ] 기존 투사체 경로 무회귀: 기존 PlayMode smoke 통과 (원거리 유닛 발사→히트 정상)
- [ ] execute_code 로 배치 유닛에 부착 → `DcTriggerSlot` 버퍼 길이/필드 값 reflection 확인 (같은 카드 2회 부착 = 슬롯 2개, instanceId 상이)

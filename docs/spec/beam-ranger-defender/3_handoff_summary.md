...
- **끝점 해석은 뷰 풀만 보면 안 된다.** `BattleBridge.ResolveBeamViewPos` 가 cast anchor → 유닛 view → **ECS 위치** 3단으로 폴백한다. 뷰 풀만 보면 풀에 없는 적이 끝점일 때 세션이 즉사한다(실제로 배치 빔 전멸의 원인).
- **조사(DotNearby) 중에는 기본 공격을 하지 않는다.** 배치 시 `AttackState.cooldownRemaining` 을 `onPlaceDuration` 만큼 밀어 둔다. 지속을 갖는 **채널형** on-place 효과라서이며, 순간 효과(MeleeBurst·StunNearby 등)에는 적용하지 않는다 — 그래서 억제 코드가 DotNearby 분기 안에 있다.
...
3. **정렬(sortingOrder) — 두 제보의 실제 원인.** 벤더 프리팹은 렌더러가 `sortingOrder 0~2` 로
   들어온다. 그대로 두면 같은 "Default" 레이어에서 **유닛(보드 행 기준 수백대) 뒤에 깔린다**.
   바닥(−20/−10)보다는 위라서 **빈 땅 위 구간만 보이고 유닛에 가린 구간은 사라진다** = "끊겨 보인다".
   적이 여럿이면 빔 대부분이 그 적 자신에게 가려 = "1개만 나온다". 두 증상이 한 뿌리였다.
   `BoardSortOrder.BeamOrder(15000)` 신설 — 투사체(+1000) 위, 피격바(16000) 아래. 프리팹 내부의
   상대 순서(0/1/2)는 **더해서 보존**하고, 풀 재사용분에 두 번 더하지 않도록 신규 인스턴스에만 적용한다.
   ⚠ 벤더 VFX 를 가져올 때는 항상 정렬 대역을 확인할 것 — 이 프로젝트의 유닛은 수백대를 쓴다.

4. **롤(축 회전) 미고정 — "가로세로 직선으로만 보인다".** `forward` 만 맞추면 Unity 가 world-up
   으로 롤을 임의 결정한다. 이 보드는 `BoardSpace.ToView` 가 sim XZ 를 그리드 셀로 보내는
   **XY 평면 정면 뷰**라 빔 방향이 화면 평면 안에 놓이고, 옆에서 보도록 저작된 빔 메시를
   축 방향에서 보게 된다. `Quaternion.LookRotation(fwd, -Camera.main.transform.forward)` 로
   빔의 면이 카메라를 향하게 고정한다.
   같은 오해로 넣었던 `endpoint.z = sourceView.z` 평탄화는 **제거**했다 — `ToView` 결과는 이미
   한 평면 위라 무의미했고, 방향 성분만 깎을 위험이 있었다.

5. **끝점 해석이 `spineUnitPool` 에만 의존했다** (2차 제보: "배치 스킬 빔이 1개만 나온다").
   배치 스킬은 대상 수만큼 세션을 **정상적으로 연다**(실측: frame 0 에 활성 3개). 그런데 다음
   프레임에 끝점을 못 찾아 전부 닫혔다 — 풀에 없는 적(폴백 뷰·워밍업 중)이 끝점이면 즉사한다.
   배치 빔은 한 번만 열리므로 그대로 전멸했고, 0.2초마다 다시 열리는 **공격 빔 하나만** 남아
   "1개만 나온다"로 보였다. 해석기를 브리지가 주입하는 형태로 바꾸고
   (`BeamPresenter.ViewPosResolver`), **cast anchor → 유닛 view → ECS 위치** 3단 폴백으로 고쳤다.
   프레젠터가 ECS 를 직접 못 보므로(제약 1) 주입이 필요했다.

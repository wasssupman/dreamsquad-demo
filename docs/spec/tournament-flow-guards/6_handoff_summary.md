...
> unit 7 (2026-07-27, `6ce13433`): 락 유형 play 실패 시 pending attemptId 로 complete(0) → play 1회 재시도 자동 복구(tracked 한정, orphan 은 여전히 서버 이관) + 실패 팝업 락 메시지 분기·raw 에러 상세 표기. `ReconcilePending(onDone)` 오버로드 — released=true(complete 실제 성공)에만 재시도 게이트. 라이브 강제 재현 통과(7_ 문서 확인 라인).
> unit 8 (2026-07-27, `cf119dfe`): 결함 A 해소 — `BeginMatch()` adopt-or-reset, 비게이트 진입(TestMode/에디터 직접 Play)은 attempt 무발행(발행 창구=`BeginMatchFromLobby` 유일). 라이브: TestMode 진입 play 미발행 + 로비 경로 adopt 무회귀 확인.
...
- `6ce13433` unit 7 — 락 자동 복구(tracked) + 입장 실패 상세 에러
- `cf119dfe` unit 8 — 비게이트 진입은 attempt 를 만들지 않는다 (결함 A)

...
상태: **완료 2026-08-15 — units 0~3 + 연출 rev 5 + 코드리뷰 반영 · unit 5(이탈 쿨타임 분리) 추가.**
...
**unit 5(2026-08-15, 사용자 결정)로 아래 「열린 밸런스 항목」이 닫혔다** — 퇴근과 사망이 각자의
재배치 대기를 갖고 퇴근이 더 짧다. 상세 [`5_exit_cooldown_split.md`](5_exit_cooldown_split.md).
값 저작은 이제 **시트**(`deathCooldown` · `retireCooldownRatio` 열)가 소유한다.
...
7. ~~**쿨타임 값 = 기존 `DefenderUnitData.placementCooldown` 재사용.**~~
   > **rev — unit 5 (2026-08-15, 사용자 결정).** 재사용을 폐기했다. 퇴근과 사망이 **각자의 이탈
   > 쿨타임**을 갖고 퇴근이 더 짧다(`deathCooldown` + `retireCooldownRatio`, 둘 다 시트 저작).
   > `placementCooldown` 은 "방금 놓았다"가 거는 연사 게이트로 남아 이제 **다른 축**이다.
   > 재사용이 만든 실제 상태는 *퇴근 4초 / 사망 0초* — 즉 **방치가 항상 이득**인 인버전이었고,
   > 값 저작 없이 코드 재사용만으로 라이브에 들어가 있었다. 아래 원문은 이력으로 남긴다.
...
| 5 | 밸런스 축 | `5_exit_cooldown_split.md` | 이탈 쿨타임 분리 (퇴근 < 사망) + 시트 저작 |
...
## ✅ 닫힌 밸런스 항목 — 이탈 쿨타임 인버전 (unit 5)

> 원문: "**사망에는 쿨타임이 없고 퇴근에는 있다.** `placementCooldown` 을 크게 잡으면 *퇴근시키느니
> 죽게 두는 게 빠르다* 가 성립한다. 지금 판단은 그대로 두고 값으로 조절이다 (…) 실플레이에서
> '방치가 최적'으로 굳으면 **사망에도 같은 쿨타임을 태우는 rev** 로 간다."

**실측을 기다릴 것이 아니었다.** 이 항목은 "값을 크게 잡으면 성립할 수 있다"고 썼지만, 라이브
에셋 26종이 이미 `placementCooldown: 4` 를 들고 있었다 — 즉 **인버전은 가정이 아니라 이미
켜져 있는 상태**였다(퇴근 4초 / 사망 0초). 스펙이 그것을 못 본 이유는 값을 저작한 spec
(`defender-placement-cooldown`)과 값을 소비하기 시작한 spec(clock-out unit 2)이 달라서다.

> **교훈**: "값으로 조절한다"고 미룰 때는 **지금 그 값이 뭔지 먼저 읽는다.** 미래형으로 적어 둔
> 위험이 이미 현재형인지 아닌지는 에셋 한 줄 grep 이면 갈린다.

unit 5 가 두 출구에 각자의 대기를 주고 **퇴근 ≤ 사망을 비율로 구조화**해 닫았다.

## 후속 후보 (현 스코프 밖)

- **`maxOnBoard > 1` 유닛의 쿨타임 세탁** — 런타임이 유닛 타입 키로 **덮어쓰기**라, 2기 중 하나가
  죽어 사망 대기가 걸린 뒤 다른 하나를 퇴근시키면 짧은 값으로 덮인다. 현재 라이브 에셋은 전부
  `maxOnBoard = 1` 이라 관측 불가. 상한을 2 이상으로 저작하는 순간 **같이 처리할 것**
  (이탈 트리거에 `max(remaining, new)` 진입점 — 배치 경로의 full-reset 의미와 갈라야 한다).

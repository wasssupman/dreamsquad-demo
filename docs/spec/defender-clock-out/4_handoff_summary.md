...
- (rev 3 키링 회수는 **시도 후 기각·롤백** — 커밋 없음. 이유는 `3_retire_exit_flight.md` 상단)
...
- 퇴근 유닛은 쓰러지지 않고 **웅크렸다가 위로 뽑혀 나간다**(rev 2 스냅, ~0.28초). 떠난 칸에
  배치 링이 한 번 치고, 트레이 칸이 1회 튕긴다.
...
- **육안 확인 남음**: 퇴근 스냅(웅크림·뽑힘·배치 링·그림자 제거) + 트레이 칸 펄스.
- **연출 개선은 보류**(사용자: "나중에 개선한다"). ⚠ **부착·연결 계열 어휘는 재시도하지 말 것** —
  0.28초 스냅에 «관계를 보여주는» 연출이 안 들어간다는 것이 rev 3 의 결론이다. 뽑는 주체가
  필요하면 **시간을 요구하지 않는** 어휘(상승 임팩트·속도선 등)로 가야 한다.

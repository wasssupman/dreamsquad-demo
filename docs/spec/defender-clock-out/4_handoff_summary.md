...
- unit 3 **rev 4 = 확정** — "퇴근 중"(연결 → **저항 1.75s** → 뱅글뱅글 튕겨나감, ~2.4초).
  길이를 늘려 rev 3 의 제약을 없앴다. 상세·튜닝값은 `3_retire_exit_flight.md` 상단
...
- 퇴근 유닛은 쓰러지지 않는다. **키링 줄이 걸려 당기고, 유닛은 박힌 채 움찔거리며 버티다가**
  뽑혀서 **뱅글뱅글 돌며 화면 밖으로** 튕겨 나간다(rev 4, ~2.4초). 뽑히는 순간 떠난 칸에
  배치 링이 치고, 트레이 칸이 1회 튕긴다.
...
- **연출은 rev 4 로 확정.** ⚠ 길이(≈2.4초)를 줄이려면 **키링·저항을 함께 버려야** 한다 —

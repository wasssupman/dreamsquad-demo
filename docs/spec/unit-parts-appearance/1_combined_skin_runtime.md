...
- [ ] 파츠 목록 있는 유닛이 스폰/드래그 프리뷰 **양쪽에서** 조합 외형으로 렌더 — 에디터 시각 항목, unit 4 적용 시 확인
- [x] 빈 목록 유닛은 기존과 동일 (full_skins), SpineUnitView·프리뷰의 기존 중복 스킨 코드 제거됨
- [x] 없는 스킨 경로는 경고 후 생존 (스모크에서 no/such_skin 스킵 검증)
- [x] 같은 조합 재요청 시 동일 Skin 인스턴스 (캐시 히트 검증)
- [x] 배치 스모크 PASS (single=6, combined=9 실측 — skin/skin_1 6슬롯 + top_c_1 1 + helmet_c_1 2)

확인 2026-07-07 (배치 검증 기준). 발견: 본체 스킨명은 `skin/skin_1` (`_c_` 인픽스 없음) — README 규약에 예외 기재.

...
> **unit 6(2026-08-15)이 인코딩을 폐기했다.** 아래는 생값 제출 기준으로 갱신된 항목이다.

- [ ] 제출 로그 `[TournamentReporter] complete ok — score=N` 의 `N` 이 처치 점수 그대로
      (점수 47 → `47`. 안정도가 어떻든 값이 달라지지 않는다)
- [ ] **결과 화면 리더보드** · `LeaderboardList` · 히스토리 패널 셋 다 `47`
- [ ] 구 인코딩 기록은 원값(10억대)으로 뜬다 — 변환하지 않는 것이 결정이다
...
| 리더보드에 10억대 숫자 | **unit 6 이후 정상**(구 인코딩 기록의 원값). 새 기록이면 제출 지점이 아직 인코딩 중 |

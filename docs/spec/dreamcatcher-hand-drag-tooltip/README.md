...
상태: 완료 2026-07-14 (units 0~1 커밋 · 사용자 Play 확인)
...
- **위치** (rev 1, 사용자 결정 2026-07-14): 선택(드래그 시작) 카드의 **우측**에 붙는다.
  우측이 safe area 를 넘으면 좌측 플립. 드래그 중 손가락을 따라다니지는 않지만,
  카드 idle bob 문법의 **플로팅 둥실거림**을 가진다. 이웃 카드 위로 렌더.

# unit 9 — TileAoe 피해풀 진영 대칭

## 목적

**TileAoe 계열 광역 피해(메테오)가 적 거점에 먹게 한다.** 승리 조건에 필수는 아니다 — unit 8 의 근접·호밍만으로도 적 마음을 깎을 수 있다. 그래도 하는 이유는 빼면 **조용한 무효**가 남기 때문이다: 메테오가 적 마음 위에 정확히 떨어져도 0 데미지. 플레이어는 그것을 버그로 읽는다. `SingleSplash`·bounce·경로 스윕은 이 unit 의 범위 밖이며 기존 적 유닛 풀을 유지한다.
...
- `Assets/_Project/Scripts/Battle/Combat/Projectile/ProjectileHitSystem.cs` — TileAoe 용 victim 스냅샷 + 진영 비트 스윕

## 구현

**TileAoe 피해자 스냅샷 + 진영 비트 필터.**

ECS 쿼리는 `FactionTag` 의 **값**으로 필터할 수 없다(shared component 가 아니다). 그리고 `StructureTag` 는 양 진영 공용이라 태그 조합만으로는 «적 거점» 을 뽑을 수 없다. 그래서 TileAoe 피해 후보를 한 벌로 모은 뒤 값으로 진영을 가른다. 기존 `aoeEntities` 는 `SingleSplash`·bounce 의 유닛 전용 동작을 보존하기 위해 남는다.
...
- TileAoe 스윕의 추가 비용은 이미 거리 검사가 도는 루프 안 비트 검사 한 줄이다. 대신 legacy 유닛 전용 풀과 TileAoe victim 풀이 공존하므로 이 변경을 «배열 할당 감소»로 보지는 않는다.
- **TileAoe 의 `GoalTowerTag` 특례가 은퇴한다** — 방어 마음은 `StructureTag` + `DefenderCore` 비트로 걸린다. 미래의 방어 본능도 코드 변경 0으로 걸린다(현행은 못 걸린다).
...
- 컴파일 0
- EditMode 신설 3케이스:
  - 방어 TileAoe(`targetFaction = Enemy`)가 **적 마음·적 본능을 깎는다** (신규 동작)
  - 같은 광역이 **방어 마음을 깎지 않는다** (자기편 오폭 금지 — 통합 풀의 최대 위험)
  - 보스 TileAoe(`targetFaction = Defender`)가 **방어 마음을 깎는다** (`GoalTowerTag` 특례 은퇴 후에도 `:95` 가 고친 동작이 보존됨)

...
| 9 | 시뮬 | `9_aoe_pool_symmetry.md` | 광역 피해풀 진영 대칭(두 풀 → 1벌 + 진영 비트 필터, `GoalTowerTag` 특례 은퇴) | 있음 — 광역도 적 거점에 먹는다 |
| 10 | 판정 | `10_siege_resolution.md` | 공성 승패 **축**(적 마음 축 신설 + 타이머 축 비교식 통합) + 공성 전용 덱 저작 | 있음 — 승패 공식 |
| 11 | 검증 | `11_siege_live_verification.md` | 공성 라이브 1개(상호 교전 + 적 마음 파괴 승리). 구 후속 «본능 발사 라이브 검증» 흡수 | 0 |
...
- ~~**본능 발사의 라이브 검증**~~ → **unit 11 에 흡수**. 미검증 사유가 «방어유닛을 배치하지 않아» 였고 unit 11 이 배치한다. unit 8 로 반대 방향도 열려 **상호 교전**이 된다.
- ~~**공성 모드 콘텐츠**~~ → **unit 10 이 승패까지 담당** (사용자 규칙: 마음 HP 0 = 즉시 종료 / 3분 만료 = 절대값 우위). 점수 축은 손대지 않는다 — 현행 킬 점수 그대로.
- **적 본능의 사거리 vs 배치 배제 여유** [S] · **교전이 한 방향으로만 성립한다** (2026-08-10 unit 11 라이브 실측). 배치 배제(체비셰프 ≤ 4)가 본능 사거리(`Structure_TestInstinct.attackRange = 4`)와 **같아서** 최근접 합법 칸 거리 5 가 사거리 밖이다 → 본능은 `DefenderUnit` 단독을 노리는데 그 대상이 정의상 사거리에 못 들어온다. 반대로 사거리 6 저격수는 그 칸에서 본능을 깎는다(검증됨) → 본능은 «저격으로 안전하게 철거되는 벽».
  코드가 아니라 값의 문제다. 셋 중 하나: 배제 여유를 사거리보다 **작게**(포탑이 위협이 된다) · 사거리를 배제 밖까지 **늘린다** · **「저격 out-range 가 정답」이 의도면 현행 유지**(그러면 이 항목은 닫힌다). **의도 확인 필요.**

...
- **적 본능의 사거리 vs 배치 배제 여유** [S] · **저작 수치 모순** (2026-08-10 unit 11 실측). 9×9 배제가 체비셰프 ≤ 4 를 닫아 최근접 합법 칸이 5인데 `Structure_TestInstinct.attackRange = 4` 다 → 본능이 **한 발도 쏠 수 없다**(대상은 `DefenderUnit` 단독). 부술 수 있는 벽으로만 기능한다. 코드가 아니라 값의 문제이므로 둘 중 하나: 배제 여유를 사거리보다 **작게**(포탑이 위협이 되도록) 또는 사거리를 배제 밖까지 **늘려**. 「저격으로 out-range 하는 것이 정답」이 의도라면 현행이 맞고 이 항목은 닫힌다 — **의도 확인 필요**.

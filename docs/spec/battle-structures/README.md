...
6. **잠자는 `GoalPoint`/goal-stability 엔티티 경로를 걷어내나** — 기본값: **걷어낸다.** 골이 두 벌인 채로 `StructureTag` 를 붙이면 "어느 골에 붙였나" 가 실제 버그가 된다. 단 **`goal-stability` 스펙 문서는 남긴다** — 왜 그 설계였고 왜 접혔는지가 이 spec 의 근거다.

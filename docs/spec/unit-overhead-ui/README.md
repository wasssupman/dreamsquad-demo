...
> **확장 완료 2026-07-22**: 드림캐쳐 행 위에 **스택 이상효과 아이콘 행**(피로도/열기 + 카운트) 추가 — unit 6~10, Play 검증 통과. 아래 [확장 섹션](#확장-스택-이상효과-아이콘-행-2026-07-22) 참조.
...
**상태**: 완료 2026-07-22 (unit 6~10, **Play 검증 통과**) — 기존 오버헤드 UI(체력바 → 드림캐쳐 행) 위에 **스택 이상효과 아이콘 행** 추가. 계약/뷰/gather(듀얼A)/아이콘(Codex)/registry·씬 배선 커밋·컴파일0·테스트 그린·사용자 Play 확인. 인계: [11_handoff_summary.md](11_handoff_summary.md). StatusFx(몸통 VFX)는 구조가 달라 재사용하지 않는다(사용자 결정).

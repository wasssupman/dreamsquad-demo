...
**상태**: 구현 완료(unit 6~10) 2026-07-22 — 기존 오버헤드 UI(체력바 → 드림캐쳐 행) 위에 **스택 이상효과 아이콘 행** 추가. 계약/뷰/gather(듀얼A)/아이콘(Codex)/registry·씬 배선 커밋·컴파일0·테스트 그린. ⚠ Play 육안 검증만 남음. 인계: [11_handoff_summary.md](11_handoff_summary.md). StatusFx(몸통 VFX)는 구조가 달라 재사용하지 않는다(사용자 결정).

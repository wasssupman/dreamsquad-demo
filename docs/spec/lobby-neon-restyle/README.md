...
> 상태: **완료 2026-07-31** (사용자 실기 확인 통과). 인계 지도는 `3_handoff_summary.md`.
...
| 5 | `5_day_background.md` | 낮 배경 도입 → 디졸브 시간대 전환 활성화 | 낮/밤 페어 완성 |
...
- **낮/밤**: 2026-07-31 낮 버전(`lobby_bg_neon_day.png`) 수급 완료 — 디졸브 전환이 실제로
  시간대를 바꾼다. 뎁스맵은 **1장 공유**(낮/밤 baked depth 상관 0.9997, 평균차 0.59% 로
  실측 검증. 항구 페어의 0.998/1.2% 선례와 동급). 낮/밤 아트를 교체할 때는 씬의 day/night
  슬롯 + 뒤 레이어 Image + `SceneTransition.prefab` 의 같은 슬롯을 **함께** 바꿔야 한다.
...
> 종료와 함께 `docs/spec/README.md` 의 **Follow-up Backlog → 로비 네온 리스킨** 으로 이관됨.
> 아래는 종료 시점 기록.

- ~~네온 시티 낮 버전 배경~~ → 완료 (2026-07-31, unit 5).

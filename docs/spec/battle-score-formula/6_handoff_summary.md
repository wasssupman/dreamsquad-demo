...
- 유출 한계 10 → 만점 구성 `시간 18,000 + 스트레스 9,000 + 킬(가변)`.
  **킬 예산은 밸런스마다 바뀐다** — wave-pattern 밸런싱 후 11,200(74기). README 예산 표가 최신
...
- Play — 처치 1기당 정확히 +100 (당시 HUD 는 +10 이었다. `score-tally-sequence` unit 0 이 통일)
...
- ~~라이브 HUD 점수 ↔ 최종 점수 통합~~ → **완료** (`score-tally-sequence` unit 0, `80cb9eee`).
  `score_events[]` 도 이제 유닛별 실제 킬점수를 쌓으므로 `result.score` 의 킬축과 일치한다

...
- ~~**보스 로테이션**(`bossPool`)~~ → **`docs/spec/boss-slasher/0_boss_pool_field.md` 로 승격(2026-07-29)**. 두 번째 보스 "슬래셔" 추가와 함께. `bossUnit` 은 **rename 하지 않고 유지** + `bossPool` 추가 + 폴백 — 라이브 덱 9개가 guid 를 들고 있어 rename 하면 보스가 조용히 사라진다.

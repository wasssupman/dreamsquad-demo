...
구현 커밋 9a465578 (2026-07-25). 이벤트에 killer 도 동봉(owner 귀속) — 폭발발 킬의 OnKill 연쇄 재발동을 사양으로 확정.

e2e 확인 (2026-07-25, 라이브 에디터 더미 계측): 킬 +0.01s 에 인접(반경 1) 더미가 정확히 −25 — 발동·범위·flat 데미지 모두 스펙대로. **검증 함정 2건**: ① 근접 유닛(가디언)이 막타를 가져가면 OnKill 은 발동하지 않는 게 사양 — 검증 시 킬 귀속 분리 필수. ② Result 페이즈에선 브리지 드레인 정지라 킬 이벤트가 큐에 잔류 — 발동 검증은 Battle 페이즈 중에만 유효.

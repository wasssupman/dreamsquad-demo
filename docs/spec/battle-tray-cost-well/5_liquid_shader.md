...
- [ ] 라운드 코너가 매끈하다 / 액체가 모서리 밖으로 안 샌다 (사용자 Play 확인)
- [ ] 실기 성능 — 프래그먼트에 `sin` 2회 + SDF, 물통 하나뿐이라 무시 가능 수준이나 실기 확인
...
**완료 확인 2026-07-21** — 셰이더 신규 작성·머티리얼 생성·config 배선·Play 실측. 최초 구현은 `Mask` 때문에 완전히 정지 상태였고(사용자 지적), `Mask` 제거 + 셰이더 SDF 라운드로 근본 수정했다. 커밋: (아래 참조)

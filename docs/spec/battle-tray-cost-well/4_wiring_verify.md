...
**완료 확인 2026-07-21 — 사용자 확인 완료.**

에이전트가 실측한 것(Play, 16:9 / SafeAreaRoot 1920):

- 트레이 1280×158 = `1920 − 640` 클램프 적중, 셀/슬롯 폭 동일
- 코너 위젯 좌표 교집합 0 — 각성 여유 52 · Dock 여유 30 · 시작버튼 경계 접촉
- 배치 3회 재진입에도 코스트 셀 생존, 억제 시 슬롯 리플로우 없음
- 보드 최하단까지 약 245 units 여유(정적 계산) — 세로는 제약이 아니었다
- 레일 삭제로 하단 클러스터 상단이 `198 → 190` 으로 **8 감소**

**미이행 — 후속으로 넘김:**

- **EditMode 겹침 회귀 테스트를 만들지 않았다.** Play 좌표 실측으로 대체했으므로, 나중에 트레이 폭이나 코너 위젯이 바뀌면 자동으로 잡히지 않는다. 사각형 겹침 판정을 순수 함수로 빼고 화면비 × 슬롯 수 파라미터 테이블 테스트로 고정하는 것이 원안이었다.
- 실기기 `Screen.safeArea` 로그, 셰이더 성능 실측 — 에디터에서 대신할 수 없어 사용자 판단에 맡겼다.

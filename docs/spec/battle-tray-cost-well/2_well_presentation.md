...
`+N` 플로팅도 순 델타로 띄운다. 실제 획득량과 다를 수 있지만(`cost 3 / gain 5` → `+2`), **보유량 변화로서는 정확**하다.

> 소스 구동은 하지 않는다 — 후속. `AddCost` 는 실제 획득 정수를 반환하므로(`CostRuntime.cs:63-69`) 그 값으로 `+N` 을 직접 구동하면 더 정확하지만, 그러려면 `BattleBridge` 가 UI 를 호출해야 해 경계가 지저분해진다. 획득량 자체를 강조할 필요가 생기면 그때 이벤트 채널로 뺀다(README 후속 후보의 `CostRuntime` 이벤트화와 같은 작업).
...
| 항목 | 결과 |
|---|---|
| sentinel | 배치 진입 `0 → 10` 점프를 `_prevInt` 가 흡수, **MaxBurst 오발 없음** (H1 회귀 가드) |
| 소비 시 물통 | `7.5 → 4.5`, 숫자 7→4, **fill 0.500 유지** (핵심 계약) |
| max 분기 | `10/10` 에서 fill 1.000, idleGlyph off |
| 리젠 정지 | idleGlyph "대기" on + 액체 alpha **0.45** |
| 자연 충전 | `+N` 미발생, 리젠 재개 시 dim 해제 |
| 외부 획득 | `AddCost(2)`: 5.5 → 7.5, 숫자 5→7, **GainDelta "+2"**, 물통 0.5 유지 |

**구현 중 발견한 버그(수정함)**: `ApplyIdleState` 가 상태 전환 프레임에만 액체 alpha 를 건드렸는데, `Update` 는 매 프레임 `LiquidColorFor` 로 색을 새로 쓴다 → dim 이 한 프레임 만에 덮여 사라졌다. idle dim 을 색 계산 안으로 옮기고 `ApplyIdleState` 를 색 적용보다 먼저 호출하도록 순서를 고정했다. **정적 검토로는 안 잡혔고 Play 실측에서만 드러났다.**

미확인(연출 순간 포착이 어려워 unit 4 로): MaxBurst / WellTick / SpendFlash 의 시각적 적절성.

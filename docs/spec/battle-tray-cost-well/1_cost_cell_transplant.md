...
**완료 확인 2026-07-21** — Play 실측(BattleScene, Game view 16:9 → SafeAreaRoot 1920):

| 항목 | 결과 |
|---|---|
| 트레이 크기 | **1280×158** = `1920 − 640` 클램프 적중 |
| 패널 자식 | `[CostCell][SlotContainer]` 2개 (D1 구조) |
| 셀 / 슬롯 폭 | **131.1 / 131.1** — 동일. `childForceExpand` 였다면 셀 288.75 (D2 회귀 가드) |
| 배치 3회 재진입 | CostCell **ALIVE** 유지 (D1 회귀 가드) |
| 레일 잔존 | `CostPanel` 0개 |
| max 분기 | `cur=10.00` → `10/10`, fill **1.000**, surface off |
| 소수부 | `cur=7.50` → `7/10`, fill **0.500**, surface on, 색 Lerp 중간값 |
| 소비 | `10.00 → 7.00` 시 fill `1 → 0`, 숫자만 감소 |
| 억제 리플로우 | alpha 0/1 전환에도 슬롯 폭 131.1 불변 (D9 회귀 가드) |
| 코너 겹침 | 각성 여유 52 · Dock 여유 30 · 시작버튼 **경계 접촉(겹침 0)** |

슬롯 수는 씬 직렬화 pool 그대로 **8개**(D8 예측 적중). 16:9 + 8슬롯 조합에서는 클램프 때문에 슬롯이 131.1 로, 현재 128 대비 +2.4% 에 그친다. 실플레이(스쿼드 7 + 20:9)에서는 154(+20%).

미확인: pulse 시각 확인, 손패 플립 중간 프레임 — 정상 배틀 흐름이 필요해 unit 4 로 이관.

...
## 리팩토링 반영 (2026-08-02, `8953a924`) — Play 확인 완료

4각도 품질 리뷰(재사용·단순화·효율·고도) 병렬 실행 결과 반영. 효율은 권고 0건.
EditMode 1815 중 1813 통과·실패 0 · **사용자 Play 확인 완료**.

**품질 리뷰가 잡은 실제 결함 4건** (셋은 배선 누락)

| 결함 | 증상 |
|---|---|
| `UltimateLeapVisualEventsSingleton` 파괴 목록 누락 | **2판째부터** orphan 2개 → 예고·이탈·강하 없이 순간이동만. 콘솔 깨끗 |
| `SetPlacementHighlightAboveUnits` 등록 누락 | 유닛을 빼려 드래그하는 **바로 그 순간** 예고가 배치 하이라이트 아래로 묻힘 |
| `ProjectileEmitterSystem` 적 후보 풀 누락 | 패턴 유닛이 화면 밖 보스를 겨눠 빈 타일 사격 |
| `BossLeap` 의 `LeapFlight` remove 에 소유권 가드 없음 | 일반 도약 창이 끝나며 궁극기 잠금을 걷어감 → 무적인 채 공격·이동 |

**동작이 바뀐 변경 1건** — `AttackSystem` 의 도약 잠금을 쿼리(`WithNone`)에서 술어
(`actionLocked`)로 이동. 이제 비행 중에도 **쿨다운이 돌고 진행 중 스윙이 해소된다**(CC 규약과
동일 — 깨어난 유닛이 즉시 때리는 성질). 이전에는 둘 다 얼었다. `MovementSystem` 이 같은 diff 에서
이미 OR 로 접은 것과 고도를 맞춘 것이다. **되돌려 쿼리 필터로 옮기지 말 것.**

**거짓 근거 2건 정정** — 초판 주석이 사실이 아니었다. ① 셀↔월드 왕복은 **항상 정확**하다
(`floor(cell+0.5)=cell`); 두 필드를 드는 진짜 이유는 의존 회피다. ② 카드텍스트 `default` 는
`return false` → 에셋 description 폴백이라 "빈 문자열" 위험은 없었다(도달 불가 case 삭제).

**트리거 정정 2건** — 예고 refcount 는 "생존당 1회"가 아니라 **동시 2보스**가 트리거다(동시성을
막는 건 판에 보스가 1기라는 사실). `PendingDeployment` 통합은 "세 번째 메커니즘"이 아니라
**소비처 집합이 실제로 일치할 때**다.

`UltimateLeapState` 주석에 소비 사이트 5곳을 열거해 뒀다 — 손열거 집합이라 컴파일러가 돕지
않는다(실제로 하나 놓쳤다). 적 후보 쿼리를 새로 만들면 그 목록도 갱신할 것.

...
```yaml
- trigger:  { kind: 5 (HealthThreshold), fraction: 0.8 }   # 20% 에서 1회. 0.5 이상 = 생존당 1회 보장
  payload:
    kind: UltimateLeap
...
## 확정값 (2026-08-02 사용자 결정)

`fraction: 0.8` → **체력 20% 에서 1회**. 0.5 이상이라 두 번째 경계가 음수(−60%)이므로 생존 중
재발동이 수학적으로 불가하다.

⚠ **진동갑주(0.2)의 20% 경계와 정확히 겹친다.** 0.8 은 0.2 의 배수라 20% 를 지나는 순간 폭발과
궁극기가 **같은 프레임에 함께 발동**한다(boss-jjangssen 계약 5 가 피해뒀던 배치). 순서는 시스템
순서가 고정한다 — 폭발이 이탈 전 위치에서 터지고 그다음 이탈이다. 겹침을 피하려면 0.79(21%)로
한 칸 옮기면 되지만, **"터지면서 뛰어오른다"가 오히려 궁극기다운 연출**이라 겹친 채 확정했다.

## 예고 타일 배선 함정 (2026-08-02 실측)

`telegraphTile` 은 **모든 `TileSetData` 에 배선해야 한다.** 실사용 tileSet 이
`TileSet_AutoTileTest` 인데 `TileSet_Desert` 에만 넣었다가 폴백이 `placeableTile`(자체 색 회색
0.80)로 떨어져 **"색 없는 어두운 dim"** 으로 보였다. 색·정렬·머티리얼·스프라이트를 차례로
의심하게 만든 원인이 배선 누락 하나였다.

진단이 결정적이었던 지점: 보이는 타일맵(range)과 안 보이는 타일맵(telegraph)을 **같은 프레임에
나란히 덤프**하니 `tileCount` 가 1 vs 0 이었다. 색이 문제였다면 tileCount 는 같아야 한다.
- 확인: 2026-08-02 · 사용자 Play 확인 — 이탈·예고(주황 채움)·착지 정상. 발동 경계 20% 확정.

...
이탈 순간부터 착지까지, 착지 셀 중심 `slamTileRange` 반경을 빨갛게 표시한다.

⚠ **이 문단은 두 군데가 낡았다** (2026-09-07 · `distance-based-range` unit 24 — 여기 남기는 이유는
이 spec 이 무엇을 «의도했는지»가 그 unit 을 읽는 데 필요하기 때문이다):

- ~~Chebyshev~~ → **원**이다. 피해는 unit 4b 부터 원이었는데 예고만 사각이라 모서리가 거짓
  예고였다. 지금은 `SkillMath.ReachFromCell` 하나를 지난다(도달 = `N + 칸 반폭`).
- ~~「예고 타일 = 피해 타일」~~ → **성립하지 않는다.** 예고는 「중심점 기준 N거리, 몸체 크기
  무관」이고 피해는 제약 13 대로 대상의 몸을 더한다. 두 집합이 같아지려면 표기가 「누구의
  몸이냐」를 골라야 하고, 그 순간 화면이 유닛마다 다른 규칙을 말한다 — 그래서 **안 같게 두는
  쪽**을 골랐다. 대가(몸 있는 유닛이 예고 밖에서 맞는 앵커 수)는 `24_landing_telegraph_round.md`
  가 실측으로 적어 두고 `TileAoeTests` 가 표본을 핀으로 박았다.

살아 있는 의도는 **「예고와 피해가 같은 «중심»과 같은 «자»를 쓴다」** — 계산이 두 벌로 갈리면
예고가 거짓말한다는 원래 경고는 그대로 유효하다.

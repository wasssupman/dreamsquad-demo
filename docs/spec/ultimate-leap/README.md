...
>
> ⚠ **가운데 질문은 2026-09-07 에 «아니오»로 확정됐다** (`distance-based-range` unit 24, 사용자 결정).
> 예고는 **「중심점 기준 N거리, 몸체 크기 무관」**이고 피해는 제약 13 대로 **대상의 몸을 더한다** —
> 두 집합은 정의상 안 같고, 몸이 있는 유닛은 칠해진 칸 밖에서도 맞는다. 표기를 몸에 맞추면
> 「누구의 몸이냐」가 생겨 화면이 유닛마다 다른 규칙을 말하게 되므로 **의식적으로 고른 대가**다.
> 회피 플레이 성립(첫 질문)은 여전히 검증 대상이다.

...
- [ ] Play — 부착 카드 드래그 시작 즉시 불가 유닛 전부가 붉게, 가능 유닛은 시안 링
- [ ] Play — 캡(3/3) 유닛과 메커닉 불일치(예: 통통구슬×탄도) 유닛이 모두 붉은가
- [ ] Play — 커밋/취소/ESC/바깥 탭/페이즈 이탈 후 틴트가 전부 원복되는가
- [ ] Play — 락온 이동(유닛 A→B) 중 틴트가 어긋나거나 굳지 않는가
- [ ] Play — dim(0.42) 아래에서 붉은 틴트가 실제로 판독되는가 (안 되면 색 값만 조정)

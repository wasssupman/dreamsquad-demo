...
**확인 2026-07-29 · 커밋 `527ea53d`**(+ VFX 튜닝 `7dcb38b8`·`0436b29d`·`48f41cc0`) — 사용자 Play 확인.
경계마다 자기중심 폭발이 방어유닛에게 들어가고 VFX 가 보인다. 연출은 실플레이 피드백으로 2회 교체:
범용 `vfx_Hit_Rock03` → `EarthBlast` → **`WindAoeVFX`**(도약이 흙이라 원소를 갈라 구분), 스케일 최종 2.08.
EditMode 1575 중 1573 통과·실패 0.

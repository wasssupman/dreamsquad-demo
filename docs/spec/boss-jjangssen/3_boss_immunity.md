...
**확인 2026-07-29 · 커밋 `67875169`** — 컴파일 + EditMode 신규 12건(면역 술어 순수함수) 통과,
전체 1575 중 1573 통과·실패 0. 사용자 Play 세션에서 보스가 가디언에 끌려가지 않고 스스로 사냥하는 것 확인.

**미검증(남김)**: PlayMode e2e 5항목 — ① 잡몹에는 `Aggroed` 부착(면역이 전역이 아님) ② `AggroCapacity.held`
카운트 ③ 직접 Sleep 거절 ④ **Bleed 스택 → 보스 HP 감소**(스택 통과 가드) ⑤ 말파이트 넉업 시 보스가
떠오르지 않음. 술어 자체는 EditMode 로 고정돼 있으나 **배선 경로는 육안·자동 모두 미확인**이다.

...
> 상태: **완료 2026-07-29 (사용자 Play 확인)**. units 0~4 + unit 6(도약 비행) 구현·커밋,
> `905cfb3f`~`828a89ed`. EditMode 1575 중 1573 통과·실패 0.
> 실플레이 피드백 반영 튜닝 다회(연출 구분·아치 높이·경계 50%·10%). 인계: `5_handoff_summary.md`.
> 설계 근거·리뷰 이력은 `docs/plans/2026-07-29-boss-jjangssen-design.md`.
>
> **PlayMode e2e 는 미작성**이고 면역·도약의 개별 항목 일부는 육안 미검증이다 — 각 unit 하단
> "미검증(남김)" 과 handoff 의 Follow-up 참조.
...
> 자기중심 폭발**하고 **50%·10% 에서 밀집 지점으로 도약해 내리찍는가**? 나이트메어와 로테이션으로
> 공존하면서 **기존 7덱의 웨이브 편성은 무회귀**인가?

> 최종 능력 구성(2026-07-29 튜닝 확정): 상시 cleave 3(cd 0.6) · 진동갑주 `HealthThreshold(0.20)`
> ×`SelfTileAoe`(반경 2 / 60, WindAoeVFX) · 집단 도약 `HealthThreshold(0.50)`·`(0.90)` ×`SelfBlink`
> (밀집 반경 2 / 링 6 / 착지 슬램 반경 1·50, EarthSlamSpikes 아치 비행 0.83s).
...
5. **폭발과 도약의 경계는 분리돼 있다**(2026-07-29 튜닝): 폭발 0.20(20%씩 4회) vs 도약 0.50·0.90.

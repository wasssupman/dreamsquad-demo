...
- 비행 시작 프레임 → ~~출발 퍼프~~ **재생하지 않는다** (rev 2)
- 비행 종료 프레임 → **착지 연출**(`EarthSlamSpikesAoeVFX` — 원래 착지에 어울리는 연출)

로 재생한다. 도약 연출이 한 곳에 모인다.

**rev 2 (2026-07-29 사용자 확정) — 출발 퍼프 제거.** 도약 연출이 바닥에서 솟는 스파이크라
**착지에만** 어울린다. 발이 뜨는 자리에서 스파이크가 솟으면 인과가 거꾸로 읽힌다. 현 arm 은
단일 `dataIndex` 라 출발/착지에 서로 다른 연출을 줄 수 없으므로, 출발 전용 연출이 생기면
그때 인덱스를 분리해 되살린다(후속 후보).
착지 연출은 슬램(rev 1)이 있을 때 그 착탄 이벤트가 그린다 — 퍼프 경로는 슬램 없는 authoring
(`slamDamage <= 0`)용 폴백으로만 남는다.
...
**확인 2026-07-29 · 커밋 `7aa769ba`**(rev1 `6ab41555` · rev2 `74cbbcf0` · rev3 `aa7537b3` · rev4 `828a89ed`)
— 사용자 Play 확인. 순간이동이 아치 도약으로 바뀌고 팝이 사라진 것을 확인.

실플레이로 잡힌 것 3건이 전부 반영됐다: ① **1프레임 팝**(드레인이 `Update` 에 있어 sim 텔레포트 뒤
`LateUpdate` 가 오버라이드 없이 착지점을 그림 → `LateUpdate` 로 이동) ② 출발 퍼프가 인과상 거꾸로
읽힘 → 제거 ③ 비행 시간 +50%(0.83s)·아치 높이 상향.

**미검증(남김)**: 비행 중 보스 사망 시 공중 정지 없음, 슬로모 중 배율 동행,
착지 슬램 50 이 반경 1 안에 정확히 들어가는지(수치 대조).

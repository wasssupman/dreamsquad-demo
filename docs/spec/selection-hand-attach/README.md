...
- ~~**첫 세션 튜토리얼 문안** — `HandOpened` 원샷 힌트가 탭 즉발을 가르치지 않음(critic L4)~~
  → `docs/spec/first-session-tutorial/` **units 15·16** 으로 이관(2026-07-30). 조사 중
  **첫 판 각성 봉인이 유닛 선택으로 뚫리는 회귀**가 함께 발견돼 unit 15 가 그것을 먼저 막는다.

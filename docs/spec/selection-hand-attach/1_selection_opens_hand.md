...
> **이 절은 units 8·9 가 개정했다.** 항아리 토글(unit 9)과 0장 자동 닫힘(unit 8)은 이제 선택까지
> 데려간다 — 아래 서술 중 그 둘에 해당하는 부분은 무효다. `ForceClose` 만 원문대로 남는다.
> 결과적으로 "손패 닫힘 + 선택 생존" 은 도달 불가 상태가 됐다(계약 7 개정판 참조).
...
- ~~손패가 닫힌 채 선택이 살아 있으면 항아리 탭으로 재오픈 가능~~ — 도달 불가(위 참조).
...
- [ ] Play: 선택 중 항아리 탭 → **손패+선택 동시 해제**(unit 9 개정 — 원문은 "손패만 닫히고
      선택 유지" 였다), 항아리 재탭은 무선택 오픈
- [ ] Play: 커밋으로 사용 가능 0장 → 자동 닫힘(재딜인 생략) + **선택도 해제**(unit 8 개정 —
      원문은 "선택 유지" 였다)

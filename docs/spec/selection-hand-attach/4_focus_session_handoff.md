...
- [ ] ~~Play: **선택 중 항아리 탭으로 손패만 닫기** → 리티클이 살아난다(FocusCleared 경로)~~
      → **units 8·9 로 도달 불가**(두 경로 모두 선택까지 해제). `FocusCleared` 는 `_reticleShown`
      stale 리셋과 `ForceClose` 커버로 남긴다 — 손패를 선택보다 먼저 닫는 경로가 다시 생기면
      H2 가 그대로 재발하므로 지우지 않는다. 재주장의 주 깔때기는 `InteractionEnded` 로 좁아졌다.
- [ ] ~~Play: **마지막 카드 커밋(0장 자동 닫힘)** → 침강 완료 후에도 리티클 유지~~
      → **unit 8 로 개정**: 선택까지 해제되므로 리티클도 함께 사라지는 것이 정상이다.

...
- [x] compile 클린
- [x] Play: 유닛 선택 → 항아리 탭으로 손패 열기 → 선택(줌·리티클·패널) 유지, 카드 D&D 로
      다른 유닛 부착 정상(조준 중 선택 잔존 — 리티클 겹침은 unit 4 전 임시 상태로 허용)
- [x] Play: 선택 중 ActiveTile 카드 드래그 → 조준 시작 시 줌이 풀리고(홈 프레이밍) 커밋/취소 후 줌 복귀
- [x] Play: 선택 중 트레이 드래그 시작 → 선택 즉시 해제(기존 동작 보존)
- [x] Play: 선택 중 이동모드 진입 → 선택 해제(기존 동작 보존)

확인: 2026-07-29 사용자 Play 확인("확인함"). 커밋 해시는 git log 참조.

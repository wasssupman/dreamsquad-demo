...
1. **셀 층은 `tiles` 에서 파생한다. 저작된 `placeMask` 를 통행으로 읽지 않는다** (rev 2.1 정정 — 아래 상자).

> ### ⚠ rev 2.1 정정 (2026-08-09) — 계약 1 초판이 라이브를 깨뜨렸다
>
> 초판은 *"셀 층 비트필드 한 벌을 배치·통행이 공유한다"* 였다. **양립 불가였다.**
>
> `BattleBridge` 는 필드를 굽기 **직전**(`:1061-1070` `CloseCellLayers`) **스폰·골 칸의 `placeMask` 를 0 으로 닫는다** — «적이 튀어나오는 칸 위에 방어유닛을 못 세운다»는 배치 불변식이다. 그런데 통행 계약 7 은 «스폰·골은 **반드시** 지날 수 있어야 한다»를 요구한다. **한 배열로 동시에 만족할 수 없다.**
>
> unit 1b 초판(`fd25d53b`)이 실제로 이걸 밟았다: 골의 통행 층 0 → `FlowFieldBuilder` 가 `walkMask[srcIdx]==0` 소스를 버림 → **유효 소스 0 → 전 셀 `MaxValue`/zero → 적이 한 발도 안 움직인다.**
>
> 그리고 저작 의미 자체가 다르다. `MapDocument_Test` 는 `Walk` 칸 23개에 마스크 0 을 저작해 뒀다 — *"여기 **배치** 금지"* 이지 *"여기 **통행** 금지"* 가 아니다. 그걸 통행으로 읽으면 통로 23칸이 사라지고 데코 7칸이 새 통로가 됐다(실측).
>
> **정정: 통행 층은 `PlacementLayers.Derive(tiles[i])` 로만 만든다.** 그러면 `cellLayers` 는 `walkMask`(1비트)의 **N비트 일반화**가 되어 라우팅과 벽 충돌의 정의가 **갈릴 수 없다**. 배치 저작은 통행에 닿지 못한다.
>
> 물타일은 여전히 3줄이다(`Water` 비트 + `Derive` case + 페인터). 통행을 **저작으로** 나눠야 할 날이 오면 그때 별도 배열을 만든다 — **그때는 실제 예가 있을 것이다. 지금은 없다.**

   ~~셀 층 비트필드는 하나를 공유하고 유닛 쪽 축만 둘이다~~ (폐기).

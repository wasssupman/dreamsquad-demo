...
**D1 — 마스크가 다른 유닛끼리 «앞선 적» 순서를 어떻게 정하나?** `FrontmostTargeting` 은 `dist` 오름차순으로 "골에 가장 가까운 적"을 고르는데, 필드가 여러 벌이면 **물 적의 dist 와 육지 적의 dist 는 다른 그래프 위의 값**이라 직접 비교가 의미를 잃는다(물길이 돌아가면 물 적의 dist 가 항상 크게 나와 영원히 후순위가 될 수 있다).
후보: (a) 정규화 — `dist / 그 마스크의 최대 dist` (b) 골까지 직선거리로 대체 (c) 마스크별로 따로 랭킹 후 교차. **제품 감각이 필요한 결정이라 사용자 판단 요청.**

...
| 1b | 라우팅 다중화 | **마스크 집합을 받아** 슬롯 N개를 굽는다(`(cellLayers & mask) != 0` 기반) + 도달 실패 폴백. 집합은 **인자** — 지금 호출자가 `{Path}` 하나를 넘긴다 | 0 (1종이면 바이트 동일) |
| 2a | 유닛 축 + 로스터 수집 | 유닛 `traversalLayers`(SO+컴포넌트) + 스폰 주입 + **로스터에서 마스크 집합 수집**(오름차순)이 1b 에 실제 입력을 공급 | 0 (폴백이 현행) |
...
> **⚠ 순서 정정 (2026-08-09, 1b 착수 시 발견)**: rev 2 초판은 1b 에 «로스터 수집»을 넣고 2a 에 «유닛 축»을 뒀는데, **의존성이 역전**돼 있었다 — 로스터에서 마스크를 모으려면 유닛이 먼저 마스크를 가져야 한다. 그래서 1b 는 «집합을 **받아** 슬롯을 굽는다»(합성 집합으로 테스트 가능)로 좁히고, 수집은 축이 생기는 2a 로 옮겼다.

...
- [x] compile 에러 0 · **EditMode 2044 중 2041 통과 · 실패 0**(스킵 3은 기존 `[Ignore]`)
- [x] 신규 2건: ① 층 `Ground|Path` nav 는 배치지 칸을 통행 가능으로 본다 ② 층 `Path` nav 는 기존 `walkMask` nav 와 셀 단위 동일(무변경 축)
- [x] **라이브 배틀 로그**: 순찰병이 15칸을 밟았고 **경로 6 · 배치지 9** 로 실제로 건넜다 (`10,3 → 9,3 → 9,2 → 8,2 → 8,1 → …`)
- [x] **증상 단언의 red-green 확인** — `PatrolDefenderPlayTest` 에 «순찰병이 움직인다»를 넣고, 수정을 일시 되돌려 **그 단언 하나만 빨간 것**을 확인한 뒤 복구했다. 한 번 초록인 것으로 끝내지 않는다
- [x] 적 이동 회귀 없음 — `MovementIntegritySmokeTest` 통과. 적은 층 `Path` 라 nav 가 기존과 셀 단위 동일
- [x] PlayMode 전체 재측정 (2026-08-11, 94건 중 19건 실패) — **이 diff 영역은 전부 통과**:
  `MovementIntegritySmokeTest`(이동·통행층) · `PatrolDefenderPlayTest`(순찰·소환·재배치) · 타겟팅 스위트 전부.
  실패 19건은 UI·드림캐쳐·오라·인증·거점 영역이고, 그중 3건(`StructureLivePlayTest`)은 이후 올라온
  `battle-structures`/`structure-hunter` 작업의 자기 계약 단언이다.

  > ⚠ «사전 실패 여부」는 **끝내 못 갈랐다.** 그 사이 다른 세션이 코드를 크게 얹어 지금 재는 것은
  > «그때의 13건」이 아니라 «지금의 19건」이다. 베이스라인은 **변경 직후에만** 의미가 있다 —
  > 미루면 영원히 못 뜬다.

---

**완료 기준 확인**: 2026-08-10 · 커밋 `4cbfe751` · 사용자 Play 확인 완료

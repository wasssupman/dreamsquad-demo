...
마스크가 Walk 셀을 포함할 때 "적·유닛 행동 불변, 위치 제약만 해제"(B-1)가 실제로 성립하는지 검증한다. 코드 변경은 원칙적으로 0 — 이 유닛은 "배치칸 = 벽(비-walkable)" 암묵 전제 위에 선 시스템 6곳의 **재검토·검증**이 본체다. 재검토 중 결함이 나오면 최소 수정으로 좁힌다(기능 확장 금지).

> **진행 상태 (2026-08-07)**: 자동 축 완료 — 라이브 경로 테스트 2건 그린, `FindNearestPathDirection` 자기 셀 제외 커밋(3eca983e), 주석 3곳 현행화. **육안 축 대기** — 검증용 맵 저작(Map Painter 마스크 브러시로 제작), Play 시나리오 4종, 아래 표의 확인 결과 추기.

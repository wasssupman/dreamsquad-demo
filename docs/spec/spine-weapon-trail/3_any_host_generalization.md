...
### 계측기 한계 — 보스 룩은 아직 판정 못 했다

보스 하네스는 **URP 추가 데이터가 없는 맨 `Camera`** 로 찍었다. 그 탓에 화면 전체에 어두운
사각 헤이즈가 끼는데, `CopyFrom(main)` 으로 실제 보드 카메라를 복제해 찍은 디펜더 컷에는
없던 현상이다. **파티클을 꺼도 남으므로 파티클 탓이 아니고, 계측기 탓으로 본다.**

따라서 보스 룩의 **절대 판정은 무효**다. 다만 같은 조건의 **상대 비교는 유효**하므로,
Simple 만 사각 경계로 깨지고 ToonWater 는 깨끗했던 관찰에 근거해 나이트메어를 ToonWater 로
바꿨다. 최종 확인은 실제 보드 카메라로 다시 해야 한다.

(BattleScene 을 열지 못한 이유: OutgameScene 에 미저장 변경이 있어 씬 교체를 중단했다.
남의 작업을 버릴 수 없다.)

...
> **이 절은 unit 17 이 무효화했다.** 아래 원문은 "완료 저장은 첫 B 1회"를 전제로 쓰였는데,
> 그 설계가 실제로 **먼저 뜬 쪽이 반대쪽을 삼키는** 결함이었다(사용자 보고 2026-07-30).
> 지금은 저장이 경로별이고 `_awakeningArmedThisBattle` 은 제거됐다 —
> `17_independent_attach_hints.md` 가 최신 계약이다.
...
- [x] Play 둘째 판 전투 시작: 0단계가 **두 문 문구 2줄**로 뜨고 포커스 링은 항아리에 있다 (2026-07-30 확인)
- [x] Play: 0단계 상태에서 **항아리로** 열면 `…끌어보세요!` + usable 슬롯 포커스 (2026-07-30 확인)
- [x] Play: 0단계 상태에서 **유닛 탭으로** 열면 `카드를 탭하면…` 2줄 + usable 슬롯 포커스 (2026-07-30 확인 — unit 17 rev 필요했다)
...
구현 `652d2b4f` (2026-07-30). 0단계 두 문 문구와 **양쪽 경로의 안내 문구**는 사용자 Play 확인
완료 — 단 유닛 탭 경로는 `unit 17` rev(`4d87fa3f`) 가 있어야 뜬다. 나머지 항목(즉발 부착 일치 ·
저장 후 재노출 없음 · `OnCardPeeked` · 첫 판 무노출 · 셋째 판)은 미확인이며 `18_handoff_summary.md`
의 Follow-up 에 모았다.

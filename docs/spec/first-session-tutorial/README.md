...
> 기믹 리빌 안내(23~24) **완료 2026-08-01** (`5be4fa9b`·`2805f70f`, 리뷰 반영 `56c5c65d`·`a7a2a228`)
> — **사용자 Play 확인 통과**. 인계는 `25_handoff_summary.md` ·
> 효과 타일 배치 안내(unit 26) **완료 2026-08-02** — 사용자 Play 확인 통과
...
| 26 | `26_buff_tile_placement_hint.md` | 효과 타일 안내 | 두 번째 판 배치 구간, 비차단 한 줄 + 타일 마커 |
...
- **효과 타일 안내(unit 26)의 게이트는 `!_awakeningLockedThisMatch` + 자기 토큰이다.** 리빌
  안내 완료를 체인하지 않는다(unit 23 계약) — 순서는 페이즈(`Gift → Gimmick → Placement`)가
  이미 보장한다. `IsCorePending` 으로 첫 판을 가르려 하지 말 것.
- **보드 위에 링을 얹는 안내는 말풍선과 겹친다 — 위치와 대상 선택을 함께 잡아야 한다(unit 26).**
  말풍선은 상단 고정이고 링은 보드 어디든 뽑히므로, 전용 오프셋만 올리거나 대상만 바꾸는 것
  중 하나로는 맵에 따라 다시 겹친다. 마커 라벨은 **달지 않는다** — 라벨 플레이트가 링 위로 붙어
  말풍선을 파고들고, 말풍선이 이미 대상을 부르므로 잃는 정보가 없다(core 안내가 라벨을 다는 건
  옆에 설명 문장이 없어서다).
- **효과 타일은 맵에 0개일 수 있다.** `desert.asset` 은 `effectTiles: []` 다. 그 판에서는 안내를
  생략하고 **토큰을 저장하지 않는다** — 저장하면 효과 타일을 한 번도 못 본 계정에서 안내가
  소진된다. 경고가 아니라 로그다(정상 플레이 경로).
- **타일 좌표는 `TilemapMapView` 가 준다. `BattleBridge._effectTilesByCell` 을 열지 않는다** —
  컨트롤러에 `bridge` 참조를 새로 넣으면 BattleScene 배선이 필요하고, "어디에 빛나는 타일이
  보이나" 는 뷰 지식이다. 앵커 형태는 `_spawnVisualAnchorsWorld` 선례를 따른다.

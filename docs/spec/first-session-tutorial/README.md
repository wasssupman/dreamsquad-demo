...
- **HUD 안내는 비차단이다.** 전투 중이라 `SetTapToContinue`(풀스크린 캐처)를 쓰지 않고 시간
  경과로 넘긴다. 웨이브 안내는 클릭을 요구하지 않는다 — ④ `단, 준비가 되었을때!` 가 "지금 누르지
  마"라는 뜻이라 행동 요구와 모순이고, 첫 판에 웨이브를 겹치면 안내가 패배를 유도한다.

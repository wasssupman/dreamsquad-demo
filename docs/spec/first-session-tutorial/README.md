...
- **리빌 안내에 `SetElevated` 를 쓰지 않는다.** guidance canvas 는 1500, 리빌 패널은 20 이라 이미
  위다. 선물(30)에서 필요했던 호출을 복붙하면 원복 경로만 늘어난다.
- **리빌 말풍선은 기본 앵커를 쓸 수 없다.** 리빌 콘텐츠가 y `+390`(아이콘 상단) ~ `-290`(탭힌트)
  을 점유해 기본 앵커(y `356`~`240`)가 아이콘과 겹친다. 전용 앵커로 탭힌트 아래에 둔다.

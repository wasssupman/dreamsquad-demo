...
- **클래스 안내 문구 정합성 3건** (전부 사실 확인 완료, 문구 변경은 제품 판단이라 보류):
  ① 배치 트레이 role 배지가 `원/수/근/술/보` 단일 글자라 안내문 클래스 이름과 겹치는 글자가 0개 —
  읽어도 트레이에서 대응시킬 앵커가 없다. ② `적 이동경로에 방해물을 설치` 는 캐스터 4종 중
  `BlockingCaster` 하나에만 맞고 Fire/Ice/Poison 은 장판이다(기본 스쿼드에 Blocking·Fire 둘 다 포함).
  ③ 표기 4중 드리프트 — 배지 `보`, `UnitLabels.ClassLabel` `서포트`, 유닛 desc `서포트`, 안내문 `서포터`.
  게임 desc 는 `어그로` 대신 `도발` 을 쓴다.
- 서포터(Healer)는 기본 스쿼드 7개(카탈로그 앞 7개)에 없다 — 첫 판 트레이에 `보` 배지가 한 칸도 없다.
  나중에 만날 클래스를 미리 알리는 구조임을 인지할 것.

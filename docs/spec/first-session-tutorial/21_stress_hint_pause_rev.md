...
**완료 확인 2026-08-01** — 사용자 Play 확인 통과. 커밋 `65a4fb74`. 전투가 실제로 멈추고(남은시간·적·웨이브 정지) 탭으로 재개, 방치 시 폴백 자동 해제, **정지 중 씬 이탈 후 다음 판 정상 속도**(lease 누수 없음), 손패 슬로모 0.3x 와 겹친 뒤 복귀까지 확인. TimeManager Battle 도메인 lease 방어 4겹이 의도대로 동작한다.

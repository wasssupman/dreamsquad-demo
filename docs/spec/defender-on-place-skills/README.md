...
- **전방 관통 4종의 배치 스킬 재설계** [M] · 지금은 각자의 평소 공격과 **같은 방향·같은 직선**이고
  ...
  같은 것도 여기서 함께 푼다.
- **재배치는 조준을 갱신하지 않는다** [S] · `DeployedFacing` 은 배치 확정 시 1회 기록 후 불변인데
  재배치는 on-place 를 재무장한다 → 옮겨 간 방향 지정 유닛의 재발동 일격은 **이사 전 조준**으로
  나간다. 그 유닛의 평소 공격도 같은 stale 값을 읽으므로 일관되기는 하다(신규 결함 아님).
  unit 4 가 처음으로 배치 스킬 방향을 이 값에 묶었으므로 기록해 둔다.
- **테스트 더미가 통행 층을 안 가진다** [S] · `SpawnDummy` 가 `PathFollowState` 를 안 붙여
  `PlacementLayers.CanTarget` 이 조기 true 를 낸다 → 레이어 마스크 계산이 틀려도 초록이다.
  킨들러류 하드 마스크 유닛의 회귀를 못 잡는다.

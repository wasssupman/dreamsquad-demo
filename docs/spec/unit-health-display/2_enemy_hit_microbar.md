...
— 완료 확인 2026-07-04 · 커밋 `f51e41e`. compile 0, EditMode 447 중 446 통과(무관 사전 ObstaclePlacer). Play 검증: HP 만피→빈사 14적에 바 등장, fill 길이/색(녹→황→적)이 HP 반영, 머리 위 빌보드, 1적=1바(스택無, 14적=14바), 2s 후 전부 fade+pool 재활용(active 0), HitBar 콘솔 에러 0. 데미지숫자 병행(sorting 16000<32000)·막타 fill0 페이드는 코드 경로 확인. 구현 편의: 프리팹 대신 절차 생성(흰 스프라이트 shared) — 값은 전부 SO.

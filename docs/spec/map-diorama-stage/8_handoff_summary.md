...
- **Commit**: `426229aa`(unit 5 육안 기록·프리뷰 툴·tile-scale 초안) · `355440fa`(unit 6 재해석 — 포탈 프랍 마커 비주얼) · 이 문서 마감 커밋. main 은 feature 를 fast-forward 로 편입(GitHub main ≡ GitLab master).
- **Implemented(최종)**: 스테이지 프리팹 = 맵 정본(스캔→조립, bake 없음) · 마커 6종(Spawn/Goal/Route/BonusSpawn/Structure(본능)/PlacementBlockZone + PropFootprint) · 라이브 풀 4장(Duel·Street·Subway·StreetDay, 맵별 덱) · 골/스폰 = 수직 포탈 프랍(빨강/노랑) · 레거시 KayKit 스테이지·MapDocument 계열 은퇴 · 원격 육안 도구(`RenderPrefabPreview`).
...
- **Review**: critic APPROVE-WITH-NITS(Major 3 · Minor 7) → 전부 반영. 핵심: 라이브 풀 그물은 «공용 프랍 내장 금지»(맵 전용 저작 허용) · `GoalMarker` 캐시는 `visualRoot` 변경 시 재구축 · 설치자 스윕은 자기 씬·Play·활성 서브트리만.
- **Facing**: 포탈 방향은 `MarkerPropStyle.propEulerAngles`(현재 Y −90 = 링 정면이 카메라 쪽, 사용자 «Y축 90도» 2026-08-27). 프리팹 `Portal` 링이 yaw −95° 로 서 있어 identity 면 옆면(얇은 타원). 러너 `preview_all_props` 로 스타일만 바꾼 뒤 4맵 프리뷰.

...
- [x] compile (Unity 없으면 `dotnet build` — csproj 파일 명시 나열 주의)
- [x] 빈 씬에 `MapStage` + 프랍 몇 개를 놓고 기즈모로 차지 셀·playArea·스폰 라벨이 보인다 (에디터 육안)
- [x] 제안 버튼 2종(playArea/footprint)이 동작한다
- [x] 런타임 로직 0 확인 — Update/Awake 등 수명 메서드 없음

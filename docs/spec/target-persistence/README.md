...
상태: **units 0·1·2 구현 완료 2026-08-09 · ⚠ Play 육안 미확인 · units 3·4 미착수**

- unit 0 (`0_attack_commit.md`) — 완료 기준 확인 **미확인**
- units 1·2 (`1_shared_predicate_and_range_release.md`, 한 커밋으로 묶임) — 완료 기준 확인 **Play 육안 미확인**
- units 3·4 — 미착수. 미결 결정은 **해소됨**(아래 §미결 결정): D1 = 가디언 제외 · D2 = 즉시 해제
- **다음 세션의 첫 할 일은 units 0~2 의 Play 육안 확인**이다. 코드는 들어가 있고 EditMode 는 초록이지만, 그것으로 «증상이 사라졌다»를 말할 수 없다(`CLAUDE.md` 버그 수정 절차). B1(A 겨누고 B 때림)·B2(Focus 적이 골로 걸어감) 두 증상을 눈으로 봐야 한다.

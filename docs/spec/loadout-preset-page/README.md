...
> **은퇴 2026-07-30** — 이 spec 의 "프리셋 = 기획자 authoring 읽기전용 콘텐츠 + [적용]으로 현재 로드아웃 덮어쓰기" 모델은 폐기됐다. 후속 후보로 적어둔 "플레이어가 현재 로드아웃을 프리셋으로 저장"이 정식 기능이 되면서 페이지 자체가 사라지고, 프리셋은 스쿼드·드림캐쳐 각 페이지 안으로 들어갔다. → `docs/spec/authored-preset-removal/` (철거) · `docs/spec/page-local-presets/` (대체 기능). 아래 내용은 읽기 전용 이력이다.
>

...
### 안전망의 실제 사거리 (정정 1 후속 · 2026-08-31)

`summoner` 시나리오를 코퍼스에 넣고 구웠다 — **이벤트 227 · 킬 9 · 유출 0 · configHash
`64b200d54e5910a9`**(다른 7건과 다른 해시 = 덱이 실제로 갈렸다는 증거). 8건 전건 통과.

**무엇이 열렸나 (측정치):** 순찰병 2기(simId 9·39)가 각각 **공격자로 8회** 등장한다.
순찰병은 이 프로젝트의 **유일한 연속 이동 아군**이므로, 이 16번이 곧 `AttackReach` 의
2차 게이트(`bothContinuous == true`)가 코퍼스 안에서 실제로 도는 횟수다. **이전에는 0이었다.**

**무엇이 아직 안 열렸나:** 순찰병이 **피해를 받은 기록은 0건**이다. 즉 「적(다중타격) ×
순찰병」 방향 — 정정 1 이 지목한 바로 그 `:1551` 조합 — 은 이 시나리오에서도 안 걸렸다.
열린 것은 반대 방향(순찰병이 공격자)이고, 그건 주 타겟 선정·락 유지 경로를 지킨다.
**이 한계를 「열렸다」고 적지 않는다** — 그렇게 적는 순간 이 안전망도 「통과하지만 아무것도
증언하지 않는」 것이 된다.

**덱 구성을 만지지 말 것.** 2종(`[summoner, archer]`)·3600틱으로 소환사 지분을 5회 중 3회로
올리고 시간을 2배 줘 봤지만 **순찰병 스폰은 2회 그대로**였고(소환 게이트가 시간이 아니라
상태로 걸린다) 캐논이 빠져 판만 무너졌다(킬 9→1 · 유출 0→1). 되돌렸다.

**측정 자체가 한 번 틀렸다 — 그래서 트레이스를 고쳤다.** 처음 「순찰병 공격 2회 · 피격 0회」로
읽었는데 둘 다 무의미했다: `PatrolSpawn` 이 **소환사** id 를 싣고 순찰병 id 는 아예 안 실어서
트레이스에서 순찰병을 식별할 방법이 없었고, `DamageNumber` 는 피해자를 `a` 에 싣는데 `b` 로
찾고 있었다. `PatrolSpawn` 을 **생성 뒤**로 옮기고(전에는 스냅 실패로 소환이 취소돼도 찍혔다)
`b` 에 순찰병 SimEntityId 를 실었다. 기존 7건은 `PatrolSpawn` 이 0건이라 무영향(git 무변경 확인).

**하네스 게이트 2종 추가** — 이 spec 이 반복해서 밟은 「통과하지만 아무것도 증언하지 않는」
사고를 코드로 닫는다:
- **셋업 게이트**: 시나리오 덱을 못 만들면(카탈로그 미발견·id 미해결) 그 트레이스를 **저장하지
  않는다.** 부분 덱도 거부한다 — 소환사 하나만 빠져도 축이 사라지는데 판은 멀쩡히 돈다.
- **`Bake Missing Goldens Only` 메뉴**: 코퍼스에 시나리오를 **추가**할 때 기존 기준선을 덮지
  않는다. 전체 재생성으로 대신하면 기존 7건이 「지금 코드」로 리베이스돼 회귀 감시가 그 자리에서
  무효가 된다. 새 축을 여는 것과 기존 축을 리베이스하는 것은 다른 동작이다(후자는 unit 6).

...
| **B** | `SelfAreaBlastSkill` · `DeathSiteBlastSkill` · `BossLeap:230` → `ProjectileHitSystem:747` | **자기 자리 폭발**(브루저 배치폭발 · 궁지/실드/진동갑주 카드 · 시체폭발 · 작별선물 · 재앙의심장 · 퇴근운석 · ~~보스 도약 슬램~~ ⚠ 착지 슬램 2건은 unit 24 에서 **자리형으로 환원**) | 트리거 대상 유닛 | 칸 반폭 0.5 (**intent 경계 뒤**) | critic + audit |
...
- [x] **자기 자리 폭발 전수** — 자폭(`SelfAreaBlastSkill`) · 사망/시체/퇴근(`DeathSiteBlastSkill`) ·
  ...
  > ⚠ **이 2건은 2026-09-07 에 되돌렸다 — 형을 틀리게 읽었다**(unit 24, 사용자 결정).
  > 착지 슬램은 「그 몸이 내리찍는 것」이 아니라 **운석과 같은 「자리에 떨어지는 것」**이다 —
  > 보스가 **지정한 좌표**에 내리고, 지정은 기하가 아니라 귀속이다. 두 생산자는 이제
  > `originBodyRadius = 0` 을 싣는다. 라이브 Δ 는 그때도 0 이었다(아래 코퍼스 표와 같은 이유).
  > **교훈**: `flightTime == 0`(즉발) 은 배선의 성질이지 **형의 판별이 아니다.** 이 감사는
  > 「즉발이면 폭심에 주인이 있다」로 묶었고, 그 묶음이 두 건을 잘못 데려왔다.
...
- [x] Play 육안: 배스티온 도발이 옆구리 적을 실제로 끌어오는가 — **사용자 확인 2026-09-07**.

> **unit 23 종료 2026-09-07.** 구현 `7d439668`(23a) · `cf95fac7`(23b) · CRITICAL 수정
> `fc6bfa7c` · 테스트 복구 `5ec31249` · 마감 `bd243f27`. 잔여 없음.
...
| 자리 | 문제 |
|---|---|
| `BattleBridge.UltimateLeap.cs:68` → `:2781` | **오늘 이미 거짓말 중.** 착지 예고가 `(2N+1)²` **사각**인데 피해는 **원**. 주석의 「예고 셀 = 피해 셀 계약」은 unit 4b 이후 거짓 → **unit 24 에서 원으로 전환.** ⚠ 이 행이 적은 *「N≥2 부터 모서리가 **거짓** 예고」* 는 **틀렸다**(2026-09-07 재검산): 피해 도달이 `N+0.5+대상 몸`이고 방어유닛 몸 최솟값이 0.5 라 모서리 칸에 선 유닛은 **로스터 전부 맞는다**. 대상을 점으로 놓고 잰 것이 오진의 원인이다 |
...
| 단위 | 내용 | 게이트 | 크기 |
|---|---|---|---|
| **23a** | **도달 질의의 자** — 진입점 2개 · 상수 private · `SelfArea` **9곳**(브레스 포함 — 아래 ⚠) · `CasterRef` 몸 · 부착 프리뷰 host 인자 · 금지 가드 · 차등 단언(**폭 2 이상**) · 골든 A/B | 없음 | 파일 ~12 · 테스트 3종 · 골든 1회 |

⚠ **브레스는 `SelfArea` 로 «바꿨다»**(리뷰 M-3 — 초안은 「제외」였다). 프리필터를 자리형으로
두면 **원점이 유닛인데 칸 자**라는 명제 위반이 그 자리에 남기 때문이다. 행위 변화는 **0** 이다 —
콘 게이트(`SkillCone:41`)가 여전히 양쪽 몸 0 이라 언제나 더 좁게 자른다. **결함 D(콘 자체)는
여전히 미해결**이고 별건이다 — 「체크박스만 켜졌다」로 읽지 말 것.
...
## 골든 A/B — 실측 결과 (2026-09-06)

**방법**: `SkillMath` 에 임시 스위치를 두어 원점 항을 옛 동작으로 되돌리고 **현재 저작 조건**에서
기준선 A 를 굽고(8/8), 스위치를 원복해 `Verify`. 측정 후 스위치를 제거하고 `SkillMath` 가
커밋본과 **바이트 동일**임을 확인했다.

| | A(현재 저작 + 옛 판정) | B(unit 23 판정) |
|---|---|---|
| 8건 중 일치 | — | **7건 ✓** |
| 갈린 것 | — | **`force_wave` 의 `finalStateHash` 하나** |
| 이벤트 수 · 킬 · 유출 | 기준 | **8건 전부 동일** |

### ⚠ 이것은 «측정 실패» 다 — 예측대로 되지 않았고, 이유가 밝혀졌다

예측은 「킬 ↑ · 유출 ↓ · 공격/피해 이벤트 ↑」였는데 **총계가 전부 그대로**다. 배치 로그로
원인을 확인했다 — **코퍼스가 배치하는 것은 아처·가디언·캐논·힐러·파이어/아이스/포이즌캐스터이고
배스티온이 없다.** 즉 **Δ 가 가장 큰 유닛(+1.0 · 면적 ×1.96)이 측정에서 통째로 빠졌다.**
(감사가 「기본 덱에 Archer·Guardian·Bastion 이 전부 있다」고 한 것은 **덱 목록**이었고,
실제 **배치 정책이 뽑는 4기**에는 배스티온이 안 들어간다.)

남은 둘도 약하다: 아처 둔화(3.5→4.0)·가디언 공버프(2.5→3.0)는 **오라**라 킬에 간접적이고,
코퍼스 배치가 (3,4)·(1,5)·(0,2)·(5,4) 로 이미 2.5칸 안에 몰려 있어 **넓혀도 대상 집합이 안 변한다.**

**`force_wave` 의 `finalStateHash` 만 갈린 것이 효과가 실재한다는 증거**다 — 이벤트 스트림은
같은데 최종 상태(체력 등)가 다르다. 오라 적용이 미세하게 달라졌지만 킬을 못 바꿨다는 뜻이다.

### 두 결론을 «섞지 않는다»

1. ✅ **회귀 위험이 낮다** — 8건 중 7건 바이트 동일, 총계 무변.
2. ⚠ **효과를 측정하지는 못했다** — 이 코퍼스는 **자기중심 광역 축의 회귀 탐지기로 약하다.**
   고치려면 **배스티온이 배치되는 시나리오**를 코퍼스에 넣어야 한다(후속 후보).

### 코퍼스 재베이크의 내용

재베이크된 8건의 diff 대부분은 **unit 23 이 아니라 `enemy-detection-range` 저작 드리프트**다
(`basic` 이벤트 70→76 등). 위 A/B 가 unit 23 단독 효과를 `force_wave` 하나로 한정했으므로,
이 커밋의 큰 변화는 **감지 저작이 기준선에 처음 반영된 것**으로 읽어야 한다.

## ⚠ 골든 「무변」의 **두 번째 원인** — 23b 경로가 죽어 있었다 (리뷰 C-1)

앞 절의 A/B 는 **23b 가 런타임에서 작동하지 않는 상태**에서 잰 것이다. `state.originBodyRadius`
대입이 `BallisticArcToPoint` 분기 한 줄뿐이었고 자기 자리 폭발 4종은 전부 `SkyFall` 이라,
`ProjectileState.originBodyRadius` 가 **항상 0** 이었고 `ReachFromImpact` 가 칸 반폭으로 폴백했다.

즉 앞서 「무변의 원인 = 배스티온 미배치」라고 적은 것은 **절반만 맞다.** 두 번째 원인이 있었고,
**23b 축은 애초에 신호가 날 수 없었다.** `force_wave` 의 `finalStateHash` 하나는 **23a(오라) 몫**이다.

**두 리뷰어(code-reviewer · ecs-reviewer)가 서로 모르는 상태에서 같은 결함을 지목**했다.

### 왜 그물이 못 잡았나 — 네 번째로 같은 형태

가드가 **생산자 파일**과 **소비자 파일**의 소스 문자열은 봤지만 **그 사이의 복사 지점**은 어떤
단언도 지나지 않았다. 그리고 `0` 이 「자리형」과 「안 실었다」를 겸직해 관측상 완전히 조용했다.

> **일반화**: 배관을 고칠 때 **양 끝만 보는 그물은 중간을 못 잡는다.** 값이 A→B→C 로 흐르면
> A 와 C 뿐 아니라 **B 를 이름으로** 고정해야 한다.

→ `OriginRadius_IsCopiedInTheCommonInitializer_NotPerBranch` 신설(초기화자 형태 강제 +
  분기 대입 금지). 수정 자체도 **분기가 아니라 공통 초기화 블록으로 승격**했다 —
  분기별 복사가 원인이었으므로 다음 movement 가 생길 때 같은 누락이 재발하지 않게.

## 골든 A/B — **23b 순효과 측정 (2026-09-07, C-1 수정 후)**

C-1 수정으로 23b 가 실제로 작동하게 됐고, 코퍼스가 **「23a 살아있음 + 23b 무효」** 상태로
구워져 있어서 **토글 없이 순수 23b 효과**를 잴 수 있었다.

**결과: 8/8 전부 일치(바이트 동일).**

### ⚠ 이번엔 「왜 안 움직였나」에 **구조적 답**이 있다 — 측정 실패가 아니라 관측 불가

리뷰가 재측정 «전» 에 확인하라고 한 항목(「그 seam 이 코퍼스에서 발화하는가」)을 확인했다.
**코퍼스는 23b 를 관측할 수 없다** — 우연이 아니라 구성상 그렇다:

| 23b 가 바꾸는 것 | 코퍼스에서 |
|---|---|
| 자폭(`Ability_MeleeBurst_Bruiser`) | **브루저가 덱에 없다**(배치되는 것은 아처·가디언·캐논·힐러·캐스터 3종) |
| 궁지폭발·실드폭발·진동갑주·시체폭발·작별선물·재앙의심장·퇴근운석 | **전부 드림캐쳐 카드** — 하네스는 카드를 안 붙인다 |
| 보스 도약 슬램 · 궁극기 강습 슬램 | 코퍼스 보스 `boss_jjangssen` 의 **`bodyRadius` 가 정확히 0.5** = 칸 반폭 → **Δ = 0** (⚠ 이 2건은 unit 24 에서 자리형으로 되돌렸다 — 같은 이유로 되돌릴 때도 Δ = 0) |

즉 **세 경로 모두 Δ 가 구조적으로 0** 이다. 「8/8 일치」는 **무회귀의 증거**이지
**「23b 가 아무것도 안 한다」의 증거가 아니다.** 두 문장을 섞지 말 것.

### 관측 가능하게 만들려면 (후속 후보)

- 코퍼스에 **브루저를 배치하는 시나리오**(자폭이 유일한 «유닛 저작» 자기 자리 폭발이다), 또는
- 몸이 0.5 가 **아닌** 보스(`nightmare` 0.615 · `mamemo` 0.558)가 나오는 장수 시나리오, 또는
- 하네스가 **카드를 붙일 수 있게** 확장(시체폭발의 음수 Δ 는 그때만 보인다).

⚠ **`boss_jjangssen` 의 몸이 0.5 인 것이 이 축의 사각을 만든다** — 「보스가 있으니 슬램은 잡힌다」로
읽지 말 것. 값이 칸 반폭과 같아 **판정식이 항상 같은 답**을 낸다.

## 스테일 어셈블리 사고 (2026-09-07) — 같은 함정을 내가 밟았다

C-1 수정 커밋(`fc6bfa7c`)의 「EditMode 어셈블리 2603 / 실패 0」은 **스테일 결과였다.**
그 시점에 컴파일 에러가 3건 있었다:
- `Tests/PlayMode/AttachRangePreviewTest.cs` ×3 — `DcRangeSpec` 기본 인자 제거의 여파
- `ProjectileOriginRadiusCarryTests` — `ProjectileData` using 누락
- `ReachEntryPointGuardTests` — `SkillsRoot` 를 없는 클래스에서 참조

**총계가 2770 에서 안 움직인 것이 정확히 그 신호였는데**(`test-procedure.md` 의 절), 타겟 실행이
통과했다는 이유로 「리포팅 아티팩트」로 넘겼다. 고친 뒤 실제 값은 **2777 = 2770 + 7** 로,
산수가 맞았고 신호가 옳았다.

> **덤으로 밝혀진 것**: Play 모드가 안 들어가던 것도 포커스가 아니라 **컴파일 에러**였다
> (`EnterPlaymode()` 가 `willPlay=True` 를 세웠다가 취소된다). 「Play 가 안 든다」를 만나면
> **콘솔 에러부터** 본다.

...
| `efc76750` | spec 종료 문서(인계 + 확인 기록) |
| `d09a2a99` | unit 8 — 화염구 다중화(팽이 2개, 위상 균등 분할) |
| `b4ebad82` | 튜닝 — 주기 5초(지속과 같아 끊김 없음) + 겹침 경고 임계 `<=`→`<` |
...
- **불꽃 팽이** — host 주위를 **화염구 2개가 180° 마주 보고** 돈다. 주기 5초 = 지속 5초라
  사실상 상시 회전. 스치는 적을 구슬마다 0.5초 간격으로 20씩. 부착 즉시 첫 발동.
  유닛 뒤로 돌면 몸에 가린다.
...
- `Battle/Combat/Projectile/Orbit.cs` — 궤적 수학 + `Tangent` + `PhaseOf`(2π/n 분할 소유)
...
- EditMode **2449 전량** — 잔여 실패 5건(`MapDocument_*` 4 · Whirlpot 1)은 이 작업과 무관한
...
- 위상 분할 3건(이웃 간격 2π/n · 위상이 시계를 안 건드림 · n<=1 안전).
- 사용자 Play 확인 완료 (2026-08-16) — 도는 그림 · 차폐 · 즉시 발동 · 크기/지속 · 2개 회전.
...
5. **위상은 각도만 옮긴다 — `elapsed` 오프셋으로 대신하지 말 것.** `elapsed` 는 수명도 재는
   값이라, 위상을 그걸로 흉내내면 그 구슬만 일찍 죽는다(`orbitPhase` 가 별도 축인 이유).
   2π/n 분할은 `Orbit.PhaseOf` 가 소유한다 — arm 이 다시 유도하면 각도 규약이 갈린다.
6. **겹침 경고 임계는 `<` 다.** 주기 == 지속은 «쌓임» 이 아니라 «이어짐» 이다(앞 구슬이
   죽는 프레임에 다음이 태어난다). `<=` 로 되돌리면 라이브 저작에서 매 부착마다 오경보.
7. **퇴근 스위트의 단정은 «운석 한 발만큼 빠졌나»** 이지 «체력이 그대로» 가 아니다. 대조군
...
- **원격 푸시** — 사용자 승인 대기 (GitHub `main` 이 정본, GitLab 은 `main:master` 미러).
...
- **밸런스 재점검** — 구슬 2개 × 상시 회전(주기 5 = 지속 5)이라 초당 피해가 초기 설계
  (1개 · 주기 6 · 지속 3)의 약 3배다. 세면 카드 `magnitude`(현재 20)를 내린다 — 코드 무변경.
- 기능 확장 후보는 README 「후속 후보」 참조 (`BossPeriodicTriggerSystem` 개명 ·
  재타격 쿨타임의 다른 소비자 · `OnRetire` × 다른 payload · 실아트 3장).

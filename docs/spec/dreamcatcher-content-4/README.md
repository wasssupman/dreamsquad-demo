...
2. **궤도 중심은 발사 시점 고정점이다.** 방어유닛은 타일 고정이라 host 를 추적할 필요가 없다
   → `BallisticArcToPoint` 계열(대상 엔티티 무참조).
   ⚠ **후속 정정(content-5, 2026-08-17 사용자 결정)**: 이 계약의 뒷문장은 «host 가 죽거나 퇴근해도
   이미 나간 화염구는 자기 수명을 산다» 였고 **뒤집혔다.** 화면에서 주인 없는 빈 자리에서 혼자 도는
   구슬이 됐기 때문이다. 이제 `owner` 가 사라지면 구슬도 즉시 사라진다(판정은 `ProjectileMoveSystem`
   의 궤도 arm). **중심이 고정점이라는 앞문장은 그대로다** — 위치 계산은 여전히 host 를 안 본다.

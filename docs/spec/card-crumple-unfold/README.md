...
## 아키텍처 노트 — 원래 의도(3D 카드덱), 미반영 (2026-07-13)

사용자 원래 의도는 **UGUI 가 아니라 3D 공간의 카드덱** 연출이었다(캔버스 없음). 3D 라면 카드가 진짜 quad
메시라 **깊이·라이팅·회전 있는 진짜 구김/접힘**이 자연스럽고, UGUI 채널 세금·Overlay Z-inert 한계가 없다.

**현재 구현 현실**: 드림캐쳐 손패 전체(deal-in: 아치·스프링·press·idle·덱드로우 + 드래그 타겟팅·TMP·트레이·
정렬)가 **UGUI(Screen Space Overlay)** 다. 그래서 UGUI 크럼플은 **평면 위 뒤틀림이 천장**이고, 3D 구김은 원천적
으로 안 나온다.

**3D 전환은 별도 spec 급 결정**(크럼플만이 아니라 완성된 deal-in 전체를 재작업 — 3D 카드 배치/입력 레이캐스트/
HUD 합성/z·마스킹). 하이브리드(카드만 3D quad + HUD 는 UGUI, 전용 카메라 합성) 또는 풀 3D. 지금은 미반영,
UGUI 평면 크럼플로 진행. 3D 카드덱을 진짜 추진하려면 새 spec 으로 스코핑할 것.

...
**확인 2026-08-13** — 사용자 Play 확인 통과(2단계 분열 상태). 커밋 `a4e8f26f`,
2단계 rev `12923018`, 리뷰 반영 `ceb36396`.

- sack 애니는 바이너리 실측으로 `fall-in`·`walk` 2종 확정(소문자). 기존 적은 대문자라
  그대로 베끼면 `FindAnimation` 이 조용히 실패한다 — 테스트로 고정.
- **라이브 덱 `attackUnitPool` 등록은 아직 하지 않았다.** 웨이브 baseline 이 바뀌므로 별도
  커밋이며, 현재 정상 플레이 도달 경로는 TEST MODE 플랜뿐이다.

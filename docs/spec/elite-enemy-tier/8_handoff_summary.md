...
- **밸런스 실전 검증** [지금 가장 급한 것] — `66004836` 의 수치는 TEST MODE 에서만 봤는데 위 편입으로 **라이브 웨이브에 실렸다**. 슬라임은 체력 2배 + 4기 분열이라 웨이브 처리 시간에 직접 영향을 준다. 이 값들의 회귀는 테스트가 잡지 못한다(단언이 전부 상대·구조형 — 의도된 것).

...
- **라이브 덱 풀 등록** — 지금은 `EnemyCatalog` 에만 있어 일반 플레이에 안 나온다. 풀 삽입은 웨이브 재추첨을 부르므로 별도 커밋 + 새 baseline.
- **밸런스 실전 검증** — `66004836` 의 수치는 판 위에서 미검증. 이 값들의 회귀는 테스트가 잡지 못한다(단언이 전부 상대·구조형이다 — 의도된 것).
- README 「후속 후보」 7항목 — 도형 어휘 통합 · 브레스 예고 · 3단계 분열 · 엘리트 HUD/아트 · `EnemyKilledEvent` 태그화 · 남은 5개 e2e 의 헬퍼 이관.

...
- **런타임 세대 카운터를 넣지 않는다**(계약 2). 집행은 **bake 시점의 사슬 검증**이 한다 —
  `SplitChain.Validate`(순환 · 깊이 `MaxDepth` · 자손 총수 `MaxTotalOffspring`). 런타임에 남긴
  것은 직접 자기순환 1줄 가드뿐이며, 그건 bake 경고를 무시하고 플레이한 경우의 안전판이다.
  (초판은 «자식 SO 에 메커니즘이 없어 재귀가 구조상 불가능» 이라고 적었는데 **2단계 분열이
  출하되면서 거짓이 됐다** — 중간 단계는 메커니즘을 갖는다. 2026-08-12 정정.)
...
**확인 2026-08-13** — 사용자 Play 확인 통과. 커밋 `bb74b476`, 리뷰 반영 `ceb36396`.

리뷰(투트랙)가 잡은 것 중 이 단위에 해당하는 것:
- **자식이 부모 셀을 벗어날 수 있었다** — 오프셋 기준점이 부모의 연속 좌표였다.
  `MovementCellTrim` 이 0.499 타일까지 이탈을 허용하므로 +0.25 면 인접 셀에 태어나고, 그 셀이
  골이면 «처치했는데 유출» 이 된다. 기준점을 **부모 셀 중심**으로 옮겨 해소(주석이 주장하던
  불변식이 그제서야 성립). e2e 단언도 셀 동일성으로 조였다.
- **④ 계약의 테스트가 거짓 보증이었다** — 「킬 집계와 자식 존재가 같은 관측」은 드레인 위치와
  무관하게 참이라 그 회귀를 못 잡는다. `_nextWaveIndex` 단언으로 교체하고 **뮤테이션으로
  증명**했다(드레인을 옛 자리로 되돌리면 `Expected 1 / But was 2` 로 빨개진다).
- 자손 총수 예산(`SplitChain.MaxTotalOffspring`) 추가 — 깊이·폭 상한이 독립이라 8⁸ 저작이
  둘 다 통과했다. + 카탈로그 전수 pin · 자기순환 런타임 안전판 · magnitude 초과 loud.

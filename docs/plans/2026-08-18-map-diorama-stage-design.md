...
맵 저작을 "에디터 창에서 셀 칠하기(MapPainter → MapDocument)"에서 **"씬에서 Ground + 프랍을 자유 배치하는 디오라마 방식"** 으로 전환한다. 비주얼은 Ground 메쉬/터레인 + 프랍 그 자체가 되고(자동 생성 타일맵 페인팅 은퇴), 논리는 지금처럼 **격자 위에서** 돈다 — 프랍 위치를 셀로 양자화해 논리 격자를 자동 파생한다. 레퍼런스: 로스트캐슬2, 컬트오브램(마당 구조) + **아르케랜드·창세기전 모바일**(자연 3D 맵 위 온디맨드 셀 오버레이 — 2026-08-18 이미지 대조 확인: 두 게임 모두 «수제 3D 지형 + 판정 시에만 칠해지는 셀 그리드» 구조이며, 우리 spec 의 오버레이 7채널 존치(unit 3)가 정확히 그 표현을 담당한다. 단 두 SRPG 의 다층 지형은 D4 로 배제 — 우리 판은 단층).
...
| D7 | 가디언 | **(a) 전 마당 배치 허용** — Path 전용 정체성 완화, 깊이 제한은 맵별 BlockZone 저작. SO 저작값은 유지(물 영역 도입 시 Water 층으로 전환할 자리) |

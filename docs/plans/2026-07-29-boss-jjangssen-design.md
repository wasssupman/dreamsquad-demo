# 짱쎈놈 보스 — 설계 (2026-07-29)

> 브레인스토밍 결과물(얇은 설계). 구현 단위와 계약 상세는 `docs/spec/boss-jjangssen/` 에 둔다.
...
- `displayName` = 짱쎈놈 · `id` = `boss_jjangssen`
...
웨이브에서 사라진다. **짱쎈놈의 시계는 초가 아니라 사건이다.**
...
구현은 `docs/spec/boss-jjangssen/` 에 `README.md` + `0_*.md` ~ `5_*.md` 로 쪼갠다.

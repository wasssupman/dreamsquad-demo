...
| `production_impact` | 정규 프로젝트에서도 상태마다 명시적 소유 모듈을 두고, 다른 모듈은 명령 또는 이벤트를 통해 변경을 요청한다. 서버가 authoritative state의 최종 쓰기 소유자다. |
| `next_validation_or_decision` | ADR-CAND-002에서 non-ECS 모듈 경계, write authority, transaction 경계를 확정한다. |
...
| `transfer_action` | `carry` |
| `production_impact` | plain input → plain output 계산과 그 테스트 벡터를 서버·클라이언트 공통 계약으로 삼을 수 있다. 언어·runtime이 달라지면 코드가 아니라 의미론과 golden vector를 이식한다. |
| `next_validation_or_decision` | ADR-CAND-003에서 수치 타입과 반올림 정책을 정한 뒤, 공유할 계산과 서버 전용 계산을 분류한다. |

`MatchSeed.GenerateRandom()`처럼 Unity나 로컬 시간에 의존하는 진입점은 이식 대상이 아니다. 파생 함수와 계산 계약만 후보로 삼는다.
...
| `transfer_action` | `adapt` |
| `production_impact` | 서버가 읽는 versioned canonical config와 hash를 정본으로 두고, 클라이언트 에셋은 같은 버전에서 파생된 표현 자료로 취급한다. |
...
| `transfer_action` | `adapt` |
| `production_impact` | 하나의 `NetworkBattleBridge`로 치환하지 않는다. input command, replicated state, presentation event, content mapping, match lifecycle을 별도 포트로 나눈다. |
...
| `transfer_action` | `adapt` |
| `production_impact` | 서버 내부 domain event와 외부 replication/presentation event를 구분한다. event ID, tick, ordering, deduplication, reliability, visibility를 명시한다. |
...
| `transfer_action` | `adapt` |
| `production_impact` | server tick·match ID·actor ID·content version을 포함하는 authoritative audit event를 만들고, metric·trace·replay 입력을 목적별로 분리한다. |
...
| `BattleBridge` | `adapt` | 명시적 경계와 변환 책임 | 단일 거대 gateway, 직접 ECS 변환 |
| `DynamicBuffer` / `NativeQueue` 이벤트 | `adapt` | producer/consumer, drain, ordering, lifecycle 계약 | 컨테이너 타입과 ECS singleton |
| ScriptableObject / Sheet import | `adapt` | 데이터 주도와 검증 | 클라이언트 로컬 에셋을 canonical truth로 취급 |
...
1. authoritative state와 점수의 최종 쓰기는 서버가 소유한다.
2. command는 유효성 검증 후 상태를 바꾸고, event는 이미 일어난 결과를 설명한다.
3. 모든 simulation-critical state에는 안정 ID, server tick, content version이 붙는다.
4. 계산은 가능한 한 순수하게 분리하고, runtime이 다르면 동일 test vector로 의미론을 고정한다.
5. reconnect는 로컬 pending 마감이 아니라 snapshot 복구와 command deduplication 문제로 다룬다.
6. 로그는 진단 문자열이 아니라 audit·metric·trace·replay 목적별 schema로 관리한다.
7. 위 원칙의 구체 기술은 후보 상태이며, 승인 전에는 정규 프로젝트 구현을 허가하지 않는다.

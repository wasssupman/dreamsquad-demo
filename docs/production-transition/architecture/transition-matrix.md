...
| `TRN-008` | 로컬 `BattleLogger`와 complete debug body | `adapt` | server audit·metric·trace·replay와 client presentation telemetry를 목적별 분리 | KPI·분석 이벤트와 보존 목적 정의 | UX·네트워크 품질, presentation version, 표시 event correlation 전송 | authoritative event·ruleset version·trace·보존·조회 소유 | ADR-CAND-011 |
...
| `TRN-015` | prediction·interpolation·reconciliation·resync | `decide` | 제한적 prediction을 비권위·폐기 가능하게 유지하고 항상 서버 판정으로 수렴 | 허용 rollback·보정 시각 기준 정의 | predict/interpolate/reconcile, correction UX, presentation side effect dedupe | authoritative snapshot·correction·resync 제공; Client 예측 결과를 수락 근거로 사용하지 않음 | ADR-CAND-006 |
...
| `TRN-020` | 관측성·감사·replay | `decide` | ruleset/presentation version과 input/command → authoritative event/tick → presentation event correlation으로 운영 진단·재현 수준 결정 | KPI·분쟁·분석 질문과 개인정보 제한 정의 | client quality signals, presentation version, 표시 event correlation 제공 | authoritative ruleset version, log, metric, trace, replay 보존·도구 제공 | ADR-CAND-011 |
...
| `TRN-008` | `fact` | `instrumented` | `E1` | 동일 기준선; client debug log 첨부 | [tournament-play-report](../../spec/tournament-play-report/README.md), [`BattleLogSchema.cs`](../../../Assets/_Project/Scripts/Logging/BattleLogSchema.cs) | server outcome과 client presentation event의 end-to-end correlation 검증 |
...
| `TRN-015` | `decision` | `untested` | `E0` | 동일 기준선; 지연·packet loss 조건 | 현 데모에 해당 구현 없음 | network simulation으로 correction rate·체감·presentation side effect dedupe 검증 |
...
| `TRN-020` | `decision` | `untested` | `E0` | 동일 기준선; authoritative observability 미구현 | [`BattleLogSchema.cs`](../../../Assets/_Project/Scripts/Logging/BattleLogSchema.cs), [tournament-play-report](../../spec/tournament-play-report/README.md) | ruleset/presentation version과 command/outcome/display correlation으로 incident reconstruction·replay fidelity 검증 |

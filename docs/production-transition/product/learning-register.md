...
> 이 문서의 2026-07-29 claim은 새 registry에서 current로 재검토하기 전에는 공식
> production input이 아니다. 특히 onboarding 흐름은 현행 spec과 drift가 등록돼 있다.
...
## LRN-PROD-013 — Live player와 Replay의 presentation 차이를 같은 경기로 이해하는지는 미검증이다

- `claim_kind`: `hypothesis`
- `evidence_status`: `untested`
- `evidence_level`: `E0`
- `as_of`: 2026-07-30, 정규 프로젝트의 Server-authoritative Replay 요구. Spectator는 도입이 결정된 경우에만 포함한다.
- 주장: Live player가 본 즉시 반응·정정·camera·UI와 Replay의 연출이 일부 달라도, 확정된 경기 진행·결과·핵심 인과가 일관되게 드러나면 사용자는 둘을 같은 경기로 이해하고 Replay를 신뢰한다. 같은 관점에서 공개되지 않아야 할 정보가 먼저 보이거나 핵심 사건의 순서·의미가 달라지면 이 가설은 성립하지 않는다.
- 근거 경로:
  ...
  - [온라인 권위 경험 학습](#lrn-prod-012--온라인-권위-전환의-반응성과-결과-이해는-미검증이다)
- 관련 커밋: 기준선 `44c87885`
- 관련 테스트: 없음 — 경기 진행의 자동 정합성 검사는 필요하지만 presentation 차이를 사용자가 어떻게 이해하는지에 대한 제품 근거를 대신하지 않는다.
- 증거 산출물: 없음
- `transfer_action`: `retest`
- 정규 프로젝트 영향: 실패하면 Replay가 경기 검토·공유·재도전 학습의 신뢰 가능한 근거가 되지 못하고, 조건부 Spectator에서도 같은 진행을 보고 있다는 인식이 깨질 수 있다.
- 다음 검증·결정: `VAL-PROD-009`에서 동일 경기를 Live player와 Replay로 비교해 결과·핵심 사건 인지, 관점 정책 준수와 신뢰를 측정하고, Spectator는 제품 범위에 포함될 때 같은 기준으로 추가 검증한다.
...
| Replay·조건부 Spectator 경험 | Live player와 Replay의 presentation 차이를 같은 경기로 받아들이는 근거 없음 | 확정 진행·핵심 인과의 이해와 관점 정책 준수를 검증하고 Spectator는 도입 시 같은 기준을 적용 |
